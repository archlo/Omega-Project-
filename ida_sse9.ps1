...
# Use py_eval to run custom IDA script for full decompilation
$pythonScript = @"
import idaapi
import idautils
import idc

def get_full_decompilation(addr):
...
result = get_full_decompilation(0x866530)
print(result)
"@

$jsonPayload = @"
{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"py_eval","arguments":{"expr":"$($pythonScript.Replace('"', '\"').Replace("`n", '\n'))"}}}
"@
...
[System.IO.File]::WriteAllText("C:\Users\jorge\OneDrive\Desktop\ts\ida_output\raw_response_py.txt", $fullData, [System.Text.Encoding]::UTF8)
...
if ($fullData -match "data: (\{.*\})") {
    ...
    try {
        ...
        if ($json.result.content) {
            ...
            # The py_eval result might have the code directly or in a nested JSON
        ...
        } elseif ($json.result.structuredContent.code) {
