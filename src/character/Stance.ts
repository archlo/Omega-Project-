...
/** Inverse of StanceMoveAction — decodes a MoveElement's `moveAction` byte
    ...
    idx 1 covers walk1+walk2) — the receiving client re-expands them via the
    avatar's own weapon entry (OG MoveAction2RawAction: idx 1 → `walkType != 1`
    picks walk2, idx 2 → `(standType != 1) + 2` picks stand2; CAvatar::
