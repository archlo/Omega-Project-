...
export class RPSGame extends GamePanel {
  ...
  private _title = new Text({ text: 'Rock Paper Scissors', style: _titleStyle });
  ...
  constructor() {
    super();
    this.container.position.set(352, 188);
  ...
  SetSubAction(subAction: number): void {
    // TODO_AUDIT.md Hundred-and-fifty-fourth pass: only CRPSGameDlg's inbound
    // sub-action byte is decoded; no verified play/claim sender is wired.
  ...
  private _drawChrome(): void {
    this._bg.clear().roundRect(0, 0, PanelW, PanelH, 5).fill({ color: 0x181820, alpha: 0.92 }).stroke({ color: 0xcccc88, width: 1 });
    this._closeBtn.clear().rect(136, 78, 58, 24).fill({ color: 0x1E2030, alpha: 0.9 }).stroke({ color: 0x707055, width: 1 });
  }
