...
export class WzPackage {
  ...
  GetItem(path: string): unknown {
    // The OG client's resource manager mounts UI.wz at a "UI/" root, so paths
    // like "UI/UIWindow2.img/Item/backgrnd" appear in decompiles. This NX set
    // exposes the same images at the package root (UIWindow2.img, Basic.img,
    // ...) with no "UI/" mount — resolve unprefixed first, then fall back to
    // the OG-prefixed form for other WZ layouts.
    if (path.startsWith('UI/')) {
      const unprefixed = this.Root.GetItem(path.substring(3));
      if (unprefixed !== null) return unprefixed;
    }
...
async function _existsAsync(path: string): Promise<boolean> {
  // Node: always use fs. `fetch` exists in Node 18+, but a bare relative
  // path is not a URL — fetch would throw/404 and silently fall back to the
  // encrypted .wz (garbage names). Browser: HEAD probe.
  const isNode = typeof process !== 'undefined' && !!process.versions?.node;
  if (isNode) return fileExists(path);
  try {
