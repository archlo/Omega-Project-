...
# Read all data from stream - accumulate until we see complete JSON
...
while ($timeout.Elapsed.TotalSeconds -lt 30 -and -not $jsonComplete) {
    try {
            ...
            if ($bytesRead -gt 0) {
                ...
                if ($fullText -match "data: ") {
                    ...
                    if ($braceCount -eq 0 -and $jsonPart.Length -gt 10) {
                        $jsonComplete = $true
                        Write-Output "JSON_COMPLETE at $($fullText.Length) bytes"
                    }
                }
            }
        }
