...
async function main() {
  ...
  sseRes.on('data', async (chunk) => {
    ...
    if (!sessionUrl) {
      ...
      if (m) {
        ...
        const payload = JSON.stringify({jsonrpc:'2.0',id:888,method:'tools/call',params:{name:'decompile',arguments:{addr:'0x706d30'}}});
        console.error('Sending decompile...');
        await postMsg(sessionUrl, payload);
        console.error('Request sent, waiting up to 300s...');
      }
      return;
    }
  ...
  setTimeout(() => { if (!done) { console.error('Timeout after 300s'); process.exit(1); } }, 300000);
}
