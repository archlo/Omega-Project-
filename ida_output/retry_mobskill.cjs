...
async function main() {
  ...
  sseRes.on('data', async (chunk) => {
    ...
    if (!sessionUrl) {
      ...
      if (m) {
        ...
        const payload = JSON.stringify({jsonrpc:'2.0',id:999,method:'tools/call',params:{name:'decompile',arguments:{addr:'0x706d30'}}});
  ...
  setTimeout(() => { if (!done) { console.error('Timeout'); process.exit(1); } }, 600000);
}
