...
export class AnimatedSprite {
    ...
    _accumMs = 0;
    _pixiSprite;
    constructor(frames, delaysMs) {
        if (frames.length === 0)
            throw new Error("At least one frame required");
        this._frames = frames;
    ...
    TotalDurationMs;
    get Current() { return this._frames[this._index]; }
    get Target() { return this._pixiSprite; }
    Update(dtMs) {
    ...
    _applyOrigin(frame) {
        this._pixiSprite.anchor.set(frame.width > 0 ? frame.OriginX / frame.width : 0, frame.height > 0 ? frame.OriginY / frame.height : 0);
    }
}
