 ...
 */
export class NxFile {
    static Magic = 0x34474B50; // "PKG4" little-endian
    _buf;
    _dv;
    _path;
    _nodeOffset;
     ...
     */
    static OpenLazy(path) {
        return new NxFile(null, path);
    }
    constructor(buffer, path = null) {
        this._buf = buffer;
        ...
        if (this._buf === null) {
            ...
            if (this._bitmapCount > 0) {
                ...
                this._bitmapOffsets = [];
                for (let i = 0; i < this._bitmapCount; i++)
                    this._bitmapOffsets.push(Number(bv.getBigUint64(i * 8, true)));
            }
            if (this._audioCount > 0) {
                ...
                for (let i = 0; i < this._audioCount; i++)
                    this._audioOffsets.push(Number(av.getBigUint64(i * 8, true)));
            }
    ...
    _readRange(offset, length, what = 'range') {
        if (this._buf !== null) {
    ...
    _checkRange(offset, length, what) {
        if (this._buf === null)
            return;
    ...
    _preloadStrings() {
        ...
        for (let i = 0; i < this._stringCount; i++) {
            ...
            if (local < 0 || local + 2 > blobBuf.byteLength) {
                this._stringCache[i] = '';
                continue;
            }
            const len = blobView.getUint16(local, true);
            if (local + 2 + len > blobBuf.byteLength) {
                this._stringCache[i] = '';
                continue;
            }
    ...
    GetChildren(node) {
        ...
        const result = new Array(node.childCount);
     ...
     */
    GetBitmapBgra(id, decompressedSize) {
...
//# sourceMappingURL=NxFile.js.map