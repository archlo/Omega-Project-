...
export async function readRangeBytesAsync(path, start, length) {
    if (length <= 0)
    ...
    if (res.status !== 206 && res.status !== 200)
        throw new Error(`Range fetch failed ${path} [${start}-${start + length - 1}]: ${res.status}`);
    return res.arrayBuffer();
}
/** Reads an exact byte range without downloading the whole file. Sync (blocking XHR/fs). */
export function readRangeBytesSync(path, start, length) {
    if (length <= 0)
        return new ArrayBuffer(0);
    if (isNode) {
        const fd = fs.openSync(path, 'r');
    ...
    if (xhr.status !== 206 && xhr.status !== 200)
        throw new Error(`Range fetch failed ${path} [${start}-${start + length - 1}]: ${xhr.status}`);
    const text = xhr.responseText;
    if (!text)
        throw new Error(`Empty range response for ${path} [${start}-${start + length - 1}]`);
    const bytes = new Uint8Array(text.length);
    for (let i = 0; i < text.length; i++)
        bytes[i] = text.charCodeAt(i) & 0xFF;
    return bytes.buffer;
}
export async function readFileBytesAsync(path, onProgress) {
    if (isNode) {
...
async function _fetchChunked(url, onProgress) {
    const head = await fetch(url, { method: 'HEAD' });
    if (head.status === 404)
        throw new Error(`File not found: ${url}`);
    if (!head.ok)
        throw new Error(`Failed to HEAD ${url}: ${head.status}`);
    ...
    for (let batch = 0; batch < chunkCount; batch += MAX_PARALLEL) {
        ...
        await Promise.all(Array.from({ length: batchEnd - batch }, (_, i) => {
            const idx = batch + i;
...
async function _fetchRange(url, start, end) {
    ...
    if (res.status !== 206)
        throw new Error(`Range fetch failed ${url} [${start}-${end}]: expected 206, got ${res.status}`);
    return res.arrayBuffer();
}
async function _fetchStream(url, total, onProgress) {
