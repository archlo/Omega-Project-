...
export class DamageDigits {
    ...
    _white = new Array(10).fill(null);
    _crit = new Array(10).fill(null);
    _miss = null;
    ...
    _digitSprites = new Map();
    _activeSlots = new Set();
    get LoadedWhite() { return this._white[0] !== null; }
    get LoadedCrit() { return this._crit[0] !== null; }
    get LoadedMiss() { return this._miss !== null; }
    constructor(effectWz, loader) {
        if (effectWz === null)
            return;
        for (const node of ['NoRed1', 'NoRed0', 'Basic']) {
            if (DamageDigits._tryLoadDigits(effectWz, loader, `${BasicEff}/${node}`, this._white))
                break;
        }
        for (const node of ['NoCri1', 'NoCri0', 'Cri']) {
            if (DamageDigits._tryLoadDigits(effectWz, loader, `${BasicEff}/${node}`, this._crit))
                break;
        }
        for (const path of [`${BasicEff}/NoMiss/0`, `${BasicEff}/Miss/0`]) {
            const mc = effectWz.GetItem(path);
            if (mc instanceof WzCanvas) {
                this._miss = loader.Load(mc);
                break;
            }
        }
    }
    ...
    beginFrame(liveSlotKeys) {
        for (const key of this._activeSlots) {
            ...
            for (let i = 0; i < MaxDigits; i++) {
                ...
                if (s) {
                    s.destroy();
                    this._digitSprites.delete(slotId);
                }
            }
            this._activeSlots.delete(key);
        }
    }
    DrawNumber(slotKey, text, screenCenter, alpha, crit) {
    ...
    DrawMiss(slotKey, screenCenter, alpha) {
        if (this._miss === null)
            return false;
        this._activeSlots.add(slotKey);
    ...
    _digitSprite(key, src) {
        ...
        if (!s) {
            ...
            s.anchor.set(src.Width > 0 ? src.OriginX / src.Width : 0, src.Height > 0 ? src.OriginY / src.Height : 0);
        }
        return s;
    }
    static _tryLoadDigits(wz, loader, basePath, dst) {
        for (let i = 0; i < 10; i++) {
...
//# sourceMappingURL=DamageDigits.js.map