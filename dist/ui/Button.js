...
export class Button {
    ...
    constructor(label) {
        ...
        this.container.__buttonInstance = this;
     ...
     * unavailable.
     */
    static fromWz(loader, buttonRoot, fallbackLabel) {
    ...
    static _loadFirst(loader, root, state) {
        const statePr = root?.Get(state);
        if (!(statePr instanceof WzProperty))
            return null;
    ...
    set position(v) { this.container.position.set(v.x, v.y); }
    hitTest(x, y) {
        const b = this.bounds;
