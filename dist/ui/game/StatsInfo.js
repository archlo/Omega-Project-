...
import { getIdealStatUp } from './StatDetailInfo.js';
...
export class StatsInfo extends GamePanel {
    ...
    static STAT_NAMES = ['STR', 'DEX', 'INT', 'LUK', 'AP', 'HP', 'MP'];
    ...
    _stringPool = null;
    _toolTip = null;
    constructor(loader, ui, stringPool) {
        super();
        ...
        const stat = ui?.GetItem('UIWindow2.img/Stat/main');
        if (stat) {
        ...
        if (stat) {
            const disabledProp = stat.Get('Disabled') instanceof WzProperty ? stat.Get('Disabled') : null;
            if (disabledProp) {
        ...
        if (stat) {
            ...
            if (this._btHpUp) {
                ...
                this._contentLayer.addChild(this._btHpUp.container);
            }
            if (this._btMpUp) {
                this._btMpUp.onClick = () => this.onMpUp?.();
                this._contentLayer.addChild(this._btMpUp.container);
            }
            if (this._btStrUp) {
                this._btStrUp.onClick = () => this.onStrUp?.();
                this._contentLayer.addChild(this._btStrUp.container);
            }
            if (this._btDexUp) {
                this._btDexUp.onClick = () => this.onDexUp?.();
                this._contentLayer.addChild(this._btDexUp.container);
            }
            if (this._btIntUp) {
                this._btIntUp.onClick = () => this.onIntUp?.();
                this._contentLayer.addChild(this._btIntUp.container);
            }
            if (this._btLukUp) {
                this._btLukUp.onClick = () => this.onLukUp?.();
                this._contentLayer.addChild(this._btLukUp.container);
            }
            // OG: Auto AP buttons — positioned by WZ, add with zIndex to ensure visibility
            if (this._btAuto) {
                this._btAuto.onClick = () => this.onAutoApUp?.(1);
                this._btAuto.container.zIndex = 50;
                this._contentLayer.addChild(this._btAuto.container);
            }
            if (this._btAuto1) {
                this._btAuto1.onClick = () => this.onAutoApUp?.(1);
                this._btAuto1.container.zIndex = 50;
                this._contentLayer.addChild(this._btAuto1.container);
            }
            if (this._btAuto2) {
                this._btAuto2.onClick = () => this.onAutoApUp?.(0);
                this._btAuto2.container.zIndex = 50;
                this._contentLayer.addChild(this._btAuto2.container);
            }
            if (this._btDetailOpen) {
                this._btDetailOpen.onClick = () => this.toggleDetail();
                this._contentLayer.addChild(this._btDetailOpen.container);
            }
        }
    }
    _loadWzSprite(loader, prop, name) {
        const node = prop.Get(name);
        return node instanceof WzCanvas ? loader.Load(node) : null;
    }
    _loadButton(loader, prop, name) {
    ...
    SetDerivedStats(atk, def, speed, jump) {
        this.atk = atk;
        this.def = def;
        this.speed = speed;
        this.jump = jump;
    ...
    get detailVisible() { return this._detailVisible; }
    toggleDetail() {
    ...
    _updateButtonStates() {
        ...
        for (const btn of [this._btHpUp, this._btMpUp, this._btStrUp, this._btDexUp, this._btIntUp, this._btLukUp]) {
            if (btn)
                btn.container.visible = showButtons;
        }
        ...
        if (hasAp && level < 20) {
            if (this._btHpUp && this._btHpUp.container.visible)
                this._btHpUp.enabled = false;
            if (this._btMpUp && this._btMpUp.container.visible)
                this._btMpUp.enabled = false;
        }
        ...
        if (this._btAuto)
            this._btAuto.container.visible = showButtons && bEnablea && !isAran;
        if (this._btAuto1)
            this._btAuto1.container.visible = showButtons && bEnablea && isAran;
        if (this._btAuto2)
            this._btAuto2.container.visible = showButtons && bEnablea && isAran;
    }
    update(_dt) {
        if (!this.isVisible)
            return;
    ...
    handleMouseButton(x, y, down) {
        ...
        for (const b of [this._btHpUp, this._btMpUp, this._btStrUp, this._btDexUp, this._btIntUp, this._btLukUp, this._btAuto, this._btAuto1, this._btAuto2, this._btDetailOpen]) {
            if (b?.handleMouseButton(x - this._root.x, y - this._root.y, down))
                return true;
        }
        const lx = x - this._root.x;
        const ly = y - this._root.y;
    ...
    autoApUp(mode) {
        ...
        if (remaining > 0) {
            if (alloc.luk > 0)
                alloc.luk += remaining;
            else if (alloc.intStat > 0)
                alloc.intStat += remaining;
            else if (alloc.dex > 0)
                alloc.dex += remaining;
            else
                alloc.str += remaining;
    ...
    _createBalloonTip(index, nX, nY, nDir, lines, font) {
