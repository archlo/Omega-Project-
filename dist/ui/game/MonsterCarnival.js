...
export class MonsterCarnival extends GamePanel {
    ...
    onGuardRequest = null;
    onMinionRequest = null;
    onBombRequest = null;
    onUncleRequest = null;
    onLock = null;
    // State
    _state = {};
    _activeTab = 0; // OG: m_nCurTab
    _guardItems = [];
    _minionItems = [];
    _specialItems = [];
    _selectedItem = -1;
    ...
    constructor(loader, uiWz) {
        ...
        this._scrollBar = new ScrollBar(LIST_X + LIST_W + 2, LIST_Y, LIST_ROWS * LIST_ROW_H, (pos) => { this._scrollOffset = pos; });
        ...
        this._makeButton(loader, uiWz, 'BtGuard', () => {
            if (this._selectedItem >= 0)
                this.onGuardRequest?.(this._selectedItem);
        });
        this._makeButton(loader, uiWz, 'BtMinion', () => {
            if (this._selectedItem >= 0)
                this.onMinionRequest?.(this._selectedItem);
        });
        this._makeButton(loader, uiWz, 'BtBomb', () => {
            if (this._selectedItem >= 0)
                this.onBombRequest?.(this._selectedItem);
        });
        this._makeButton(loader, uiWz, 'BtUncle', () => {
            if (this._selectedItem >= 0)
                this.onUncleRequest?.(this._selectedItem);
        });
        this.container.position.set(495, 82);
    }
    /** Set name resolver. */
    setResolvers(itemNameOf) {
    ...
    setItems(tab, items) {
        switch (tab) {
            case 0:
                this._guardItems = [...items];
                break;
            case 1:
                this._minionItems = [...items];
                break;
            case 2:
                this._specialItems = [...items];
                break;
        }
    ...
    _makeButton(loader, uiWz, name, onClick) {
        const prop = uiWz?.GetItem(`UIWindow2.img/MonsterCarnival/main/${name}`);
        if (!prop)
            return null;
    ...
    _drawCP() {
        ...
        personalLabel.x = CP_X;
        personalLabel.y = CP_PERSONAL_Y;
        ...
        personalValue.x = CP_X + 60;
        personalValue.y = CP_PERSONAL_Y;
        ...
        enemyValue.x = CP_X + 60;
        enemyValue.y = CP_ENEMY_Y;
    ...
    onMouseMove(_mx, _my) { }
    handleWheel(_dx, dy) {
        if (!this.isVisible || !this._scrollBar)
            return;
...
//# sourceMappingURL=MonsterCarnival.js.map