...
export class ToolTip {
    ...
    get toolTipType() { return this._toolTipType; }
    get lineSeparated() { return this._lineSeparated; }
    ...
    setBasicInfo(toolTipType, width, height, lineSeparate) {
        this.clearToolTip();
    ...
    makeLayer(left, top, doubleOutline, color = 0) {
        this._lastX = left;
        this._lastY = top;
    ...
    _initCanvas(doubleOutline, color) {
        this._bg.clear();
        // Fill background
    ...
    addInfo(text, type = FONT_TYPES.GEN_WHITE, align = 0, fontType) {
        if (this._lineNo >= 32)
            return;
    ...
    addInfoEx(mainType, subType, mainText, subText, align = 0, fontOverride) {
        if (this._lineNo >= 32)
            return;
    ...
    addOptionInfo(type, text, subType) {
        if (this._optionLineNo >= 32)
            return;
    ...
    drawTextSepartedLine(x1, x2, y, text, fontType, maxH = 9999) {
        if (!text || this._lineSeparated < 0)
            return 0;
    ...
    drawItemTitle(y, sText, bEquip = true, desc = '', titleColor, descColor) {
        if (!sText)
            return 0;
    ...
    drawTextEquipReq(x, y, label, value, met, fontType = FONT_TYPES.STAN_PRP) {
        if (value <= 0)
            return 0;
    ...
    drawTextEquipReqLevel(x, y, level, met) {
        if (level <= 0)
            return 0;
        const assets = this._assets;
        if (assets) {
    ...
    printValue(x, y, value, label, type = 0, fontType = FONT_TYPES.STAN_PRP) {
        if (value === 0 && type !== 1)
            return 0;
        let formatted;
        switch (type) {
    ...
    drawDiscountInfo(y, originalPrice, currentPrice) {
    ...
    drawNpcShopLimitedItemInfo(y, period) {
        if (period <= 0)
            return 0;
        const font = this.getFontByType(FONT_TYPES.GEN_WHITE);
    ...
    drawDiscountRate(x, y, originalPrice, discountPrice) {
    ...
    setToolTipSetItemBasic(setItemId, effects, equippedCount) {
    ...
    setToolTipItemOption(optionId, level, optionData) {
        ...
        if (optionId >= 901 && optionId <= 905) {
            if ((optionData.nProb ?? 0) <= 0 || (optionData.nTime ?? 0) <= 0)
                return;
        ...
        if (optionId === 20351 || optionId === 20352) {
            if ((optionData.nProb ?? 0) <= 0 || (optionData.nIgnoreDAM ?? 0) <= 0)
                return;
        ...
        if (optionId === 20201) {
            if ((optionData.nProb ?? 0) <= 0 || (optionData.niHP ?? 0) <= 0)
                return;
            this.addOptionInfo(FONT_TYPES.STAN_PRP, `HP Recovery: ${optionData.niHP}`, FONT_TYPES.STAN_NUM);
            return;
        }
        if (optionId === 20206) {
            if ((optionData.nProb ?? 0) <= 0 || (optionData.niMP ?? 0) <= 0)
                return;
    ...
    makePreviewPetNameTag(name, templateId, petProp, loader) {
        ...
        const loadCanvas = (node) => {
            if (node instanceof WzCanvas && loader)
    ...
    getPetDeadDate(petData, bShowLife, itemInfo) {
        if (!petData)
    ...
    showItemToolTip(x, y, itemId, name, desc = '', params) {
        this.clearToolTip();
    ...
    static getFontColor(type) {
        return FONT_COLORS[type] ?? FONT_COLORS[FONT_TYPES.GEN_WHITE];
    }
    static getFontSize(type) {
        return FONT_SIZES[type] ?? 11;
    }
    ...
    drawITCSaleInfo(y, nITCPrice, ftITCExpiredDate, sOrderComment, nItemCount, itcToolTip) {
        const font = this.getFontByType(FONT_TYPES.GEN_WHITE);
