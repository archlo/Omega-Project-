...
export class UserInfoExceptionList {
    ...
    _scrollBar = null;
    _scrollPos = 0;
    _items = []; // item IDs (0x7FFFFFFF = "All")
    _selectedIndex = -1;
    _mouseOverIndex = -1;
    // Buttons
    _btMeso = null;
    ...
    constructor(loader, ui) {
        this._loader = loader;
        const prop = ui?.GetItem('UIWindow2.img/UserInfo/exception');
        if (prop) {
            const bgNode = prop.Get('backgrnd');
            if (bgNode instanceof WzCanvas) {
                this._bg = loader.Load(bgNode);
                if (this._bg)
                    this.container.addChild(this._bg.ToPixi());
            }
            ...
            if (this._btMeso) {
                this._btMeso.onClick = () => this.onAddAll?.();
                this.container.addChild(this._btMeso.container);
            }
            if (this._btRegist) {
                this._btRegist.onClick = () => this.onOpenSearch?.();
                this.container.addChild(this._btRegist.container);
            }
            if (this._btDelete) {
                this._btDelete.onClick = () => this.onRemove?.(this._selectedIndex);
                this.container.addChild(this._btDelete.container);
            }
        }
        if (!this._bg) {
...
//# sourceMappingURL=UserInfoExceptionList.js.map