...
export function defaultStatInputs() {
...
export function resolvePrimarySecondary(s) {
...
export function primaryStatFlag(jobId) {
...
// (0x726690).
function resolveDamageBranch(s) {
    const category = Math.floor((s.jobId / 100) % 10);
    if (category === 2)
        return { primary: s.int, secondary: s.luk, tertiary: 0, k: 1.0, magic: true };
    if (s.jobId % 1000 === 0 || s.jobId === 2001)
        return { primary: s.str, secondary: s.dex, tertiary: 0, k: 1.2, magic: false };
    switch (s.weaponType) {
        case 30:
        case 31:
        case 32: return { primary: s.str, secondary: s.dex, tertiary: 0, k: 1.2, magic: false };
        ...
        case 40:
        case 41:
        case 42: return { primary: s.str, secondary: s.dex, tertiary: 0, k: 1.32, magic: false };
        case 43:
        case 44: return { primary: s.str, secondary: s.dex, tertiary: 0, k: 1.49, magic: false };
...
function getMasteryConstByWT(weaponType) {
    if (weaponType === 37 || weaponType === 38)
        return 0.25;
    if (weaponType === 45 || weaponType === 46 || weaponType === 47 || weaponType === 49)
        return 0.15;
    return 0.2;
}
function getEffectiveMastery(masteryFromSkill, weaponType) {
...
export function applyDefenseOption(getEquipOption, nDamage) {
    // Accumulate entries by probability — same-prob entries stack their values
    const flatByProb = new Map();
    const rateByProb = new Map();
    ...
    if (applied && nDamage <= 0)
        nDamage = 1;
    return nDamage;
}
export function applyWeaponOption(optionData) {
    const result = { criticalProb: 0, criticalDamage: 0, totalDAMr: 0, bossDAMr: 0, ignoreTargetDEF: 0 };
    if (!optionData)
        return result;
    if (optionData.niCr > 0)
        result.criticalProb = optionData.niCr;
    if (optionData.niCDr > 0)
        result.criticalDamage = optionData.niCDr;
    ...
    if (optionData.nIgnoreTargetDEF > 0)
        result.ignoreTargetDEF = optionData.nIgnoreTargetDEF;
    return result;
}
// OG: get_weapon_type (0x46F660) — extracts weapon type from item ID
// Returns (nItemID / 10000) % 100 for equip items
export function getWeaponType(itemId) {
    return Math.floor(itemId / 10000) % 100;
}
//# sourceMappingURL=StatDerived.js.map