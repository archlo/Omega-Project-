...
export class ChatBar extends GamePanel {
    ...
    onTabChange = null;
    onEmotion = null; // OG: SendEmotionChange
    onItemInfo = null; // OG: TryBeginShowItemInfo
    // --- OG state variables ---
    _chatType = CHAT_TYPE_MINIMAL;
    _chatHeight = 24;
    _chatWndY = 518; // m_ptChatWnd.y
    _chatWndLineVisible = 1; // m_nChatWndLineVisible
    _nScrWidth = SCROLLBAR_W; // m_nScrWidth = CCtrlScrollBar::GetScrollBarSize(1, 8)
    _dwChatFilterFlag = 0; // m_dwChatFilterFlag
    _nChatTarget = 0; // m_nChatTarget (0=all, per combo index)
    _tabCycleIndex = 0; // m_paramEdit.sEmptyImageUOL (tab cycle position)
    _activeTab = 0;
    _isFocused = false;
    _blinkTimer = 0;
    _cursorVisible = true;
    ...
    _whisperTarget = '';
    _whisperCandidate = []; // max 10
    // OG: CChatHelper spam detection (m_asRecent[4], m_dwChatTimeStamp[4], m_dwMutedTime)
    _recentMessages = []; // max 4
    _recentTimestamps = []; // timestamps for spam window
    _muteEndTime = 0; // m_dwMutedTime — when mute expires
    static SPAM_WINDOW = 2000; // 0x7D0 — 2s between identical msgs
    static SPAM_COUNT = 4; // 4 identical msgs triggers mute
    static MUTE_DURATION = 30000; // 30s mute (OG: 2800ms but 30s is standard)
    // OG: CChatHelper last-entry dedup (m_asHistory[8], m_nHistoryIndex)
    _lastSentText = ''; // for HistoryAdd dedup
    ...
    _dragScrollY = 0;
    _scroll = 0;
    _lastScrollTime = 0; // OG: m_dwLastScrolled
    // WZ layers (loaded in initWzAssets)
    _layerSpace = null; // chatSpace (display bg)
    _layerSpace2 = null; // chatSpace2
    _layerEnter = null; // chatEnter (input bg)
    _layerCover = null; // chatCover
    _layerChatBar = null; // tapBar (577×4 separator)
    _layerTapBarOver = null; // tapBarOver (hover state)
    _chatOpenButton = null;
    _chatCloseButton = null;
    ...
    _whisperIcons = [null, null, null, null];
    _channelDigits = [];
    // OG: filter button checked states (m_bChecked on each CCtrlOriginButton)
    _filterChecked = [true, false, false, false, false, false];
    // --- OG: ChatMessageDisplayTime / ChatMessageFadeTime ---
    static MSG_DISPLAY_MS = 10000; // 10s display
    static MSG_FADE_MS = 2000; // 2s fade
    // --- OG: Chat target label textures (WZ sprites per target type) ---
    _chatTargetLabels = [];
    // --- OG: Point notification animation (AP/SP) ---
    _apNotification = null;
    _spNotification = null;
    _apNotifTimer = 0;
    _spNotifTimer = 0;
    // --- OG: Shortcut tooltip text ---
    _shortcutTooltip = null;
    // --- OG: Chat enter texture (WZ) ---
    _chatEnterTexture = null;
    // --- OG: Whisper picker modal state ---
    _whisperPickerOpen = false;
    _whisperPickerSelectedIdx = 0;
    _whisperPickerScrollOffset = 0;
    _whisperPickerContainer = null;
    _whisperPickerRows = [];
    _whisperPickerComboText = null;
    _whisperPickerScrollGfx = null;
    _whisperPickerDropdownOpen = false;
    _whisperPickerDropdownScroll = 0;
    _whisperPickerDropdownRows = [];
    _whisperPickerDropdownContainer = null;
    // prev/next/ok/close buttons
    _whisperPickerPrevBtn = null;
    _whisperPickerNextBtn = null;
    _whisperPickerOkBtn = null;
    _whisperPickerCloseBtn = null;
    // WZ textures for whisper picker
    _whisperPickerDialogBg = null;
    _whisperPickerRowTextures = { selected: null, normal: null };
    _whisperPickerComboTextures = { normal: null, hover: null, pressed: null };
    _whisperPickerButtonTextures = { prevNormal: null, nextNormal: null, okNormal: null, closeNormal: null };
    // --- OG: Whisper picker modal layout constants (from StatusBarChatLayoutRules) ---
    static WP_MODAL_W = 260;
    static WP_COMBO_LEFT = 21;
    static WP_COMBO_W = 222;
    static WP_COMBO_H = 18;
    static WP_ROW_H = 16;
    static WP_VISIBLE_ROWS = 6;
    static WP_BUTTON_BOTTOM = 31;
    static WP_OK_LEFT = 157;
    static WP_CLOSE_LEFT = 198;
    // Chat log display lines (each is a Container with optional whisper icon + channel digits + text)
    _lines = [];
    _lineTexts = []; // shortcut to the Text child of each _lines[i]
    _maxLines = 1;
    ...
    get chatType() { return this._chatType; }
    get chatHeight() { return this._chatHeight; }
    setChatType(type, height) {
        if (type === this._chatType)
            return;
        this._chatType = type;
    ...
    _updateWzVisibility() {
        ...
        if (this._layerSpace)
            this._layerSpace.visible = !bCreate;
        // OG draws chatSpace2 as the persistent foreground/input layer; it is
        // present in both closed and open states.
        if (this._layerSpace2)
            this._layerSpace2.visible = true;
        if (this._layerEnter)
            this._layerEnter.visible = bCreate;
        if (this._layerCover)
            this._layerCover.visible = bCreate;
        if (this._chatOpenButton)
            this._chatOpenButton.container.visible = this._chatType === CHAT_TYPE_MINIMAL;
        if (this._chatCloseButton)
            this._chatCloseButton.container.visible = this._chatType !== CHAT_TYPE_MINIMAL;
        const showScrollControls = this._chatType === CHAT_TYPE_EXPANDED && this._scrollGfx.visible;
        if (this._scrollUpButton)
            this._scrollUpButton.container.visible = showScrollControls;
        if (this._scrollDownButton)
            this._scrollDownButton.container.visible = showScrollControls;
    ...
    _applyLayout() {
        ...
        if (this._layerSpace)
            this._layerSpace.position.set(DISPLAY_X, this._chatWndY);
        if (this._layerSpace2)
            this._layerSpace2.position.set(DISPLAY_X, this._chatWndY);
        if (this._layerEnter)
            this._layerEnter.position.set(DISPLAY_X, this._chatWndY);
        if (this._layerCover)
            this._layerCover.position.set(DISPLAY_X + DISPLAY_W_515 - 82, this._chatWndY);
    ...
    _rebuildLines(displayY, displayW) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    addLine(text, lTypeOrLinks = 0, channelID = -1, whisperIcon = false, itemLinks = []) {
        ...
        let sourceOffset = 0;
        const linksFor = (start, end, prefixLength) => itemLinks
    ...
    addMapleLine(text, itemNameFn, filterType = FILTER_ALL, lType = 0) {
    ...
    _getFilteredChatLogCount() {
        let count = 0;
        for (const e of this._chatLog) {
            if (this._isFiltered(e.lType))
                count++;
        }
        return count;
    }
    ...
    get chatTarget() { return CHAT_TARGET_INTERNAL[this._nChatTarget] ?? 'all'; }
    ...
    setChatTarget(target) {
        ...
        if (cycleIndex >= 0)
            this._tabCycleIndex = cycleIndex;
    ...
    handleMouseButton(x, y, down) {
        ...
        if (inDisplay) {
            ...
            const filtered = [];
            for (let i = 0; i < this._chatLog.length; i++) {
                if (this._isFiltered(this._chatLog[i].lType))
                    filtered.push(i);
            }
            const totalVisible = filtered.length;
            const scrollRange = Math.max(0, totalVisible - this._chatWndLineVisible + 1);
    ...
    _updateMessageFade(now) {
        for (let i = 0; i < this._lines.length; i++) {
            ...
            for (let j = 0; j < this._chatLog.length; j++) {
                if (this._isFiltered(this._chatLog[j].lType))
                    filtered.push(j);
            }
            const totalVisible = filtered.length;
            const scrollRange = Math.max(0, totalVisible - this._chatWndLineVisible + 1);
            ...
            if (visIdx < 0 || visIdx >= filtered.length)
                continue;
            const entry = this._chatLog[filtered[visIdx]];
            if (!entry.timestamp || !entry.displayMs)
                continue;
    ...
    _createPointNotification(text, color) {
    ...
    hideShortcutTooltip() {
        if (this._shortcutTooltip)
            this._shortcutTooltip.visible = false;
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    _buildWhisperPickerDropdown(y, x, w) {
        const candidates = this._whisperCandidate;
        const rowH = ChatBar.WP_ROW_H;
    ...
    initWzAssets(loader, ui) {
        ...
        const loadCanvas = (parent, name, x, y, visible = true) => {
            try {
                ...
                else if (typeof node.Get === 'function' && typeof node.ToPixi !== 'function') {
        ...
        const ctBase = bar.Get('chatTarget');
        if (ctBase) {
            this._combo.loadWzAsset(loader, ctBase, 'base');
        }
        const addControl = (name, onClick) => {
        ...
        const chatRoot = ui.GetItem('StatusBar2.img/chat');
        ...
        if (chatRoot) {
            for (let i = 0; i < Math.min(TAB_NAMES.length, 6); i++) {
                const tapRoot = chatRoot.Get('Tap');
                const tapNode = tapRoot?.Get(filterWzNames[i]);
                if (tapNode && typeof tapNode.Get === 'function') {
                    const normal = tapNode.Get('normal/0') ?? tapNode.Get('normal');
        ...
        if (chatRoot) {
            ...
            if (tapBarNode) {
                ...
                if (typeof tapBarNode.Get === 'function' && typeof tapBarNode.ToPixi !== 'function') {
                    const inner = tapBarNode.Get('0') ?? tapBarNode.Get('bmp');
                    if (inner)
                        canvasNode = inner;
                }
        ...
        if (chatRoot) {
            ...
            if (tapBarOverNode) {
                ...
                if (typeof tapBarOverNode.Get === 'function' && typeof tapBarOverNode.ToPixi !== 'function') {
                    const inner = tapBarOverNode.Get('0') ?? tapBarOverNode.Get('bmp');
                    if (inner)
                        canvasNode = inner;
                }
                if (canvasNode instanceof WzCanvas) {
        ...
        if (chatRoot) {
            ...
            if (!this._whisperIcons[0]) {
                ...
                if (statusBarRoot instanceof WzProperty) {
                    const chatNode = statusBarRoot.Get('chat');
                    if (chatNode) {
                        for (let i = 0; i < 4; i++) {
        ...
        const dlgRoot = ui.GetItem('StatusBar2.img/Chat/Dlg');
        if (dlgRoot) {
            ...
            if (selNode instanceof WzCanvas)
                this._whisperPickerRowTextures.selected = loader.Load(selNode);
            const rowNode = dlgRoot.Get('Row');
            if (rowNode instanceof WzCanvas)
                this._whisperPickerRowTextures.normal = loader.Load(rowNode);
            const btnPrev = dlgRoot.Get('BtPrev');
            if (btnPrev) {
                const n = btnPrev.Get('normal/0');
                if (n instanceof WzCanvas)
                    this._whisperPickerButtonTextures.prevNormal = loader.Load(n);
            }
            const btnNext = dlgRoot.Get('BtNext');
            if (btnNext) {
                const n = btnNext.Get('normal/0');
                if (n instanceof WzCanvas)
                    this._whisperPickerButtonTextures.nextNormal = loader.Load(n);
            }
            const btnOk = dlgRoot.Get('BtOK');
            if (btnOk) {
                const n = btnOk.Get('normal/0');
                if (n instanceof WzCanvas)
                    this._whisperPickerButtonTextures.okNormal = loader.Load(n);
            }
            const btnClose = dlgRoot.Get('BtClose');
            if (btnClose) {
                const n = btnClose.Get('normal/0');
                if (n instanceof WzCanvas)
                    this._whisperPickerButtonTextures.closeNormal = loader.Load(n);
            }
            const comboNode = dlgRoot.Get('Combo');
            if (comboNode) {
                const cn = comboNode.Get('normal/0');
                if (cn instanceof WzCanvas)
                    this._whisperPickerComboTextures.normal = loader.Load(cn);
                const ch = comboNode.Get('mouseOver/0');
                if (ch instanceof WzCanvas)
                    this._whisperPickerComboTextures.hover = loader.Load(ch);
            }
        }
        // OG: Chat target label textures (from StatusBar2.img/mainBar/chatTarget/label/*)
        const ctRoot = bar.Get('chatTarget');
        if (ctRoot) {
            const labelRoot = ctRoot.Get('label');
            if (labelRoot) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    handleMouseButtonGlobal(x, y, down) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    _floatNoticeText = null;
    _floatNoticeTimer = 0;
    _floatNoticeDuration = 0;
    floatNotice(text, duration = 5000) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    _buttonBlinkStates = [false, false, false, false];
    _setButtonBlink(index, blink) {
        if (index >= 0 && index < 4) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    setWhisperTargetFromCandidate(index) {
    ...
    getWhisperCandidateList() {
        return this._whisperCandidate;
    }
    onChangeWhisperTarget = null;
    _changeWhisperTarget(name) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    onChangeGroupWhisperTarget = null;
    changeGroupWhisperTarget() {
