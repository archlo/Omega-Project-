...
export class TournamentWindow extends GamePanel {
    ...
    setInfo(text) { this._lastInfo = text; this._redrawLines(); }
    setMatchTable(text) { this._lastMatchTable = text; this._redrawLines(); }
    setSetPrize(text) { this._lastSetPrize = text; this._redrawLines(); }
    setUew(text) { this._lastUew = text; this._redrawLines(); }
    _redrawLines() {
    ...
    handleMouseButton(x, y, down) {
        ...
        if (!down)
            return true;
        if (lx >= PANEL_W - 26 && lx < PANEL_W - 4 && ly >= 4 && ly < 20) {
            this.isVisible = false;
            return true;
        }
        return true;
