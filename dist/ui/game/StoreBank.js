...
export class StoreBank extends GamePanel {
    onGetAllConfirm = null;
    _bg = new Graphics();
    _title = new Text({ text: 'Store Bank', style: _titleStyle });
    _body = new Text({ text: '', style: _bodyStyle });
    _okBtn = new Graphics();
    _okLabel = new Text({ text: 'Get All', style: _btnStyle });
    ...
    handleMouseButton(x, y, down) {
        ...
        if (this._hit(lx, ly, 42, 110, 86, 24)) {
            if (this._action?.subAction === 0x24)
                this.onGetAllConfirm?.();
            return true;
        }
        if (this._hit(lx, ly, 166, 110, 70, 24)) {
    ...
    onKeyPress(key) {
        ...
        if (key === 'Enter' && this._action?.subAction === 0x24) {
            this.onGetAllConfirm?.();
            return true;
        }
        return false;
    ...
    _refresh() {
        ...
        if (a) {
            ...
            if (a.subAction === 0x24)
                lines.push(`Get all: ${a.passingDay ?? 0} days, fee ${a.fee ?? 0}`);
            if (a.subAction === 0x25)
                lines.push(`Account ${a.accountId ?? 0}, value ${a.value ?? 0}, ch ${a.channel ?? 0}`);
            if (a.subAction === 0x26)
                lines.push('Notice-only result');
        }
        else {
            lines.push('Action: -');
        }
...
//# sourceMappingURL=StoreBank.js.map