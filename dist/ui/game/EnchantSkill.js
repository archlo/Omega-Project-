...
export class EnchantSkill extends GamePanel {
    ...
    constructor(loader, ui, font) {
        ...
        if (!(es instanceof WzProperty))
            es = ui?.GetItem('IWindow2.img/EnchantSkill');
        const esProp = es instanceof WzProperty ? es : null;
        this._background = esProp?.Get('backgrnd') instanceof WzCanvas ? loader.Load(esProp.Get('backgrnd')) : null;
        ...
        for (let i = 0; i < this._skills.length; i++) {
            if (lx >= 10 && lx < PanelW - 10 && ly >= listY + i * itemH && ly < listY + (i + 1) * itemH) {
                if (this._selectedSlot === i)
                    this.OnEnchant?.(this._skills[i].slot);
...
//# sourceMappingURL=EnchantSkill.js.map