...
export class UserInfoDetail {
    ...
    constructor(loader, ui) {
        this._loader = loader;
        const prop = ui?.GetItem('UIWindow2.img/UserInfo/item');
        if (prop) {
    ...
    setItems(chairItems, wishItems) {
        // Interleave for rendering: [chair0, wish0, chair1, wish1, ...]
        this._items = [];
        const maxLen = Math.max(chairItems.length, wishItems.length);
        for (let i = 0; i < maxLen; i++) {
            if (i < chairItems.length)
                this._items.push(chairItems[i]);
            if (i < wishItems.length)
                this._items.push(wishItems[i]);
        }
        this._rebuild();
    }
    _rebuild() {
...
//# sourceMappingURL=UserInfoDetail.js.map