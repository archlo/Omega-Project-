...
export class Incubator extends GamePanel {
    _bg = new Graphics();
    _title = new Text({ text: 'Incubator', style: _titleStyle });
    _body = new Text({ text: '', style: _bodyStyle });
    _eggSlot = new Graphics();
    _eggSlotLabel = new Text({ text: 'Drop egg here', style: new TextStyle({ fill: '#888888', fontSize: 9, fontFamily: 'monospace' }) });
    _hintText = new Text({ text: '', style: new TextStyle({ fill: '#AAAAAA', fontSize: 9, fontFamily: 'monospace' }) });
    ...
    tryAcceptDrag(payload, _x, _y) {
        if (!this.isVisible || !payload || typeof payload !== 'object')
            return false;
        const p = payload;
        const id = p.itemId;
        if (!isIncubatorEgg(id))
