...
const Tabs = ['AVAILABLE', 'IN PROGRESS', 'COMPLETED', 'PARTY'];
...
export class QuestLog extends GamePanel {
    ...
    _levelFilters = new Array(8).fill(false);
    constructor(opts = {}) {
        super();
    ...
    _hitTestQuest(lx, ly) {
        if (lx < ContentX || lx >= ContentX + ContentW)
            return 0;
        if (ly < ContentY || ly >= ContentY + ContentH)
            return 0;
