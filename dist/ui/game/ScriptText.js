...
export class ScriptText {
    ...
    _bold;
    _widthCache = new Map();
    constructor(raw, wrapWidth, margin, font, boldFont, defaultColor, subst) {
        this._font = font;
        this._bold = boldFont;
    ...
    static _tokenize(raw, defaultColor, subst) {
        const atoms = [];
        ...
        const emit = (text) => {
            for (const ch of text) {
                ...
                if (rune === 0x0d)
                    continue;
                if (rune === 0x0a) {
                    atoms.push({ rune: 0, color, bold, linkIndex: link, newLine: true });
                    continue;
                }
    ...
    static _rawToken(raw, hashIndex, next) {
        return raw.substring(hashIndex, Math.min(next, raw.length));
    }
    _layoutAtoms(atoms, wrapWidth, margin) {
        ...
        for (const a of atoms) {
            ...
            if (lineWidth + w > usable && line.length > 0) {
                if (lastSpace >= 0 && lastSpace < line.length - 1) {
                    ...
                    lineWidth = 0;
                    for (const t of line)
                        lineWidth += this._runeWidth(t.rune, t.bold);
                    lastSpace = -1;
                    widthAtLastSpace = 0;
                }
                else {
