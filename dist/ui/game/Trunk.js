...
export class Trunk extends GamePanel {
    ...
    constructor(loader, ui, font) {
        ...
        this._background = trunkProp?.Get('backgrnd') instanceof WzCanvas ? loader.Load(trunkProp.Get('backgrnd')) : null;
        if (this._background)
            this.container.addChildAt(this._background.ToPixi(), 0);
        this._btClose = this._makeButton(loader, trunkProp, 'BtClose', () => this._close());
        this._applyLayout();
    }
    Open(money, trunkItems, invItems) {
    ...
    _refresh(money, trunkItems, invItems) {
        this._money = money;
        this._trunkItems = [...trunkItems];
        this._invItems = [...invItems];
        if (this._selected >= this._currentList.length)
            this._selected = -1;
    }
    SetMoney(money) { this._money = money; }
    get _currentList() { return this._tab === 0 ? this._trunkItems : this._invItems; }
    _close() {
        this.isVisible = false;
        this.OnClosed?.();
    }
    _promptMoney(callback) {
    ...
    _applyLayout() {
        if (this._btClose !== null)
            this._btClose.container.position.set(this.container.position.x + 418, this.container.position.y + 4);
    }
    update(_dt) { this._applyLayout(); }
    handleMouseButton(x, y, down) {
        ...
        if (down) {
            if (x >= px + 8 && x < px + 128 && y >= py + 24 && y < py + 44) {
                this._tab = 0;
                this._scroll = 0;
                this._selected = -1;
                return true;
            }
            if (x >= px + 136 && x < px + 256 && y >= py + 24 && y < py + 44) {
                this._tab = 1;
                this._scroll = 0;
                this._selected = -1;
                return true;
            ...
            if (x >= px + PanelW - 80 && x < px + PanelW - 12 && y >= py + ListBot + 8 && y < py + ListBot + 30
                ...
                if (this._tab === 0)
                    this.OnWithdraw?.(item.invType, item.position);
                else
                    this.OnDeposit?.(item.position, item.itemId, item.quantity > 0 ? item.quantity : 1);
                this._selected = -1;
                return true;
            }
        }
    ...
    onKeyPress(key) {
        ...
        if (key === 'ArrowUp' && this._selected > 0) {
            this._selected--;
            this._clampScrollToSelected();
            return true;
        }
        if (key === 'ArrowDown' && this._selected < list.length - 1) {
            this._selected++;
            this._clampScrollToSelected();
            return true;
        }
