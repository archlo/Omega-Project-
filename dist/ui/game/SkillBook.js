...
function isBeginnerJob(job) { return job % 1000 === 0; }
...
function getJobLevel(job) {
    if (isBeginnerJob(job) || job === 2001)
        return 1;
    const v1 = Math.floor(job / 10) === 43 ? (job - 430) / 2 : job % 10;
    const v2 = v1 + 2;
    if (v2 >= 2 && (v2 <= 4 || (v2 <= 10 && (Math.floor(job / 100) === 22 || job === 2001))))
        return v2;
    return 0;
}
...
function getJobChangeLevel(job, subJob, step) {
    ...
    if (v3 === 3 || Math.floor(job / 100) === 22 || job === 2001)
        return 200;
...
export class SkillBook extends GamePanel {
    ...
    _lastSkillUpTime = 0; // OG: 500ms cooldown between skill up requests
    // OG: ExtendSP — dual-blade extended SP tracking
    _extendSP = [0, 0, 0, 0]; // ExtendSP::Get(tab)
    // OG: Per-tab SP — beginner SP, job SP, extend SP tracked separately
    _noviceSp = 0; // SP for beginner tab (job % 1000 === 0)
    ...
    setExtendedSp(encoded) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (degree >= 0 && degree < next.length)
                next[degree] = value;
        }
        this._extendSP = next;
    ...
    _titleSecond;
    _spText;
    _tabSprites = []; // OG: WZ canvas sprites for tab backgrounds
    _tabLabels = [];
    ...
    _rowLevels = [];
    _rowCds = [];
    _rowBonuses = []; // OG: m_pFontBonus per row
    _rowSpBtns = [];
    _scrollBar;
    _macroBtn;
    ...
    _skillSlotNormalTex = null; // m_pCanvasSkill[0] — skill0
    _skillSlotEnabledTex = null; // m_pCanvasSkill[1] — skill1
    _rowSlotBgs = []; // one bg sprite per visible row (4 total)
    _recommendBgTex = null; // m_pCanvasRecommendSkill — recommend/0
    _rowRecommendBgs = []; // one recommend bg per row
    _lineBgTex = null; // m_pCanvasLine — line separator
    _background2 = null;
    _background3 = null;
    _rowLineBgs = []; // one line per row (rows 0-2 only)
    _bookIcon = null; // pBookIcon — book icon at (15, 55)
    _hoverIndex = -1;
    _recommendSkillId = 0; // from GetRecommendSKill
    _tooltip = null;
    _viewW = 1024;
    _viewH = 768;
    _mouseX = 0;
    ...
    constructor(loader, ui, font, icons, descOf, setItemOf, optionOf, itemInfo, strings) {
        ...
        if (loader && ui) {
            ...
            if (bg2Node instanceof WzCanvas) {
                ...
                if (this._background2)
                    this._root.addChild(this._background2);
            }
            ...
            if (bg3Node instanceof WzCanvas) {
                ...
                if (this._background3)
                    this._root.addChild(this._background3);
            }
                ...
                if (disabledProp instanceof WzProperty && enabledProp instanceof WzProperty) {
                    for (let i = 0; i < 5; i++) {
                        ...
                        if (dNode instanceof WzCanvas) {
                            ...
                            if (ws)
                                this._tabDisabledTex.push(ws.Texture);
                        }
                        if (eNode instanceof WzCanvas) {
                            const ws = loader.Load(eNode);
                            if (ws)
                                this._tabEnabledTex.push(ws.Texture);
                        }
                    }
                }
            }
            ...
            if (coolProp instanceof WzProperty) {
                for (let i = 0; i < 16; i++) {
                    ...
                    if (frame instanceof WzCanvas) {
                        ...
                        if (ws)
                            this._coolTimeTex.push(ws.Texture);
                    }
                }
            }
            // OG: Aran special tab buttons — Tab/AranButton/Bt1-Bt4
                    ...
                    for (let i = 1; i <= 4; i++) {
                        ...
                        if (btnProp instanceof WzProperty) {
                            ...
                            if (normal instanceof WzCanvas) {
                                ...
                                if (ws)
                                    this._aranBtnTex.push(ws.Texture);
                            }
                            if (disabled instanceof WzCanvas) {
                                const ws = loader.Load(disabled);
                                if (ws)
                                    this._aranBtnDisabledTex.push(ws.Texture);
                            }
                        }
                    }
                }
                    ...
                    if (dNode instanceof WzProperty && eNode instanceof WzProperty) {
                        for (let i = 0; i < 7; i++) {
                            ...
                            if (d instanceof WzCanvas) {
                                ...
                                if (ws)
                                    this._dualTabDisabledTex.push(ws.Texture);
                            }
                            if (e instanceof WzCanvas) {
                                const ws = loader.Load(e);
                                if (ws)
                                    this._dualTabEnabledTex.push(ws.Texture);
                            }
                        }
                    }
                }
        ...
        this.skillIncPanel.setOnSkillUp((skillId) => {
            ...
            if (!selected || !this._resetSelectedDecrease)
                return;
            this._resetSelectedIncrease = selected;
            this.skillIncPanel.isVisible = false;
            this.skillChangeConfirm.open(selected, this._resetSelectedDecrease, this.characterJob, this._resetOrigin.x, this._resetOrigin.y);
        });
        ...
        this._root.addChild(this.skillIncPanel.container, this.skillDecPanel.container, this.skillChangeConfirm.container);
        ...
        if (font && icons && loader && ui) {
            ...
            this._tooltip = new ItemTooltip(font, icons, assets, descOf ?? null, setItemOf ?? null, optionOf ?? null, itemInfo ?? null, strings ?? null);
        }
    }
    get tooltipContainer() { return this._tooltip?.root ?? null; }
    setViewSize(w, h) { this._viewW = w; this._viewH = h; }
    _getJobLevel(job) { return getJobLevel(job); }
    ...
    canSkillUp(skillId) {
        ...
        if (!skill || skill.level >= (skill.masterLevel > 0 ? skill.masterLevel : skill.maxLevel))
            return false;
        for (const [requiredId, requiredLevel] of info?.RequiredSkills ?? []) {
            if ((this._findSkill(requiredId)?.level ?? 0) < requiredLevel)
                return false;
        }
    ...
    onSkillLevelUp(skillId) {
        ...
        if (now - this._lastSkillUpTime < 500)
            return false;
        ...
        if (info && info.MaxLevel > 0) {
            ...
            if (sk && sk.level >= (sk.masterLevel > 0 ? sk.masterLevel : info.MaxLevel))
                return false;
        }
        ...
        if (isBeginnerJob(job) || isExtendspJob(job)) {
            ...
            if (!this.canSkillUpDualJob(skillId))
                return false;
        }
        else {
            if (!this.canSkillUp(skillId))
                return false;
        }
    ...
    setSkillRecords(records) {
        ...
        for (const root of roots) {
            for (const skillId of this.skillService?.EnumerateSkillIds(root) ?? [])
                ids.add(skillId);
        }
        ...
        for (const skillId of Array.from(ids).sort((a, b) => a - b)) {
            ...
            rows.push(new SkillRow(skillId, info?.Name || this.nameOf(skillId) || `Skill ${skillId}`, level, maxLevel, info?.Passive ?? false, record?.masterLevel ?? info?.DefaultMasterLev ?? maxLevel));
        }
        this.setSkills(rows);
    }
        ...
        const addJobRoots = (value) => {
            ...
            if (tier > 0) {
                ...
                if (branch > 0) {
                    ...
                    for (let i = 1; i <= 8 && value % 10 >= i; i++)
                        roots.add(branchRoot + i);
                }
            }
            ...
            if (value >= 2000 || Math.floor(value / 10) === 43)
                roots.add(value);
        };
        addJobRoots(job);
        for (const skillId of knownSkillIds)
            addJobRoots(Math.floor(skillId / 10000));
        if (isBeginnerJob(job) || job === 0)
            roots.add(0);
    ...
    rebuildTabs() {
        ...
        for (let i = 0; i < TAB_PREFIXES.length; i++)
            tabs.push([]);
        ...
        for (const sk of this._skills) {
            ...
            if (Math.floor(jobRoot / 10) === 43)
                hasDual = true;
            if (jobRoot === 2000 || (jobRoot >= 2100 && jobRoot < 2200))
                hasAran = true;
        }
        if (hasDual) {
        ...
        for (let i = 0; i < VISIBLE_ROWS; i++) {
            ...
            if (sk) {
                ...
                if (iconCanvas) {
                    ...
                    if (ws)
                        this._rowIcons[i].texture = ws.Texture;
                }
                ...
                if (i < 3)
                    this._rowLineBgs[i].visible = false;
            }
        }
    ...
    handleMouseButton(x, y, down) {
        ...
        if (this.skillIncPanel.handleMouseButton(x, y, down))
            return true;
        if (this.skillDecPanel.handleMouseButton(x, y, down))
            return true;
        if (this.skillChangeConfirm.handleMouseButton(x, y, down))
            return true;
        ...
        if (iconIdx >= 0) {
            ...
            if (abs < tab.length) {
                ...
                if (!sk.passive && sk.level > 0) {
                    ...
                    if (isDoubleClick)
                        this.onSkillUse?.(sk.id, sk.level);
                }
            }
            return true;
        }
    ...
    onMouseMove(x, y) {
        ...
        if (this._hoverIndex >= 0 && this._tooltip) {
            ...
            if (skill) {
                ...
                this._tooltip.DrawSkillTooltip(skill.id, skill.name, info?.Description || this.nameOf(skill.id), skill.level, skill.masterLevel > 0 ? skill.masterLevel : skill.maxLevel, info?.LevelDescriptionAt(skill.level) ?? '', info?.LevelDescriptionAt(skill.level + 1) ?? '', reqSkills, this._mouseX, this._mouseY + 20, this._viewW, this._viewH, true, info ? {
    ...
    onKeyPress(key) {
        ...
        if (this.skillIncPanel.onKeyPress(key))
            return true;
        if (this.skillDecPanel.onKeyPress(key))
            return true;
        if (this.skillChangeConfirm.onKeyPress(key))
            return true;
        if (key === 'Escape') {
            this.isVisible = false;
            return true;
    ...
    onTabChanged(newTab) {
        if (newTab < 0 || newTab >= this._tabs.length)
            return;
...
//# sourceMappingURL=SkillBook.js.map