...
export class CharacterSale extends GamePanel {
    onCheckName = null;
    _bg = new Graphics();
    _title = new Text({ text: 'Character Sale', style: _titleStyle });
    _body = new Text({ text: '', style: _bodyStyle });
    _checkBtn = new Graphics();
    _checkLabel = new Text({ text: 'Check Name', style: _btnStyle });
    ...
    SetCheckResult(id, resultCode) {
        this._lastCheck = { id, resultCode };
        this.isVisible = true;
        this._refresh();
    }
    SetCreateResult(mode, code) {
    ...
    handleMouseButton(x, y, down) {
        ...
        if (this._hit(lx, ly, 42, 110, 98, 24)) {
            const name = globalThis.prompt?.('Character name:') ?? '';
            if (name.trim())
                this.onCheckName?.(name.trim());
            return true;
        }
        if (this._hit(lx, ly, 178, 110, 70, 24)) {
...
//# sourceMappingURL=CharacterSale.js.map