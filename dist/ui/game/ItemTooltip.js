...
export class ItemTooltip {
    ...
    _bgAlpha = 240 / 255;
    _grade = 0;
    _blitSprites = []; // Track sprites added by BlitAt for cleanup
    // OG: SetToolTip_Equip parameters cache
    _equipWidth = EquipWidth;
    _toolTip;
    _optionOf;
    constructor(font, icons, assets, descOf = null, setItemOf = null, optionOf = null, itemInfo = null, strings = null) {
        this._font = font;
        this._icons = icons;
        this._assets = assets;
        this._descOf = descOf;
    ...
    SetPlayer(level, str, dex, intt, luk, job = 0) {
        this._pLevel = level;
        this._pStr = str;
        this._pDex = dex;
        this._pInt = intt;
    ...
    Draw(itemId, name, grade, _quantity, mouseX, mouseY, viewW, viewH, equippedSetCount = 0, petLevel, petTameness, petRepleteness, petRemainLife, equipStats, bundleOpts) {
    ...
    _string(id, fallback, ...args) {
        return (args.length > 0
            ? this._strings?.formatById(id, ...args)
            : this._strings?.getById(id)) ?? fallback;
    }
    _itemIcon(itemId) {
        return this._assets.LoadCanvas(this._itemInfo?.GetItemIconCanvas(itemId) ?? null)
            ?? this._icons.LoadIcon(itemId);
    }
    _txt(idx, x, y, text, color, size) {
    ...
    _clearTexts(from = 0) {
        for (let i = from; i < this._texts.length; i++)
    ...
    _drawEquip(itemId, name, grade, attr, mouseX, mouseY, viewW, viewH, equippedSetCount = 0, equipStats) {
        ...
        let y = mouseY + 16;
        if (x + w > viewW)
            x = Math.max(0, mouseX - w - 4);
        if (y + h > viewH)
            y = Math.max(0, viewH - h);
        this._g.clear();
        this._clearTexts(0);
        ...
        if (titleDesc)
            this._txt(1, nameX + nameW, yName, titleDesc, ToolTip.getFontColor(11), 9);
        ...
        for (let d = 4; d < w - 4; d += 6)
            this._dot(d, yName + lh + 4);
        ...
        for (const [label, val, met, isPop] of reqs) {
            ...
            if (val <= 0) {
                ...
                if (isPop) {
                    ...
                    if (none) {
                        this._blitAt(none, ReqValueRight - none.Width + none.OriginX, rowY - none.Height + none.OriginY);
                    }
                }
                else {
        ...
        for (let il = 0; il < info.length; il++) {
            ...
            const line = info[il];
            if (line.kind === 0 /* InfoKind.Text */ && line.text) {
    ...
    _drawConsumable(itemId, name, attr, mouseX, mouseY, viewW, viewH, petLevel, petTameness, petRepleteness, petRemainLife, bundleOpts) {
        const lh = this._font.lineHeight;
        ...
        let ti = 1;
        if (tradeOption) {
            this._txt(ti++, 0, 31, tradeOption, ToolTip.getFontColor(14), 10);
        }
        if (tradeOptionEx) {
            this._txt(ti++, 0, tradeOption ? 50 : 31, tradeOptionEx, ToolTip.getFontColor(14), 10);
        }
        if (expiryStr) {
            this._txt(ti++, 16, optionY + 29, expiryStr, ToolTip.getFontColor(22), 10);
        }
        if (titleLine) {
            this._txt(ti++, 16, optionY + 31 + (expiryStr ? 16 : 0), titleLine, ToolTip.getFontColor(14), 10);
        }
        if (donatorLine) {
            this._txt(ti++, 16, optionY + 31 + (expiryStr ? 16 : 0) + (titleLine ? 16 : 0), donatorLine, ToolTip.getFontColor(10), 9);
        }
        ...
        if (isPet) {
            for (const line of petLines) {
                this._txt(ti++, 10, yCursor, line, StatColor, 9);
                yCursor += lh - 1;
            }
        }
        if (protectedLine) {
        ...
        if (discountStr) {
            this._txt(ti++, 10, Math.max(descH, 68) + cashDescOffset + 40, discountStr, ToolTip.getFontColor(14), 10);
        }
        if (itcStr) {
    ...
    _drawGrowthItem(y, attr, isGrowth) {
        ...
        if (levLabel)
            this._blitAt(levLabel, 94, y);
        const expLabel = this._assets.GrowthLabel(1, isGrowth);
        if (expLabel)
            this._blitAt(expLabel, 94, y + 12);
        if (!isGrowth) {
        ...
        if (pctGlyph)
            this._blitAt(pctGlyph, 161, y + 12);
    }
    ...
    _buildInfoLines(_itemId, attr, equippedSetCount = 0, equipStats) {
        const lines = [];
        if (attr === null)
            return lines;
        const push = (text, color = InfoColor) => lines.push({ kind: 0 /* InfoKind.Text */, sprite: null, text, color });
        ...
        if ((attr.DurabilityMax ?? 0) > 0)
            push(this._string(OG_TOOLTIP_STRING_IDS.durability, 'Durability:'));
        ...
        const optional = attr;
        if (optional.IsPreventSlipItem || optional.preventSlip || optional.PreventSlip)
            push('Prevent Slip:');
        if (optional.IsSupportWarmItem || optional.supportWarm || optional.SupportWarm)
            push('Warm Support:');
        ...
        const optionLevel = Number(s?.level ?? 0);
        for (const optionId of [s?.option1 ?? 0, s?.option2 ?? 0, s?.option3 ?? 0]) {
            ...
            for (const [label, value, type] of optionRows)
                addStat(label, value, type);
        }
    ...
    _wrapText(text, maxW) {
        ...
        for (const paragraph of text.replace('\\r', '\n').replace('\\n', '\n').replace('\r', '').split('\n')) {
            ...
            for (const word of paragraph.split(' ')) {
                ...
                if (this._font.measure(trial).x > maxW && cur.length > 0) {
                    lines.push(cur);
                    cur = word;
                }
                else {
    ...
    DrawSkillTooltip(skillId, skillName, description, currentLevel, maxLevel, currentHelp, nextHelp, reqSkills, mouseX, mouseY, viewW, viewH, bShowLevel = true, 
    ...
    skillData) {
        ...
        const wrapInfo = (text) => text ? this._wrapText(text, w - 20) : [];
    ...
    DrawPetTooltip(petName, templateName, description, level, tameness, repleteness, isDead, deathStr, donator, expiryStr, skills, mouseX, mouseY, viewW, viewH, nOriginalPrice = 0, nPrice = 0, itemId = 0, goodsInfo) {
        const lh = this._font.lineHeight;
    ...
    DrawRingTooltip(ringName, description, partnerName, _ringType, expiryStr, itemId, mouseX, mouseY, viewW, viewH, equipStats, equipAttr, options) {
        const lh = this._font.lineHeight;
        const w = EquipWidth;
        ...
        const info = this._buildInfoLines(itemId, ringAttr, 0, equipStats);
        ...
        const ringH = ringImage ? (ringImage.Height ?? 68) : 0;
        ...
        for (const line of info) {
            if (line.text)
                this._txt(ti++, 10, yCursor, line.text, line.color ?? InfoColor, 9);
            yCursor += lh - 2;
        }
        if (info.length) {
            for (let d = 4; d < w - 4; d += 6)
    ...
    DrawMacroSysSkillTooltip(macroName, skills, mouseX, mouseY, viewW, viewH) {
    ...
    DrawSlotIncTooltip(itemName, description, category, slotIncrease, slotType, mouseX, mouseY, viewW, viewH) {
        const lh = this._font.lineHeight;
        const w = 290; // OG: 290px
    ...
    DrawEquipExtTooltip(itemName, isExpired, expiryStr, mouseX, mouseY, viewW, viewH) {
        const lh = this._font.lineHeight;
        const w = 290; // OG: 290px
    ...
    DrawString2Tooltip(lines, mouseX, mouseY, viewW, viewH) {
        const lh = this._font.lineHeight;
        const w = 200;
    ...
    DrawStringMultiLineTooltip(text, fontType, mouseX, mouseY, viewW, viewH) {
        const lh = this._font.lineHeight;
        const w = 290;
