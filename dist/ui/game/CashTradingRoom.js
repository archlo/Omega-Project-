...
export class CashTradingRoom extends GamePanel {
    ...
    OnChat = null;
    // State
    _myItems = new Array(GRID_COLS * GRID_ROWS).fill(null);
    _theirItems = new Array(GRID_COLS * GRID_ROWS).fill(null);
    _myMoney = 0;
    _theirMoney = 0;
    _myConfirmed = false;
    _theirConfirmed = false;
    _partnerName = '';
    // UI
    _bg;
    _dynamicChildren = [];
    ...
    constructor(loader, ui) {
        ...
        const cashTrade = ui?.GetItem('UIWindow2.img/CashTradingRoom');
        if (cashTrade) {
    ...
    putTheirItem(slot, itemId, quantity) {
        if (slot < 0 || slot >= this._theirItems.length)
            return;
    ...
    draw() {
        ...
        theirLabel.x = THEIR_GRID_X;
        theirLabel.y = GRID_Y - 14;
        ...
        myMoneyText.x = MY_GRID_X;
        myMoneyText.y = GRID_Y + GRID_ROWS * (SLOT_H + SLOT_GAP) + 10;
        ...
        theirMoneyText.x = THEIR_GRID_X;
        theirMoneyText.y = GRID_Y + GRID_ROWS * (SLOT_H + SLOT_GAP) + 10;
        ...
        if (this._myConfirmed) {
            ...
            myConf.x = MY_GRID_X + 100;
            myConf.y = GRID_Y + GRID_ROWS * (SLOT_H + SLOT_GAP) + 10;
        ...
        if (this._theirConfirmed) {
            ...
            theirConf.x = THEIR_GRID_X + 100;
            theirConf.y = GRID_Y + GRID_ROWS * (SLOT_H + SLOT_GAP) + 10;
            this._root.addChild(theirConf);
            this._dynamicChildren.push(theirConf);
        }
    }
    _drawGrid(items, baseX) {
...
//# sourceMappingURL=CashTradingRoom.js.map