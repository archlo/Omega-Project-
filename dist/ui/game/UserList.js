...
export class UserList extends GamePanel {
    ...
    _guildNameText;
    _titleText;
    _locationText = null; // OG: m_pFontLocation — friend location at (10, 285)
    ...
    onBlockAdd = null;
    onBlockDelete = null;
    onAllianceWithdraw = null;
    onAllianceInvite = null;
    onAllianceKick = null;
    onAllianceChangeMaster = null;
    onAllianceGradeChange = null;
    onAllianceSetNotice = null;
    onAllianceWhisper = null;
    onAlliancePartyInvite = null;
    getAllianceInviteName = () => '';
    getAllianceNotice = () => '';
    onExpeditionCreate = null;
    onExpeditionInvite = null;
    onExpeditionKick = null;
    onExpeditionWithdraw = null;
    onExpeditionChangeBoss = null;
    getExpeditionInviteName = () => '';
    ...
    constructor(loader, uiWz) {
                ...
                for (let i = 0; i < 6; i++) {
                    if (enabledProp instanceof WzProperty) {
                        ...
                        if (canvas instanceof WzCanvas) {
                            ...
                            if (s)
                                this._tabEnabledCanvases.push(s);
                        }
                    }
                    if (disabledProp instanceof WzProperty) {
                        ...
                        if (canvas instanceof WzCanvas) {
                            const s = loader.Load(canvas)?.ToPixi();
                            if (s)
                                this._tabDisabledCanvases.push(s);
                        }
                    }
                }
            }
        ...
        this._locationText.x = 10;
        this._locationText.y = 285;
    ...
    setUsers(users) { this._users = users; this._selFriend = -1; this._scrollOffset = 0; this._rebuild(); }
    updateFriendStatus(charId, online) {
    ...
    updateFriendEntry(charId, channel) {
        const entry = this._users.find(u => u.charId === charId);
        if (entry) {
    ...
    updateGuildMemberOnline(charId, online) {
    ...
    setParty(party) { this._party = party; this._selParty = -1; this._scrollOffset = 0; this._rebuild(); }
    setPartyBoss(bossCharId) {
        for (const m of this._party)
            m.isLeader = m.charId === bossCharId;
        this._rebuild();
    }
    updatePartyMemberStat(charId, level, job) {
    ...
    setGuild(name, members) { this._guildName = name; this._guild = members; this._selGuild = -1; this._scrollOffset = 0; this._rebuild(); }
    addGuildMember(entry) {
        if (!this._guild.some(m => m.charId === entry.charId)) {
            this._guild.push(entry);
            this._rebuild();
        }
    }
    removeGuildMember(charId) {
    ...
    get onlineFriendIds() { return new Map(this._users.filter(u => u.job !== 'Offline').map(u => [u.charId, u.name])); }
    get guildMemberIds() { return new Map(this._guild.filter(m => m.online).map(m => [m.charId, m.name])); }
    SetBlackList(names) { this._blackList = names; this._selBlock = -1; this._scrollOffset = 0; this._rebuild(); }
    ...
    setAlliance(name, members) { this._allianceName = name; this._alliance = members; this._selAlliance = -1; this._scrollOffset = 0; this._rebuild(); }
    get allianceMemberIds() { return new Map(this._alliance.map(m => [m.charId, m.name])); }
    setExpedition(members) {
        ...
        for (const m of members) {
            ...
            if (!arr) {
                arr = [];
                this._expeditionSubParties.set(m.subPartyIdx, arr);
            }
    ...
    _createGuildDynButtons() {
        for (const b of this._guildDynButtons)
    ...
    _createAllianceDynButtons() {
        for (const b of this._allianceDynButtons)
            this._root.removeChild(b);
    ...
    _setScrollBar() {
        ...
        switch (this._activeTab) {
            ...
            case TAB_INDICES.PARTY:
                itemCount = this._party.length;
                break;
            case TAB_INDICES.GUILD:
                itemCount = this._guild.length;
                break;
            case TAB_INDICES.ALLIANCE:
                itemCount = this._alliance.length;
                break;
            case TAB_INDICES.BLACKLIST:
                itemCount = this._blackList.length;
                break;
            case TAB_INDICES.EXPEDITION:
                itemCount = this._expeditionMembers.length;
                break;
        }
    ...
    _rebuildActionButtons() {
        ...
        switch (this._activeTab) {
            case TAB_INDICES.FRIEND:
                add('Add', () => { const n = this.getInviteName(); if (n)
                    this.onFriendAdd?.(n); });
                add('Delete', () => { if (this._selFriend >= 0)
                    this.onFriendDelete?.(this._users[this._selFriend]?.charId); });
                // OG: CTabFriend::OnWhisper (0x8D4CC0)
                add('Whisper', () => { if (this._selFriend >= 0)
                    this.onFriendWhisper?.(this._users[this._selFriend]?.name); });
                // OG: CTabFriend::OnGroupWhisper (0x8B7250)
                add('Group', () => { this.onGroupWhisper?.('all'); });
                // OG: CTabFriend::ChangeBlockOption (0x8B7280)
                add('Block', () => { if (this._selFriend >= 0)
                    this.onFriendBlock?.(this._users[this._selFriend]?.charId, true); });
                // OG: CTabFriend::OnFindFriendView (0x8B7270)
                add('Find', () => { this.onFindFriend?.(); });
                break;
            case TAB_INDICES.PARTY:
                add('Create', () => this.onPartyCreate?.());
                add('Invite', () => { const n = this.getInviteName(); if (n)
                    this.onPartyInvite?.(n); });
                add('Kick', () => { if (this._selParty >= 0)
                    this.onPartyKick?.(this._party[this._selParty]?.charId); });
            ...
            case TAB_INDICES.GUILD:
                if (this._guildName) {
                    ...
                    add('Invite', () => { const n = this.getInviteName(); if (n)
                        this.onGuildInvite?.(n); });
                    add('Kick', () => { if (this._selGuild >= 0) {
                        const g = this._guild[this._selGuild];
                        this.onGuildKick?.(g.charId, g.name);
                    } });
                    add('Admin', () => { if (this._selGuild >= 0) {
                        const g = this._guild[this._selGuild];
                        this.onGuildAdmin?.(g.charId, g.name);
                    } });
                    add('Expel', () => { if (this._selGuild >= 0) {
                        const g = this._guild[this._selGuild];
                        this.onGuildExpel?.(g.charId, g.name);
                    } });
                    add('Level', () => {
                        if (this._selGuild < 0)
                            return;
                        ...
                        if (raw !== null)
                            this.onGuildLevel?.(this._guild[this._selGuild].charId, parseInt(raw, 10));
                    });
                    add('Leave', () => this.onGuildLeave?.());
                }
                else {
                    add('Create', () => { const n = this.getGuildName(); if (n)
                        this.onGuildCreate?.(n); });
                }
                break;
            case TAB_INDICES.ALLIANCE:
                if (this._allianceName) {
                    add('Withdraw', () => this.onAllianceWithdraw?.());
                    add('Invite', () => { const n = this.getAllianceInviteName(); if (n)
                        this.onAllianceInvite?.(n); });
                    add('Notice', () => { const t = this.getAllianceNotice(); if (t)
                        this.onAllianceSetNotice?.(t); });
                ...
                break;
            case TAB_INDICES.BLACKLIST:
                add('Add', () => { const n = this.getBlockName(); if (n)
                    this.onBlockAdd?.(n); });
                add('Delete', () => { if (this._selBlock >= 0)
                    this.onBlockDelete?.(this._blackList[this._selBlock]); });
                break;
            case TAB_INDICES.EXPEDITION:
                add('Create', () => this.onExpeditionCreate?.());
                add('Invite', () => { const n = this.getExpeditionInviteName(); if (n)
                    this.onExpeditionInvite?.(n); });
                add('Kick', () => { if (this._selExped >= 0)
                    this.onExpeditionKick?.(this._expeditionMembers[this._selExped]?.charId); });
    ...
    handleMouseButton(x, y, down) {
        ...
        if (this._tab.handleMouseButton(tabLx, tabLy, down))
            return true;
