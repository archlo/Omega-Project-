...
(function (DialogType) {
    ...
    DialogType[DialogType["Next"] = 1] = "Next";
    DialogType[DialogType["PrevNext"] = 2] = "PrevNext";
    DialogType[DialogType["YesNo"] = 3] = "YesNo";
    DialogType[DialogType["Menu"] = 4] = "Menu";
    DialogType[DialogType["AskText"] = 5] = "AskText";
    DialogType[DialogType["AskNumber"] = 6] = "AskNumber";
    DialogType[DialogType["Quiz"] = 7] = "Quiz";
    DialogType[DialogType["AskAccept"] = 8] = "AskAccept";
...
export class NpcTalk extends GamePanel {
    ...
    constructor() {
        ...
        this._portraitArea.x = 6;
        this._portraitArea.y = 6;
    ...
    show(text, type = DialogType.Ok, sayMsgType = 0) {
    ...
    showQuiz(text, hint, min, max, time) {
        ...
        if (!this._quizText) {
            ...
            this._quizText.x = PANEL_W - 56;
            this._quizText.y = BTN_Y;
            this._root.addChild(this._quizText);
        }
        this.isVisible = true;
        this._root.visible = true;
    }
    showMenu(text, choices) {
    ...
    showAskText(text, _default, _min, _max) {
    ...
    showAskAccept(text, questId, npcId, x, y) {
    ...
    showAskNumber(text, _default, _min, _max) {
    ...
    onTextInput(ch) {
        ...
        if (ch.length === 1 && this._inputValue.length < 20) {
            this._inputValue += ch;
            this._syncInput();
        }
    }
    ...
    onKeyPress(key) {
        ...
        if (key === 'Escape') {
            if (this._currentType === DialogType.YesNo || this._currentType === DialogType.Quiz)
                this.onNo?.();
            this.isVisible = false;
            this._root.visible = false;
            return true;
    ...
    _rebuildButtons(type) {
        this._btnRow.removeChildren();
        const btnDefs = [];
        switch (type) {
            case DialogType.Ok:
                btnDefs.push({ label: 'OK', callback: this.onOk });
                break;
            case DialogType.Next:
                btnDefs.push({ label: 'Next', callback: this.onNext });
                break;
            case DialogType.PrevNext:
                btnDefs.push({ label: 'Prev', callback: this.onPrev }, { label: 'Next', callback: this.onNext });
                break;
            case DialogType.YesNo:
            case DialogType.AskAccept:
                btnDefs.push({ label: 'Yes', callback: this.onYes }, { label: 'No', callback: this.onNo });
                break;
