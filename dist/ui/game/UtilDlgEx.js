...
export var UtilDlgType;
(function (UtilDlgType) {
    UtilDlgType[UtilDlgType["TEXT"] = 0] = "TEXT";
    UtilDlgType[UtilDlgType["YESNO"] = 1] = "YESNO";
    UtilDlgType[UtilDlgType["INPUT"] = 2] = "INPUT";
    UtilDlgType[UtilDlgType["INPUT_STR"] = 3] = "INPUT_STR";
    UtilDlgType[UtilDlgType["LIST"] = 4] = "LIST";
    UtilDlgType[UtilDlgType["AVATAR"] = 5] = "AVATAR";
    UtilDlgType[UtilDlgType["PET"] = 6] = "PET";
    UtilDlgType[UtilDlgType["COMBOBOX"] = 7] = "COMBOBOX";
    UtilDlgType[UtilDlgType["MLINPUT"] = 8] = "MLINPUT";
    UtilDlgType[UtilDlgType["IMAGE"] = 9] = "IMAGE";
})(UtilDlgType || (UtilDlgType = {}));
...
// ─── OG helper functions ──────────────────────────────────────────────────
function getWndWidth(dlgType, bNoNPC) {
    switch (dlgType) {
        case UtilDlgType.TEXT:
        case UtilDlgType.YESNO:
        case UtilDlgType.INPUT:
        case UtilDlgType.INPUT_STR:
        case UtilDlgType.LIST:
        case UtilDlgType.COMBOBOX:
        case UtilDlgType.MLINPUT:
            return bNoNPC ? 260 : 519;
        case UtilDlgType.AVATAR:
        case UtilDlgType.PET: return 367;
        case UtilDlgType.IMAGE: return 418;
        default: return 0;
    }
}
function getWndHeight(dlgType) {
    if (dlgType === UtilDlgType.AVATAR || dlgType === UtilDlgType.PET)
        return 259;
    if (dlgType === UtilDlgType.IMAGE)
        return 348;
    return 0;
}
function getCTHeightMax(dlgType) {
    switch (dlgType) {
        case UtilDlgType.TEXT:
        case UtilDlgType.YESNO:
        case UtilDlgType.LIST: return 240;
        case UtilDlgType.INPUT:
        case UtilDlgType.INPUT_STR:
        case UtilDlgType.MLINPUT:
        case UtilDlgType.PET: return 0x7FFFFFFF;
...
function getCTHeightMin(dlgType, bNoNPC) {
    if (dlgType === UtilDlgType.COMBOBOX)
        return 300;
    return bNoNPC ? 0 : 110;
}
function getBasicCTWidth(dlgType, bNoNPC, bScrollBar) {
    return getWndWidth(dlgType, bNoNPC) - (bScrollBar ? 4 : 0);
}
function getBasicCTMargin(dlgType) {
    switch (dlgType) {
        case UtilDlgType.INPUT:
        case UtilDlgType.INPUT_STR:
        case UtilDlgType.MLINPUT:
        case UtilDlgType.PET: return 2;
        default: return 10;
    }
}
...
export class UtilDlgEx extends GamePanel {
    ...
    // ── Internal ────────────────────────────────────────────────────────────
    _lines = [];
    _apListCT = []; // OG: m_apListCT — selectable dot entries
    _scrollBar = null;
    _listItems = [];
    _bg = null;
    _contentLayer = new Container();
    _inputValue = '';
    _inputText = null;
    _inputCursor = null;
    _cursorBlink = 0;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    SetUtilDlgEx(dlgType, nTemplateID, bNoNPC, bQuest, sText) {
        ...
        if (dlgType === UtilDlgType.COMBOBOX)
            this._layoutInput();
        if (dlgType === UtilDlgType.TEXT)
            _activeDialogs.push(this);
    }
    // OG: SetUtilDlgEx_LIST (0x97e830) — collect nType==4 entries
    SetUtilDlgEx_LIST(bReset) {
        if (bReset)
            this._apListCT = [];
        for (const line of this._lines) {
            if (line.nType === 4)
                this._apListCT.push(line);
        }
        this.m_nListFocus = 0;
    }
    ...
    SetUtilDlgEx_IMAGE(bPrev, bNext) {
    ...
    SetUtilDlgEx_YESNO() { this._layoutGen(false); }
    // OG: SetUtilDlgEx_MSG — sets message image parameters
    SetUtilDlgEx_MSG(bMsgImage, bMsgImageImg) {
        this.m_bMsgImage = bMsgImage;
        this.m_bMsgImage_Img = bMsgImageImg;
    }
    SetUtilDlgEx_INPUT_STR(sStrDefault, nStrMin, nStrMax, bPasswd, bKoreanBaseLen) {
    ...
    SetUtilDlgEx_INPUT_NO(nDefault, nMin, nMax, nStrMin = 0, nStrMax = 0, bPasswd = false) {
    ...
    SetUtilDlgEx_INPUT_MLSTR(sStrDefault, nCol, nLine) {
        this.m_sInputDefault = sStrDefault;
        this.m_nInputCol = nCol;
        this.m_nInputLine = nLine;
        this._layoutMLInput();
    }
    SetUtilDlgEx_AVATAR(aCandidate, nAvatarType) {
    ...
    SetUtilDlgEx_PET(petInfos) {
        this.m_aPetInfo = [...petInfos];
        this.m_nPetIndex = 0;
        this._layoutGen(true);
    }
    SetUtilDlgEx_COMBOBOX(aStr) {
        this._lines = [];
        for (let i = 0; i < aStr.length; i++)
            this.AddDotLine(aStr[i], i, 5);
        this._layoutGen(false);
    }
    AddTextLine(text, nFuncCode = 0, fontIndex = 0) {
        this._lines.push({
            nType: 0, nItemNo: this._lines.length, nLine: 0, pFont: fontIndex,
    ...
    AddIconLine(iconPath, nLeft, nTop, conditional = true) {
        this._lines.push({
    ...
    AddFuncLine(nFuncCode, fontIndex = 0) {
        this._lines.push({
    ...
    AddDotLine(text, nSelect, fontIndex = 5) {
        this._lines.push({
    ...
    SetSpeakerOnRight(bRight) { this.m_bSpeakerOnRight = bRight; }
    AddImageList(sImagePath) {
    ...
    UpdateImage() { this._buildContent(); this._buildButtons(); }
    GetInputStr_Result() { return this.m_sInputStr_Result; }
    GetInputNo_Result() { return this.m_nInputNo_Result; }
    GetSelect() { return this.m_nSelect; }
    ...
    OnChildNotify(nId, nParam1, nParam2) {
    ...
    ValidateScroll(pCT) {
        ...
        if (ctTop < this.m_nScrollPos)
            this.m_nScrollPos = ctTop;
        else if (ctTop >= this.m_nScrollPos + this.m_scrHeight)
            this.m_nScrollPos = ctTop - this.m_scrHeight + 18;
        this.m_nScrollPos = Math.max(0, this.m_nScrollPos);
        this._updateListScroll();
    }
    SetKeyFocus(nBtnFocus) { this.m_nBtnFocus = nBtnFocus; }
    ForcedRet(nRet) { this._doTerminate(nRet); }
    // ═══════════════════════════════════════════════════════════════════════════
    // SetRet (OG: 0x982830)
    // ═══════════════════════════════════════════════════════════════════════════
    SetRet(nRet) {
        ...
        if (this.m_dlgType === UtilDlgType.INPUT) {
            ...
            if (!Number.isFinite(val) || val < 0) {
                this._showNotice('Please enter a valid number.');
                return;
            }
            if (val < this.m_nInputNo_Min) {
                this._showNotice(`Minimum value is ${this.m_nInputNo_Min}.`);
                return;
            }
            if (val > this.m_nInputNo_Max) {
                this._showNotice(`Maximum value is ${this.m_nInputNo_Max}.`);
                return;
            }
            ...
            if (this.m_sInputStr_Result.length < this.m_nInputStr_Min) {
                this._showNotice(`Minimum ${this.m_nInputStr_Min} characters required.`);
                return;
            }
        }
    ...
    _doTerminate(nRet) {
        ...
        this._fireResult({ type });
    }
    _showNotice(msg) { console.warn(`[CUtilDlgEx] ${msg}`); }
    _fireResult(result) {
    ...
    _buildContent() {
        ...
        switch (this.m_dlgType) {
            ...
            case UtilDlgType.INPUT:
            case UtilDlgType.INPUT_STR:
                this._buildInputContent(false);
                break;
            case UtilDlgType.MLINPUT:
                this._buildInputContent(true);
                break;
            case UtilDlgType.LIST:
            case UtilDlgType.COMBOBOX:
                this._buildListContent();
                break;
            case UtilDlgType.AVATAR:
                this._buildAvatarContent();
                break;
            case UtilDlgType.PET:
                this._buildPetContent();
                break;
            case UtilDlgType.IMAGE:
                this._buildImageContent();
                break;
        }
    }
    ...
    _buildTextContent() {
        ...
        for (let i = 0; i < this._lines.length; i++) {
            ...
            if (i > this.m_nCurDisplayItemIndex)
                break;
            const lineY = line.nTop || y;
            const drawY = lineY - this.m_nScrollPos;
            if (drawY < -48 || drawY >= clipH + 24) {
                y += line.nHeight || 18;
                continue;
            }
            switch (line.nType) {
                ...
                case 1: // Conditional icon (only when selected/reward/displayed)
                    if (line.nSelect === -1 && !line.bReward && i > this.m_nCurDisplayItemIndex)
                        break;
                // falls through
    ...
    _buildListContent() {
        ...
        for (let i = 0; i < this._lines.length; i++) {
            ...
            if (lineTop < -48 || lineTop >= this.m_scrHeight + 24) {
                y += h;
                continue;
            }
        ...
        if (this.m_bScrollBar) {
            this._scrollBar = new ScrollBar(clipW + this.m_ctLeft + 2, this.m_ctTop, this.m_scrHeight, (pos) => {
    ...
    _buildPetContent() {
        ...
        nameText.anchor.set(0.5, 0);
        nameText.x = this.m_ctLeft + 88;
        nameText.y = this.m_ctTop + 160;
        this._contentLayer.addChild(nameText);
        // Level at (166-w/2, 212)
        const lvlText = new Text({ text: `Lv.${pet.nLevel}`, style: this._fonts[0] });
        lvlText.anchor.set(0.5, 0);
        lvlText.x = this.m_ctLeft + 66;
        lvlText.y = this.m_ctTop + 177;
        ...
        tameText.x = this.m_ctLeft + 152;
        tameText.y = this.m_ctTop + 177;
    ...
    _buildButtons() {
        ...
        switch (this.m_dlgType) {
            ...
            case UtilDlgType.LIST:
                ...
                break;
            case UtilDlgType.AVATAR:
                this._onCreateAVATAR();
                break;
            case UtilDlgType.PET:
                this._onCreatePET();
                break;
            case UtilDlgType.COMBOBOX:
                this._onCreateCOMBOBOX();
                break;
            case UtilDlgType.MLINPUT:
                this._onCreateMLINPUT();
                break;
            case UtilDlgType.IMAGE:
                this._onCreateIMAGE();
                break;
    ...
    _makeButton(label, id) {
        const btn = new Container();
        btn.__btnId = id;
        btn.__btnLabel = label;
    ...
    onKeyPress(key) {
            ...
            if (hasBtnFocus && this.m_bFinishShow >= 0 && this.m_bFinishShow < this._apBtnFocus.length) {
                ...
                switch (this.m_dlgType) {
                    ...
                    case UtilDlgType.PET:
                        ...
                        break;
                    case UtilDlgType.IMAGE:
                        if (this.m_bImageNext)
                            this.OnButtonClicked(0x2001); // advance
                        else
                            this.OnButtonClicked(8193); // OK
                        break;
                }
            }
            return true;
        ...
        if (key === 'y' || key === 'Y') {
            if (this.m_dlgType === UtilDlgType.YESNO) {
                this.SetRet(6);
                return true;
            }
        }
        if (key === 'n' || key === 'N') {
            ...
            if (this.m_dlgType === UtilDlgType.IMAGE) {
                this.OnButtonClicked(0x2001);
                return true;
            }
        }
    ...
    handleMouseButton(x, y, down) {
        ...
        if (lx < 0 || lx > this.m_wndWidth || ly < 0 || ly > this.m_wndHeight)
            return true;
        if (this._scrollBar) {
            const sx = lx - this._scrollBar.container.x;
            const sy = ly - this._scrollBar.container.y;
            if (this._scrollBar.handleMouseButton(sx, sy, down))
                return true;
        }
        return true;
    }
    ...
    _rebuildTypewriterText() {
        ...
        for (const c of toRemove)
            this._contentLayer.removeChild(c);
        let y = this.m_ctTop;
    ...
    GetFont(index) { return this._fonts[Math.min(index, 11)]; }
}
//# sourceMappingURL=UtilDlgEx.js.map