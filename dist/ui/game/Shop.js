...
export class Shop extends GamePanel {
    ...
    _activeTab = 0; // 0=Buy, 1=Recommended, 2=Sell
    _buySelected = -1; // OG: m_nBuySelected
    _sellSelected = -1; // OG: m_nSellSelected
    _buyScrollOffset = 0;
    _sellScrollOffset = 0;
    // Meso display
    _meso = 0;
    ...
    _sellScrollBar = null;
    // WZ assets
    _canvasSelect = null; // OG: m_pCanvasSelect
    _canvasMeso = null; // OG: m_pCanvasMeso
    // Resolvers
    _itemNameOf = null;
    _itemIconOf = null;
    constructor(loader, ui) {
        ...
        const shop = ui?.GetItem('UIWindow2.img/Shop');
        if (shop) {
        ...
        this._buyScrollBar = new ScrollBar(BUY_LIST_X + BUY_LIST_W + 2, BUY_LIST_Y, BUY_LIST_ROWS * BUY_ROW_H, (pos) => { this._buyScrollOffset = pos; });
        this._root.addChild(this._buyScrollBar.container);
        this._sellScrollBar = new ScrollBar(SELL_LIST_X + SELL_LIST_W + 2, SELL_LIST_Y, SELL_LIST_ROWS * SELL_ROW_H, (pos) => { this._sellScrollOffset = pos; });
    ...
    setResolvers(itemNameOf, itemIconOf) {
     ...
     */
    setShopData(npcTemplateId, items) {
        this._npcTemplateId = npcTemplateId;
        this._buyItems = items;
        this._rechargeItems = items.filter(i => Math.floor(i.itemId / 10000) === PREFIX_THROW_ARROW ||
    ...
    _pollWheel() {
        ...
        const lx = mx - this._root.x;
        const ly = my - this._root.y;
        if (lx < 0 || lx >= 290 || ly < 0 || ly >= 260)
            return;
        if (typeof w.__wheelDelta !== 'number')
            return;
