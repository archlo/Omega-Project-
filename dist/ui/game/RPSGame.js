...
export class RPSGame extends GamePanel {
    ...
    _tSwitchingTerm = 0;
    _tLimit = 0;
    _nCntStraightVictories = 0;
    _bReceiveCompensation = false;
    _tShowResultLayer = 0;
    _tEndResult = 0;
    _nLastTipOption = 0;
    _nCurTipLength = 0;
    ...
    // UI elements
    _bg = new Graphics();
    _userLabel = new Text({ text: 'You', style: _labelStyle });
    _npcLabel = new Text({ text: 'NPC', style: _labelStyle });
    _userIcon = new Text({ text: '✊', style: new TextStyle({ fontSize: 48 }) });
    _npcIcon = new Text({ text: '❓', style: new TextStyle({ fontSize: 48 }) });
    _statusText = new Text({ text: '', style: _statusStyle });
    _resultText = new Text({ text: '', style: _resultStyle });
    _tipText = new Text({ text: '', style: _tipStyle });
    _timerText = new Text({ text: '', style: new TextStyle({ fill: '#FF6666', fontSize: 10, fontFamily: 'Arial' }) });
    _streakText = new Text({ text: '', style: new TextStyle({ fill: '#00FF88', fontSize: 10, fontFamily: 'Arial' }) });
    // RPS selection buttons (OG: m_pBtRPS[3])
    _rpsButtons = [];
    // Main action button (OG: m_pBtMain — dynamically swapped)
    _mainBtn = null;
    _mainBtnType = MAINBUTTON_START;
    ...
    handleServerResult(npcSelect, cntStraightVictories) {
    ...
    handleMouseButton(x, y, down) {
        ...
        // Title bar drag
        if (ly >= 0 && ly < 22)
            return this.beginDrag(lx, ly, true);
    ...
    // ── UI helpers ───────────────────────────────────────────────────────────
    _setRPSButtonsEnabled(enabled) {
        for (const btn of this._rpsButtons) {
            btn.enabled = enabled;
        }
        this._refreshDisplay();
    }
    _setMainButtonEnabled(enabled) {
        if (this._mainBtn)
            this._mainBtn.enabled = enabled;
        this._refreshDisplay();
    }
    _exitBtnSetEnabled(enabled) {
        if (this._exitBtn)
            this._exitBtn.enabled = enabled;
        this._refreshDisplay();
    }
    _setStatus(text) {
...
//# sourceMappingURL=RPSGame.js.map