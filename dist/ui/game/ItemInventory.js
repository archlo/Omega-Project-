...
const INVTYPE_TO_TAB = { 1: 0, 2: 1, 3: 2, 4: 3, 5: 4 };
...
export class InvItem {
    ...
    tab;
    slot;
    constructor(id, name = '', quantity = 1, tab = 0, slot = 0) {
        this.id = id;
        this.name = name;
...
export class ItemInventory extends GamePanel {
    _items = [];
    _slots = new Map(); // key = "${invType}-${pos}"
    _bg;
    ...
    _wzBg1 = null; // backgrnd / FullBackgrnd
    _wzBg2 = null; // backgrnd2 / FullBackgrnd2
    _wzBg3 = null; // backgrnd3 / FullBackgrnd3
    _wzFullBg1 = null;
    _wzFullBg2 = null;
    _wzFullBg3 = null;
    _wzDisabled = null;
    ...
    _tabBgs = [];
    _tabSprites = [];
    _wzTabEnabled = new Array(5).fill(null);
    _wzTabDisabled = new Array(5).fill(null);
    _tabWidths = [31, 31, 31, 31, 31];
    _tabLabels = [];
    _slotBgs = [];
    _slotLabels = [];
    _slotQtys = [];
    ...
    _extended = false;
    _activeUseSlot = 0;
    _activeProjectileWeaponType = 0;
    // OG: m_anSortBtState[6] — indexed by server invType (1-5), not visual tab.
    _arrangeState = [0, 0, 0, 0, 0, 0];
    _scrollOffset = [0, 0, 0, 0, 0];
    ...
    constructor(opts = {}) {
        ...
        const arrangeRoot = itemRoot ?? (opts.uiWz?.GetItem('IWindow2.img/Item') instanceof WzProperty
            ? opts.uiWz.GetItem('IWindow2.img/Item') : null);
    ...
    _setExtended(extended) {
        ...
        if (extended)
            this._scrollOffset[this._activeTab] = 0;
    ...
    _rebuildBackgroundSprite() {
        // Remove old WZ background layers
        this._root.children
            .filter((c) => c.label?.startsWith('itemInventoryBg'))
    ...
    clear() { this._items = []; this._slots.clear(); }
    ...
    setActiveUseSlot(slot) { this._activeUseSlot = slot; }
    ...
    setActiveProjectileWeaponType(weaponType) {
    ...
    setTryToReleaseItem(bTry, useSlot) {
        ...
        for (const op of ops) {
            ...
            switch (op.opType) {
                case 0: { // add
                    ...
                    if (op.equipStats)
                        this._icons?.SetRuntimeEquip(op.itemId, op.equipStats, op.attribute ?? op.equipStats.attribute);
                ...
                case 1: { // quantity change
                    ...
                    if (existing)
                        existing.quantity = op.quantity ?? 1;
                    break;
                }
                case 2: { // move
    ...
    _handleSlotClick(lx, ly) {
        ...
        const shiftKey = typeof window !== 'undefined' && window.__shiftKey === true;
        const ctrlKey = typeof window !== 'undefined' && window.__ctrlKey === true;
        ...
        if (icon)
            this.onDragStart?.({ itemId: item.id, slotPos: item.slot, invType: item.tab + 1 }, icon.Texture, this._mouseX, this._mouseY);
        const key = `${item.tab}-${item.slot}`;
    ...
    onKeyPress(key) {
        ...
        if (key === 'PageDown' && !this._extended) {
            this._scrollOffset[this._activeTab] = Math.min(this._scrollOffset[this._activeTab] + 1, maxScroll);
            this._rebuildGrid();
            return true;
        }
        if (key === 'PageUp' && !this._extended) {
            this._scrollOffset[this._activeTab] = Math.max(0, this._scrollOffset[this._activeTab] - 1);
            this._rebuildGrid();
            return true;
        }
        return false;
    ...
    tryAcceptDrag(payload, x, y) {
        ...
        if (p.slotPos < 0 && p.invType === 1 /* InventoryType.Equip */) {
            ...
            if (slotPos > 0) {
                ...
                if (free > 0)
                    this.onUnequipToInventory?.(p.invType, bodyPart, free);
            }
            return true;
        }
        // OG: CDraggableItem::MoveItemSlot — same-panel reorder via drag.
    ...
    _rebuild() {
        ...
        if (this._btFull)
            this._btFull.container.visible = !this._extended;
        if (this._btSmall)
            this._btSmall.container.visible = this._extended;
                ...
                if (this._activeTab === 0 && item.equipStats && item.equipStats.grade > 0) {
                    ...
                    if (item.equipStats.released) {
                        switch (item.equipStats.grade) {
                            case 2:
                                gradeColor = 0xFF5CA1FF;
                                break;
                            case 3:
                                gradeColor = 0xFFC261FF;
                                break;
                            case 4:
                                gradeColor = 0xFFFFCC00;
                                break;
                        }
                    }
    ...
    get _panelW() { return this._extended ? FULL_PANEL_W : PANEL_W; }
    /** OG: CCtrlTab stores tab items with their canvas widths. Tab i starts at
     *  TAB_X + sum(widths[0..i-1]) + i * TAB_GAP. */
    _tabX(i) {
        let x = TAB_X;
        for (let j = 0; j < i; j++)
            x += this._tabWidths[j] + TAB_GAP;
        return x;
    }
    /** Which tab index was clicked, or -1 if none. Mirrors CCtrlTab hit-test. */
    _tabIndexAtPoint(lx, ly) {
        if (ly < TAB_Y || ly >= TAB_Y + 19)
            return -1;
        for (let i = 0; i < 5; i++) {
            const tx = this._tabX(i);
            if (lx >= tx && lx < tx + this._tabWidths[i])
                return i;
        }
        return -1;
    }
    _slotPositionForCell(col, row, firstPos) {
        if (!this._extended)
            return firstPos + row * COLS + col;
        const block = Math.floor(col / COLS);
        return firstPos + block * (COLS * ROWS) + row * COLS + (col % COLS);
    }
    _slotPositionFromPoint(lx, ly) {
    ...
    _updateActiveUseSlot() {
        ...
        const wantedPrefix = this._activeProjectileWeaponType === 47 ? THROWING_STAR_PREFIX
            : this._activeProjectileWeaponType === 49 ? BULLET_PREFIX
                : 0;
        for (const item of this._items) {
            if (item.tab !== 1 || item.quantity <= 0)
                continue;
            const prefix = Math.floor(item.id / 10000);
            if (wantedPrefix !== 0 ? prefix !== wantedPrefix : (prefix !== THROWING_STAR_PREFIX && prefix !== BULLET_PREFIX))
                continue;
            if (best === 0 || item.slot < best)
                best = item.slot;
        }
    ...
    _slotRectForPosition(slot) {
        ...
        if (col < 0 || col >= maxCols || row < 0 || row >= ROWS)
            return null;
        return { x: GRID_X + col * SLOT_STEP_X, y: GRID_Y + row * SLOT_STEP_Y };
    }
}
//# sourceMappingURL=ItemInventory.js.map