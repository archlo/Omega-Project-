...
export function isUnderMinMP(job, newMMP) {
    const jobCat = Math.floor((job % 1000) / 100);
    let minMP = 6; // default
    if (jobCat === 2)
        minMP = 20; // mages have higher minimum
    return newMMP < minMP;
}
export function isOverMaxHP(_job, newMHP) {
    return newMHP > 30000;
}
export function isOverMaxMP(_job, newMMP) {
    return newMMP > 30000;
}
// ── OG: AP budget calculation (from SetButtons @ 0x869910) ──────────────
export function calcTotalAP(level, job) {
    ...
    if (Math.floor(job / 1000) === 0) {
        ...
        if (subJob === 1 && jobLevel === 2)
            nSumAP += 1;
        if (subJob === 1 && jobLevel === 3)
            nSumAP += 2;
    }
    return nSumAP;
}
...
export class StatChangeDialog extends GamePanel {
    ...
    _drawStats() {
        // Clear old texts
        for (const t of this._statTexts) {
    ...
    _updateButtons() {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (isIncrement) {
                ...
                if (i <= 1 && availableAP <= 0)
                    enabled = false;
                // OG: Disable HP if over max
                if (i === 0 && isOverMaxHP(this.job, this.maxHp + incHPVal(this.job)))
                    enabled = false;
                // OG: Disable MP if over max
                if (i === 1 && isOverMaxMP(this.job, this.maxMp + incMPVal(this.job)))
                    enabled = false;
                ...
                if (i === 3 && this.dex <= 4)
                    enabled = false;
                if (i === 4 && this.intStat <= 4)
                    enabled = false;
                if (i === 5 && this.luk <= 4)
                    enabled = false;
                // OG: Job-specific minimums
                const jobCat = Math.floor((this.job % 1000) / 100);
                if (jobCat === 1 && this.str <= 35)
                    enabled = false; // Warrior
                if ((jobCat === 3 || jobCat === 4) && this.dex <= 25)
                    enabled = false; // Archer/Thief
                if (jobCat === 2 && this.intStat <= 20)
                    enabled = false; // Mage
                if (jobCat === 5 && this.dex <= 20)
                    enabled = false; // Pirate
                // OG: Disable HP/MP if under min
                if (i === 0 && isUnderMinHP(this.job, this.maxHp - decHPVal(this.job)))
                    enabled = false;
                if (i === 1 && isUnderMinMP(this.job, this.maxMp - decMPVal(this.job)))
                    enabled = false;
            }
    ...
    handleMouseButton(x, y, down) {
        ...
        for (const btn of this._buttons) {
            if (btn.handleMouseButton(lx, ly, down))
                return true;
        }
        if (this._cancelButton?.handleMouseButton(lx, ly, down))
            return true;
...
export class StatChangeConfirmDialog extends GamePanel {
    ...
    _rebuild() {
        ...
        if (statIdx >= 0) {
            ...
            if (this.dwDec !== 0) {
                // Decrement
                if (statIdx === 0)
                    newVal = oldVal - decHPVal(this.job);
                else if (statIdx === 1)
                    newVal = oldVal - decMPVal(this.job);
                else
                    newVal = oldVal - 1;
            }
                ...
                // Increment
                if (statIdx === 0)
                    newVal = oldVal + incHPVal(this.job);
                else if (statIdx === 1)
                    newVal = oldVal + incMPVal(this.job);
                else
                    newVal = oldVal + 1;
            }
    ...
    handleMouseButton(x, y, down) {
        ...
        const lx = x - this._root.x;
        const ly = y - this._root.y;
        if (this._okButton?.handleMouseButton(lx, ly, down))
            return true;
        if (this._cancelButton?.handleMouseButton(lx, ly, down))
            return true;
...
//# sourceMappingURL=StatChangeDialog.js.map