...
function nudge(sc, dx) {
    const p = Cells.get(sc);
    if (p !== undefined)
        Cells.set(sc, { x: p.x + dx, y: p.y });
}
export function getBindableScancodes() {
    return Array.from(Cells.keys());
}
export function tryGetCell(scancode) {
...
export function keyLabelOffset(scancode) {
    if (scancode === ScLShift || scancode === ScRShift)
        return { dx: 2, dy: 4 };
    if (scancode === 57)
        return { dx: 0, dy: 4 };
    return { dx: 4, dy: 4 };
}
export function paletteCell(slot) {
    return Palette[slot];
}
export function hitTestKey(x, y) {
...
export function hitTestPalette(x, y) {
    for (let i = 0; i < PaletteCount; i++) {
        const p = Palette[i];
        if (x >= p.x && x < p.x + CellSize && y >= p.y && y < p.y + CellSize)
            return i;
...
export function paletteBinding(slot) {
    if (slot < 30)
        return { type: 4 /* FuncKeyType.Menu */, id: slot };
    if (slot < 35)
        return { type: 5 /* FuncKeyType.BasicAction */, id: 50 + (slot - 30) };
    return { type: 6 /* FuncKeyType.BasicMotion */, id: 100 + (slot - 35) };
}
export function paletteSlotOf(fk) {
    if (fk.type === 4 /* FuncKeyType.Menu */ && fk.id >= 0 && fk.id < 30)
        return fk.id;
    if (fk.type === 5 /* FuncKeyType.BasicAction */ && fk.id >= 50 && fk.id <= 54)
        return 30 + (fk.id - 50);
    if ((fk.type === 6 /* FuncKeyType.BasicMotion */ || fk.type === 3 /* FuncKeyType.Emotion */) && fk.id >= 100 && fk.id <= 106)
        return 35 + (fk.id - 100);
    return -1;
}
//# sourceMappingURL=KeyConfigLayout.js.map