...
export class QuestDetail extends GamePanel {
    ...
    _summaryTexts = [];
    _scrollbarG;
    constructor(loader, ui, npcWz, font) {
        super();
        this._loader = loader;
        this._npcWz = npcWz;
        this._font = font;
        ...
        this._btAccept = this._buttonFrom(loader, qiProp, 'BtArlim', () => { if (this._quest)
            this.OnRemoteAccept?.(this._quest.Id); });
        this._btResign = this._buttonFrom(loader, qiProp, 'BtGiveup', () => { if (this._quest)
            this.OnResign?.(this._quest.Id); });
        this._btFindNpc = this._buttonFrom(loader, qiProp, 'BtNPC', () => { if (this._quest)
            this.OnFindNpc?.(this._quest.Id); });
        if (this._bg)
            this._bgLayer.addChild(this._bg.ToPixi());
        if (this._bg2)
            this._bgLayer.addChild(this._bg2.ToPixi());
        if (this._bg3)
            this._bgLayer.addChild(this._bg3.ToPixi());
        if (this._summaryBg)
            this._bodyLayer.addChild(this._summaryBg.ToPixi());
    }
    _buttonFrom(loader, root, name, onClick) {
    ...
    get selectedId() { return this._quest?.Id ?? 0; }
    SetQuest(data, state) {
        this._quest = data;
        this._state = state;
        this._scroll = 0;
    ...
    update(dt) {
        ...
        // Position buttons
        if (this._btClose)
            this._btClose.container.position.set(p.x + PanelW0 - 18, p.y + 6);
        if (this._btAccept)
            this._btAccept.container.position.set(p.x + HeaderX + 6, p.y + HeaderY + 60);
        if (this._btResign)
            this._btResign.container.position.set(p.x + HeaderX + 6, p.y + HeaderY + 60);
        if (this._btFindNpc)
            this._btFindNpc.container.position.set(p.x + PanelW0 - 92, p.y + PanelH0 - 30);
        // Draw header
        ...
        if (mx !== undefined && my !== undefined) {
            if (mx >= p.x + BodyX && mx < p.x + BodyX + BodyW && my >= p.y + BodyY && my < p.y + BodyY + BodyH) {
                if (typeof window.__wheelDelta === 'number') {
                    const d = window.__wheelDelta;
    ...
    _drawHeader(p) {
        ...
        let levelStr = '';
        if (min > 0 && max > 0)
            levelStr = `Over Level ${min}  Under Level ${max}`;
        else if (min > 0)
            levelStr = `Over Level ${min}`;
        else if (max > 0)
            levelStr = `Under Level ${max}`;
        if (levelStr) {
    ...
    _drawBody(p) {
        ...
        for (let i = 0; i < this._visibleLines; i++) {
            const idx = start + i;
            if (idx >= this._wrappedSummary.length)
                break;
            const t = new Text({
    ...
    handleMouseButton(x, y, down) {
        if (!this.isVisible)
            return false;
        const p = { x: this.AnchorTopLeft.x, y: this.AnchorTopLeft.y };
        if (this._btClose?.handleMouseButton(x, y, down) === true)
            return true;
        if (this._state === 0 && this._btAccept?.handleMouseButton(x, y, down) === true)
            return true;
        if (this._state === 1 && this._btResign?.handleMouseButton(x, y, down) === true)
            return true;
        if (this._btFindNpc?.handleMouseButton(x, y, down) === true)
            return true;
        if (!down)
    ...
    _panelRect(p) {
        return {
            ...
            contains(cx, cy) { return cx >= this.x && cx < this.x + this.width && cy >= this.y && cy < this.y + this.height; },
        };
    }
    onKeyPress(key) {
        if (!this.isVisible)
    ...
    _canvas(loader, root, name) {
        const v = root?.Get(name);
        return v instanceof WzCanvas ? loader.Load(v) : null;
    }
}
//# sourceMappingURL=QuestDetail.js.map