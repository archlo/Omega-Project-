...
export class CharInfo extends GamePanel {
    ...
    _btException = null;
    _btFamily = null;
    constructor(loader, ui, charWz, itemWz, baseWz, itemIcons) {
        super();
        this._loader = loader;
        this._ui = ui;
        ...
        this._charInfoProp = ui?.GetItem('UIWindow2.img/UserInfo/character');
        // Background
    ...
    _loadButtons(loader) {
        ...
        if (this._btParty) {
            this._btParty.onClick = () => this.onParty?.();
            this._contentLayer.addChild(this._btParty.container);
        }
        if (this._btTrade) {
            this._btTrade.onClick = () => this.onTrade?.();
            this._contentLayer.addChild(this._btTrade.container);
        }
        if (this._btFameUp) {
            this._btFameUp.onClick = () => this.onFameUp?.();
            this._contentLayer.addChild(this._btFameUp.container);
        }
        if (this._btFameDown) {
            this._btFameDown.onClick = () => this.onFameDown?.();
            this._contentLayer.addChild(this._btFameDown.container);
        }
        if (this._btPet) {
            this._btPet.onClick = () => this.setState(this._state === 1 ? 0 : 1);
            this._contentLayer.addChild(this._btPet.container);
        }
        if (this._btRide) {
            this._btRide.onClick = () => this.setState(this._state === 2 ? 0 : 2);
            this._contentLayer.addChild(this._btRide.container);
        }
        if (this._btCollect) {
            this._btCollect.onClick = () => this.setState(this._state === 3 ? 0 : 3);
            this._contentLayer.addChild(this._btCollect.container);
        }
        if (this._btException) {
            this._btException.onClick = () => this.toggleExceptionList();
            this._contentLayer.addChild(this._btException.container);
        }
        if (this._btFamily) {
            this._contentLayer.addChild(this._btFamily.container);
        }
        // OG OnButtonClicked: 0x7D0 → ToggleAddOn(1) — item info, 0x7D1 → ToggleAddOn(2) — wish list
        if (btItem) {
            btItem.onClick = () => this.toggleDetail();
            this._contentLayer.addChild(btItem.container);
        }
        if (btWish) {
            btWish.onClick = () => this.toggleWishList();
            this._contentLayer.addChild(btWish.container);
        }
    }
    ...
    toggleDetail() {
        ...
        if (this._detailVisible) {
            ...
            if (this._wishPanel)
                this._wishPanel.container.visible = false;
        ...
        if (this._detailPanel)
            this._detailPanel.container.visible = this._detailVisible;
    }
    toggleWishList() {
        ...
        if (this._wishVisible) {
            ...
            if (this._detailPanel)
                this._detailPanel.container.visible = false;
            if (!this._wishPanel) {
        ...
        if (this._wishPanel)
            this._wishPanel.container.visible = this._wishVisible;
    }
    ...
    toggleExceptionList() {
        ...
        if (this._exceptionPanel)
            this._exceptionPanel.container.visible = this._exceptionVisible;
    }
    // OG: SetChairItemInfo / SetWishItemInfo — populate sub-panel data
    setChairItems(itemIds) {
        this.chairItems = itemIds;
        this._syncDetailPanel();
    }
    setWishItems(itemIds) {
        this.wishItems = itemIds;
        this._syncDetailPanel();
        if (this._wishPanel)
            this._wishPanel.setItems(itemIds.map(id => ({ itemId: id, name: this.itemNameOf?.(id) ?? `Item ${id}`, count: 1, isCash: Math.floor(id / 100000) === 91 })));
    }
    _syncDetailPanel() {
        if (this._detailPanel) {
    ...
    setPetItemList(petIndex, itemIds) {
    ...
    _loadBackground(state) {
        // OG SetLayer: loads base bg then cover per state
        if (this._bgWz) {
            this._bgWz.ToPixi().parent?.removeChild(this._bgWz.ToPixi());
        }
        if (this._coverWz) {
            this._coverWz.ToPixi().parent?.removeChild(this._coverWz.ToPixi());
        }
    ...
    _rebuildState() {
        ...
        if (state === 1)
            ...
            this._buildTamingState();
        else if (state === 3)
            this._buildMedalState();
    ...
    _buildPetState() {
        ...
        if (pet) {
            ...
            tplText.x = PET_STAT_LEFT_X;
            tplText.y = PET_TEMPLATE_Y;
            ...
            lvlText.x = PET_STAT_LEFT_X;
            lvlText.y = PET_LEVEL_Y;
            ...
            fullText.x = PET_STAT_RIGHT_X;
            fullText.y = PET_FULL_Y;
            ...
            intimText.x = PET_STAT_RIGHT_X;
            intimText.y = PET_INTIMACY_Y;
    ...
    _loadBossPetCrown() {
        if (this._bossPetCrownLoaded || !this._charInfoProp)
            return;
        ...
        const charWzProp = this._charWz?.GetItem('Character.wz');
        if (charWzProp) {
    ...
    _buildTamingState() {
        ...
        lvlText.x = TAMING_STAT_LEFT_X;
        lvlText.y = TAMING_LEVEL_Y;
        this._stateLayer.addChild(lvlText);
        ...
        expText.x = TAMING_STAT_LEFT_X;
        expText.y = TAMING_EXP_Y;
        ...
        fatText.x = TAMING_STAT_RIGHT_X;
        fatText.y = TAMING_FATIGUE_Y;
    ...
    _buildMedalState() {
        ...
        nameText.x = MEDAL_NAME_X;
        nameText.y = MEDAL_NAME_Y;
        this._stateLayer.addChild(nameText);
        ...
        countText.x = MEDAL_COUNT_X;
        countText.y = MEDAL_COUNT_Y;
    ...
    update(dt) {
        ...
        if (this._btPet)
            this._btPet.enabled = this.isLocalChar && this.bPetActivated;
        // BtRide: enabled = m_bTamingMob
        if (this._btRide)
            this._btRide.enabled = this.hasTamingMob;
        // BtCollect: enabled = m_pMedalInfo != null
        if (this._btCollect)
            this._btCollect.enabled = this.medal !== null;
        // BtException: enabled = local char && m_bPetActivated && state==1
        if (this._btException)
            this._btException.enabled = this.isLocalChar && this.bPetActivated && this._state === 1;
        // BtPopUp/BtPopDown: enabled if not local char, OR if local char level >= 15
        const canFame = !this.isLocalChar || this.level >= 15;
        if (this._btFameUp)
            this._btFameUp.enabled = canFame;
        if (this._btFameDown)
            this._btFameDown.enabled = canFame;
        // BtParty/BtTrad: disabled for local char
        if (this._btParty)
            this._btParty.enabled = !this.isLocalChar;
        if (this._btTrade)
            this._btTrade.enabled = !this.isLocalChar;
    }
    ...
    _showItemTooltip(lx, ly, text) {
        ...
        const tipText = this._tooltipContainer.children[1];
        if (tipText)
            tipText.text = text;
