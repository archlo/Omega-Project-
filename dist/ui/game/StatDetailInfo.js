...
import { computeDerived, defaultStatInputs } from './StatDerived.js';
...
const ROW_Y = [15, 33, 51, 69, 87, 105, 123, 141, 159, 177, 195];
...
export class StatDetailInfo extends GamePanel {
    ...
    constructor(loader, ui, font) {
        ...
        this._bg = detailProp?.Get('backgrnd') instanceof WzCanvas ? loader.Load(detailProp.Get('backgrnd')) : null;
        this._bg2 = detailProp?.Get('backgrnd2') instanceof WzCanvas ? loader.Load(detailProp.Get('backgrnd2')) : null;
        this._bg3 = detailProp?.Get('backgrnd3') instanceof WzCanvas ? loader.Load(detailProp.Get('backgrnd3')) : null;
    ...
    draw() {
        ...
        if (this._bg2Sprite)
            this._bg2Sprite.visible = true;
        if (this._bg3Sprite)
            this._bg3Sprite.visible = true;
        ...
        this._statTexts = [];
        const makeText = (rowIdx, val, color) => {
        ...
        const fmtBuff = (base, buff) => {
            const total = base + buff;
            if (buff === 0)
                return { text: `${total}`, color: COLOR_NORMAL };
    ...
    handleMouseButton(x, y, down) {
        if (!this.isVisible)
            return false;
        if (this._btHpUp?.handleMouseButton(x, y, down) === true)
            return true;
    ...
    _getComboAbilityCritical(jobId) {
...
export function getIdealStatUp(jobId, level, str, dex, intStat, luk, bWantToBeInfighter) {
    const result = [];
...
//# sourceMappingURL=StatDetailInfo.js.map