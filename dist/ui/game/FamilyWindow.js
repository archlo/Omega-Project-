...
export class FamilyWindow extends GamePanel {
    ...
    _textLayer;
    _privButtons = [];
    get _panelW() { return this._bg?.Width ?? 214; }
    get _panelH() { return this._bg?.Height ?? 343; }
    constructor(loader, ui, font) {
        ...
        this._bg = famProp?.Get('backgrnd') instanceof WzCanvas ? loader.Load(famProp.Get('backgrnd')) : null;
        this._bg2 = famProp?.Get('backgrnd2') instanceof WzCanvas ? loader.Load(famProp.Get('backgrnd2')) : null;
        ...
        if (this._bg)
            this._bgPixi.addChild(this._bg.ToPixi());
        if (this._bg2)
            this._bgPixi.addChild(this._bg2.ToPixi());
    ...
    update(_dt) {
        ...
        if (this._btClose !== null)
            this._btClose.container.position.set(this._panelW - 18, 4);
        // Text content — all positions are local to _root (0-based)
        for (const child of this._textLayer.removeChildren())
            child.destroy();
    ...
    _drawPrivilege(x, y, style) {
        for (const b of this._privButtons)
            this._root.removeChild(b);
        this._privButtons = [];
        if (this._privileges.length === 0)
            return;
    ...
    handleMouseButton(x, y, down) {
        ...
        if (this._btClose?.handleMouseButton(lx, ly, down) === true)
            return true;
...
//# sourceMappingURL=FamilyWindow.js.map