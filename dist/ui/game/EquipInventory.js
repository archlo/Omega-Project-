...
export class EquipInventory extends GamePanel {
    ...
    _jobId = 0;
    _subJob = 0;
    _hasNoviceSkill1004 = true;
    _level = 0;
    _releaseEffectNode = null;
    _loader = null;
    _slotIcons = [];
    _gradeFrame = null;
    _buttons = [];
    ...
    _arrangeState = 0; // 0=gather, 1=sort
    _effectLayer = new Container();
    _releaseEffects = [];
    _font;
    _icons;
    _tooltip;
    ...
    _petSlideTargetX = 0;
    _petConsumeItemId = 0;
    _petConsumeMpItemId = 0;
    _petConsumeHpCount = 0;
    _petConsumeMpCount = 0;
    _btPetHide = null;
    _btPetSelect = [];
    _btPetPrev = null;
    _btPetNext = null;
    ...
    _mechanicBg3 = null;
    _mechanicSlotIcons = [];
    _mechanicShown = false;
    _mechanicSlideX = 0;
    _mechanicSlideTargetX = 0;
    ...
    constructor(opts = {}) {
        ...
        if (opts.loader) {
            ...
            const petProp = opts.uiWz?.GetItem('UIWindow2.img/Equip/pet') instanceof WzProperty
                ? opts.uiWz.GetItem('UIWindow2.img/Equip/pet') : null;
            if (petProp) {
    ...
    _drawNumberByImage(out, value, x, y) {
        // Remove old sprites from parent
        for (const s of out) {
            s.removeFromParent();
        }
        out.length = 0;
        if (value <= 0 || this._imgFontDigits.length === 0)
            return;
    ...
    _drawGradeFrame(g, grade, x, y, w, h) {
        let color;
        switch (grade) {
            case 1:
                color = 0x5CA1FF;
    ...
    _rebuildBackground() {
        ...
        this._bg.clear();
        const s1 = bg1.ToPixi();
        s1.label = 'equipInventoryWzBg';
        this._root.addChildAt(s1, 0);
        if (bg2) {
            const s2 = bg2.ToPixi();
            s2.label = 'equipInventoryWzBg2';
            this._root.addChildAt(s2, 1);
        }
        if (bg3) {
            const s3 = bg3.ToPixi();
            s3.label = 'equipInventoryWzBg3';
    ...
    equip(slotKey, itemName, itemId = 0, grade = 0) {
        this._equipped.set(slotKey, { itemId, name: itemName, grade, cash: Math.floor(itemId / 1_000_000) === 5 });
    }
    unequip(slotKey) { this._equipped.delete(slotKey); }
    setPetCount(count) {
        this._petCount = Math.max(1, Math.min(3, Math.trunc(count)));
        if (this._petIndex >= this._petCount)
            this._petIndex = this._petCount - 1;
    }
    setJobId(jobId, level, subJob = 0) {
        if (level !== undefined)
            this._level = level;
        if (subJob !== undefined)
            this._subJob = subJob;
    ...
    setHasNoviceSkill1004(has) { this._hasNoviceSkill1004 = has; }
    setEquipped(slotKey, itemId, name, grade = 0) {
        this._equipped.set(slotKey, { itemId, name, grade, cash: Math.floor(itemId / 1_000_000) === 5 });
    }
    findSlotByItemId(itemId) {
    ...
    bodyPartForSlot(slotKey) {
        for (const s of SLOTS)
            if (s.key === slotKey)
                return s.bodyPart;
        return -1;
    }
    setEquippedByBodyPart(bodyPart, itemId, name, grade = 0, stats) {
        if (stats)
            this._icons?.SetRuntimeEquip(itemId, stats);
        for (const s of SLOTS) {
            if (s.bodyPart === bodyPart) {
                this._equipped.set(s.key, { itemId, name, grade, cash: Math.floor(itemId / 1_000_000) === 5, stats });
                return;
            }
        }
    ...
    unequipByBodyPart(bodyPart) {
        for (const s of SLOTS) {
            if (s.bodyPart === bodyPart) {
                this._equipped.delete(s.key);
                return;
            }
        }
        for (const s of PET_SLOTS) {
    ...
    *equippedSlots() {
        for (const s of SLOTS) {
            ...
            if (v && v.itemId)
                yield { slotKey: s.key, itemId: v.itemId, bodyPart: s.bodyPart };
        }
        for (const s of PET_SLOTS) {
            const v = this._equipped.get(s.key);
            if (v && v.itemId)
    ...
    SetPlayerStats(level, str, dex, intt, luk, jobId) {
    ...
    setPetConsumeItem(itemId) { this._petConsumeItemId = itemId; }
    setPetConsumeMpItem(itemId) { this._petConsumeMpItemId = itemId; }
    setPetConsumeHpCount(count) { this._petConsumeHpCount = count; }
    setPetConsumeMpCount(count) { this._petConsumeMpCount = count; }
    handleMouseButton(x, y, down) {
        ...
        if (this._petPanel && this._petShown) {
            ...
            for (const btn of this._btPetSelect) {
                if (btn.handleMouseButton(plx, ply, down))
                    return true;
            }
            if (this._btPetHide?.handleMouseButton(plx, ply, down))
                return true;
            if (this._btPetPrev?.handleMouseButton(plx, ply, down))
                return true;
            if (this._btPetNext?.handleMouseButton(plx, ply, down))
                return true;
        }
            ...
            for (let i = 0; i < DRAGON_SLOTS.length; i++) {
                ...
                if (dlx >= s.ox && dlx < s.ox + SLOT_SIZE && dly >= s.oy && dly < s.oy + SLOT_SIZE) {
                    ...
                    if (equipped) {
                        ...
                        if (icon) {
                            this.onDragStart?.({ itemId: equipped.itemId, slotPos: -s.bodyPart, invType: 1 /* InventoryType.Equip */ }, icon.Texture, x, y);
                        }
                        else {
    ...
    tryAcceptDrag(payload, x, y) {
        ...
        if (!payload || typeof payload !== 'object' || !('invType' in payload))
            return false;
        const p = payload;
        if (p.invType !== 1 /* InventoryType.Equip */ && p.invType !== 5 /* InventoryType.Cash */)
            return false;
        const lx = x - this._root.x;
        const ly = y - this._root.y;
    ...
    _equippedSetCount(itemId) {
        ...
        for (const { itemId: equippedId } of this._equipped.values()) {
            if (this._icons.LoadAttr(equippedId)?.SetItemId === setItemId)
                count++;
        }
        return count;
    }
    ...
    _layoutButtons() {
        if (this._btDragon) {
            ...
            if (show && this._level < 50)
                this._btDragon.enabled = false;
        }
        if (this._btMechanic) {
            ...
            if (show && this._level < 50)
                this._btMechanic.enabled = false;
        }
    }
    _updateEffects(dtMs) {
    ...
    _slotVisible(slot) {
        ...
        if ((slot.bodyPart === 18 || slot.bodyPart === 19 || slot.bodyPart === 20 || slot.bodyPart === 59) && !this._expanded)
            return false;
        return true;
    }
    ...
    _isSlotDynamicallyDisabled(bodyPart) {
    ...
    _createPetPanel() {
        ...
        this._petHpIcon.x = PET_CONSUME_HP_X;
        this._petHpIcon.y = PET_CONSUME_HP_Y;
        panel.addChild(this._petHpIcon);
        this._petMpIcon = new Sprite(Texture.EMPTY);
        this._petMpIcon.x = PET_CONSUME_MP_X;
        this._petMpIcon.y = PET_CONSUME_MP_Y;
        ...
        if (!hasImgFont) {
            ...
            this._petHpCount.x = PET_CONSUME_HP_X;
            this._petHpCount.y = PET_CONSUME_HP_Y - 12;
            panel.addChild(this._petHpCount);
            this._petMpCount = new Text({ text: '', style: countStyle });
            this._petMpCount.x = PET_CONSUME_MP_X;
            this._petMpCount.y = PET_CONSUME_MP_Y - 12;
    ...
    _updatePetPanel(_dt) {
        ...
        if (this._petConsumeItemId && this._icons) {
            ...
            if (icon) {
                this._petHpIcon.texture = icon.Texture;
            }
        }
        if (this._petConsumeMpItemId && this._icons) {
            const icon = this._icons.LoadIcon(this._petConsumeMpItemId);
            if (icon) {
                this._petMpIcon.texture = icon.Texture;
            }
        }
        if (this._petHpCount)
            this._petHpCount.text = this._petConsumeHpCount > 0 ? `${this._petConsumeHpCount}` : '';
        if (this._petMpCount)
            this._petMpCount.text = this._petConsumeMpCount > 0 ? `${this._petConsumeMpCount}` : '';
        ...
        if (this._imgFontDigits.length > 0 && this._petPanel) {
            ...
            for (const s of this._imgFontHpDigits) {
                if (!s.parent)
                    this._petPanel.addChild(s);
            }
            for (const s of this._imgFontMpDigits) {
                if (!s.parent)
                    this._petPanel.addChild(s);
            }
        }
...
//# sourceMappingURL=EquipInventory.js.map