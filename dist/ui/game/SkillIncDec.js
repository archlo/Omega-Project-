...
export class SkillIncPanel extends GamePanel {
    ...
    _onSkillUp = null;
    _textureLoader = null;
    _skill0Tex = null; // normal row bg
    _skill1Tex = null; // enabled row bg
    _lineTex = null; // separator
    constructor(loader, ui) {
        ...
        if (loader && ui) {
            ...
            if (s0 instanceof WzCanvas)
                this._skill0Tex = loader.Load(s0)?.Texture ?? null;
            const s1 = ui.GetItem('UI/UIWindow2.img/Reset/SP/skill1');
            if (s1 instanceof WzCanvas)
                this._skill1Tex = loader.Load(s1)?.Texture ?? null;
            const ln = ui.GetItem('UI/UIWindow2.img/Reset/SP/line');
            if (ln instanceof WzCanvas)
                this._lineTex = loader.Load(ln)?.Texture ?? null;
        }
        // OG: Scrollbar at (1, 8, 150, 93, 155), wheelRange=156
        this._scrollBar = new ScrollBar(1, 8, 155, (pos) => {
    ...
    setOnSkillUp(cb) { this._onSkillUp = cb; }
    open(skills, x, y) {
        this._skills = skills;
...
export class SkillDecPanel extends GamePanel {
    ...
    constructor(loader, ui) {
        ...
        this._scrollBar = new ScrollBar(1, 8, 153, (pos) => {
            this._scrollOffset = pos;
        });
        this._root.addChild(this._scrollBar.container);
    ...
    setOnSkillDown(cb) { this._onSkillDown = cb; }
    open(skills, x, y) {
        this._skills = skills;
        this._scrollOffset = 0;
        this._scrollBar.pos = 0;
...
export class SkillChangeConfirm extends GamePanel {
    ...
    constructor(loader, ui) {
        ...
        if (loader && ui) {
            ...
            if (node instanceof WzCanvas) {
                const sprite = loader.Load(node)?.ToPixi();
                if (sprite)
                    this._btCancel.addChild(sprite);
            }
        }
    ...
    open(incSkill, decSkill, job, x, y) {
        ...
        if (this._ui && this._textureLoader) {
            ...
            if (incTabNode instanceof WzCanvas) {
                ...
                if (ws)
                    this._incTab.texture = ws.Texture;
            }
            ...
            if (decTabNode instanceof WzCanvas) {
                ...
                if (ws)
                    this._decTab.texture = ws.Texture;
            }
        }
        // Inc row
        ...
        if (incSkill.icon && this._textureLoader) {
            ...
            if (ws)
                this._incRow.icon.texture = ws.Texture;
        }
        // Dec row
        ...
        if (decSkill.icon && this._textureLoader) {
            ...
            if (ws)
                this._decRow.icon.texture = ws.Texture;
        }
        this._root.position.set(x, y);
        this.isVisible = true;
    }
...
//# sourceMappingURL=SkillIncDec.js.map