...
export class GameMenu extends Overlay {
    ...
    _mouseX = -1;
    _mouseY = -1;
    constructor(loader, ui, font = null) {
        super();
        this._font = font;
        this._hover = new Array(Nodes.length).fill(null);
        this._hoverSprites = new Array(Nodes.length).fill(null);
        const menu = ui?.GetItem('UIWindow.img/GameMenu');
        this._backgrnd = menu instanceof WzProperty && menu.Get('backgrnd') instanceof WzCanvas ? loader.Load(menu.Get('backgrnd')) : null;
        if (this._backgrnd) {
    ...
    _loadState(loader, root, state) {
        const s = root.Get(state);
        const c = s instanceof WzProperty ? s.Get('0') : null;
        return c instanceof WzCanvas ? loader.Load(c) : null;
    }
    get _panelWidth() { return this._backgrnd?.Width ?? PanelW; }
    get _panelHeight() { return this._backgrnd?.Height ?? PanelH; }
    get _topLeft() {
    ...
    _updateHover() {
        for (let i = 0; i < Nodes.length; i++) {
            ...
            if (sp)
                sp.visible = (this._selected === i) && this.isVisible;
        }
    }
    draw() {
        if (!this.isVisible)
