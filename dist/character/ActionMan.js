...
function isVehicle(itemId) {
    const cat = Math.floor(itemId / 10000);
    return cat === 190 || cat === 193;
}
function isEventVehicleType1(itemId) {
    return itemId >= 1932000 && itemId <= 1932006;
}
function isEventVehicleType2(itemId) {
    return itemId >= 1932010 && itemId <= 1932016;
}
function isWildHunterJaguarVehicle(itemId) {
    return itemId >= 1932020 && itemId <= 1932026;
}
function isAbleTamingMobAction(action, templateId) {
...
function isAbleTamingMobOneTimeAction(action, templateId) {
...
export class ActionMan {
    ...
    // ── Character caches ────────────────────────────────────────────────────
    _characterImgs = new Map();
    // ── Weapon afterimage ───────────────────────────────────────────────────
    _afterimages = new Map();
    ...
    _npcImgs = new Map();
    _petImgs = new Map();
    _employeeImgs = new Map();
    _summonedProps = new Map();
    _morphImgs = new Map();
    // ── Action frame caches ─────────────────────────────────────────────────
    _mobActions = new Map();
    _npcActions = new Map();
    _petActions = new Map();
    _employeeActions = new Map();
    _summonedActions = new Map();
    _dragonActions = new Map();
    _shadowPartnerActions = new Map();
    _morphActions = new Map();
    _tamingMobActions = new Map();
    // ── Face look cache ─────────────────────────────────────────────────────
    _faceLookEntries = new Map();
    // ── Cache sweep ─────────────────────────────────────────────────────────
    _lastSweep = 0;
    ...
    SetMobWz(wz) { this._mobWz = wz; }
    SetNpcWz(wz) { this._npcWz = wz; }
    SetCharacterWz(wz) { this._characterWz = wz; }
    SetSkillWz(wz) { this._skillWz = wz; }
    SetMapWz(wz) { this._mapWz = wz; }
    SetSummonWz(wz) { this._summonWz = wz; }
    // ═══════════════════════════════════════════════════════════════════════════
        ...
        for (let actionCode = 0; actionCode < 273; actionCode++) {
            ...
            if (isPieced) {
                ...
                for (let pi = 0; pi < count; pi++) {
                    ...
                    if (!(pieceProp instanceof WzProperty))
                        continue;
                    const piece = {
                ...
                for (let fi = 0; fi < count; fi++) {
                    ...
                    if (!(frameProp instanceof WzProperty))
                        continue;
                    const piece = {
                        nAction: 0,
                        nSubAction: fi,
    ...
    GetCharacterImgEntry(nUOLKey, pImg) {
        ...
        const infoNode = pImg.Get('info') instanceof WzProperty ? pImg.Get('info') : null;
    ...
    GetMobImgEntry(nTemplateID) {
        ...
        if (!root)
            return null;
        const entry = { pImg: root, tLastAccessed: Date.now() };
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    GetEmployeeImgEntry(nTemplateID) {
        if (nTemplateID <= 0)
            return null;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadMobAction(nTemplateID, nAction, outFrames) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadNpcAction(nTemplateID, nAction, outFrames) {
        const key = nAction | (nTemplateID << 8);
        ...
        const linkNode = root.Get('info') instanceof WzProperty ? root.Get('info').Get('link') : null;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadPetAction(nTemplateID, nAction, outFrames) {
        const key = nAction | (nTemplateID << 8);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadEmployeeAction(nTemplateID, nAction, outFrames) {
        const key = nAction | (nTemplateID << 8);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadSummonedAction(nSkillID, nAction, outFrames) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadShadowPartnerAction(nSkillID, nAction, outFrames) {
        const key = nAction | (nSkillID << 8);
    ...
    GetShadowPartnerProp(nSkillID) {
        ...
        if (skillNode instanceof WzProperty) {
            ...
            if (specificSkill instanceof WzProperty) {
                ...
                if (shadowNode instanceof WzProperty)
                    return shadowNode;
            }
        }
        return null;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadDragonAction(nJob, nAction, outFrames) {
        ...
        if (!(jobNode instanceof WzProperty))
            return;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadTamingMobAction(nVehicleID, nAction, aAvatarHairEquip, bTamingMobTired, outFrames) {
        if (!isVehicle(nVehicleID))
            return;
        ...
        else if (nAction === 0x10E) {
            resolvedAction = 43;
            bModified = true;
        }
        else if (nAction === 0x10F || nAction === 0x110) {
            resolvedAction = 2;
            bModified = true;
        }
    ...
    _loadTamingMobActionInternal(nAction, nVehicleID, aAvatarHairEquip, outFrames) {
        const vehicleCat = Math.floor(nVehicleID / 10000);
        if (vehicleCat === 190) {
            ...
            if (bodyIds[0]) {
                ...
                if (bodyIds[1])
                    this._loadSingleTamingMobPart(nAction, nVehicleID, bodyIds[1], outFrames);
                if (bodyIds[2])
                    this._loadSingleTamingMobPart(nAction, nVehicleID, bodyIds[2], outFrames);
            }
        }
        else if (nVehicleID) {
    ...
    _loadSingleTamingMobPart(nAction, nVehicleID, nBodyID, outFrames) {
        ...
        if (!isVehicle(nVehicleID) && nBodyID !== nVehicleID) {
            ...
            if (bodyProp instanceof WzProperty)
                root = bodyProp;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadMorphAction(nMorphID, nAction, outFrames) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    LoadFaceLook(nFaceID, nEmotion, nAcc) {
        ...
        if (!(emotionProp instanceof WzProperty))
            return null;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    GetMeleeAttackRange(sAfterimageUOL, nAction) {
        if (nAction === 74) {
    ...
    GetWeaponAfterImage(sUOL, effectWz) {
        ...
        if (!(weaponNode instanceof WzProperty))
            return null;
        const ranges = [];
        ...
        const afterimage = { uol: sUOL, ranges, frames };
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    ProcessEquipmentForAction(nAction, nGender, aAvatarHairEquip, nWeaponStickerID, nVehicleID) {
    ...
    SweepCache() {
        const now = Date.now();
        if (now - this._lastSweep < ActionMan.SWEEP_INTERVAL_MS)
            return;
        this._lastSweep = now;
    ...
    _readMoveProp(prop) {
        ...
        if (val instanceof WzVector)
            return { x: val.X, y: val.Y };
        return { x: 0, y: 0 };
    }
    _readBodyRectProp(prop) {
        if (!prop)
    ...
    _readMove(node) {
        ...
        if (moveNode instanceof WzVector)
            return { x: moveNode.X, y: moveNode.Y };
        return { x: 0, y: 0 };
    }
    _readBodyRect(node) {
    ...
    _readCanvasInt(canvas, key, defaultVal) {
        const prop = canvas.Property;
        if (!prop)
            return defaultVal;
        return this._readInt(prop, key, defaultVal);
    }
    _readActionCode(prop, key) {
    ...
    _detectPieced(actionProp, firstChild) {
        ...
        if (!(firstChild instanceof WzProperty))
            return false;
    ...
    _sweepEntityMap(map, stale) {
        for (const [key, entry] of map) {
            if (entry.tLastAccessed < stale)
                map.delete(key);
        }
    }
    _sweepActionMap(map, stale) {
        for (const [key, entry] of map) {
            if (entry.tLastAccessed < stale)
                map.delete(key);
        }
    }
    _sweepShadowPartnerMap(map, stale) {
        for (const [key, entry] of map) {
            if (entry.tLastAccessed < stale)
                map.delete(key);
        }
