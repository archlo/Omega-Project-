...
import { isBlockedArea, segmentsIntersect } from '../map/Foothold.js';
export class MobController {
    static BaseWalkSpeed = 60;
    static BaseFlySpeed = 90;
    static MinSpeed = 20;
    static IdlePauseMin = 1.5;
    static IdlePauseMax = 4.0;
    static WalkBurstMin = 1.0;
    static WalkBurstMax = 3.0;
    // CVecCtrl transitions when RelPos reaches 0 or foothold.length; there is
    // no artificial stand-off from the endpoint.
    static EdgeMargin = 0;
    static AttackRangeX = 50;
    static AttackRangeY = 60;
    static AttackCooldown = 0.7;
    static AggroDurationSec = 8;
    static HitPlayerCooldownSec = 2;
    static FlushIntervalSec = 0.25;
    static HitKnockbackSec = 0.2;
    static _fallGravity = 1500;
    static _maxMobFall = 500;
    // OG CMob::AGGRO_RANGE — proximity aggro detection range
    static AggroRangeX = 200;
    static AggroRangeY = 150;
    // OG CUser::BodyAttack — collision damage when mob touches player
    static BodyAttackCooldown = 1.0;
    _mob;
    _field;
    _info;
    onAttackPlayer = null;
    ...
    _flyTarget = { x: 0, y: 0 };
    _flyTargetTimer = 0;
    _bodyAttackCooldown = 0;
    _climb = null;
    _escortDestination = null;
    _pending = [];
    ...
    _fhLeftEdge(fh) { return Math.min(fh.X1, fh.X2); }
    _fhRightEdge(fh) { return Math.max(fh.X1, fh.X2); }
    _getFootholdBelow(x, y) {
    ...
    constructor(mob, field, info) {
        ...
        if (this._info.FirstAttack)
            this._aggroTimer = Infinity;
        this._lastSyncPos = { ...this._mob.Position };
        this._lastSyncVel = { x: 0, y: 0 };
        this._enterIdle();
    }
    get ShouldTick() { return !this._info.IsStay && this._info.EscortType === 0; }
    get CurrentAction() {
        switch (this._state) {
            case 3 /* State.Attacking */: return 13 /* MobActionType.Attack1 */ + Math.min(this._selectedAttack, 8);
            case 2 /* State.Chasing */: return 39 /* MobActionType.Chase */;
            case 4 /* State.Climbing */: return 4 /* MobActionType.Rope */;
            case 1 /* State.Walking */: return this._info.IsFly ? 3 /* MobActionType.Fly */ : 0 /* MobActionType.Move */;
            default: return this._info.IsFly ? 3 /* MobActionType.Fly */ : 1 /* MobActionType.Stand */;
        }
    }
    get IsChasing() { return this._state === 2 /* State.Chasing */ || this._state === 3 /* State.Attacking */; }
    ...
    get IsAggressive() { return this._aggroTimer > 0; }
    OnDamagedByPlayer() {
        if (this._aggroTimer !== Infinity)
            this._aggroTimer = MobController.AggroDurationSec;
    }
    get CanHitPlayer() { return this._hitPlayerCooldown <= 0; }
    NotePlayerHit() { this._hitPlayerCooldown = MobController.HitPlayerCooldownSec; }
    ApplyHitKnockback(pushPx) {
    ...
    Update(dt, playerPos) {
        ...
        if (this._hitPlayerCooldown > 0)
            this._hitPlayerCooldown -= dt;
        if (this._bodyAttackCooldown > 0)
            this._bodyAttackCooldown -= dt;
        ...
        if (aggressive) {
            ...
            if (selectedAttack >= 0) {
                ...
                this._state = 3 /* State.Attacking */;
                this._selectedAttack = selectedAttack;
                this._velocity = { x: 0, y: 0 };
            }
            else {
                if (this._tryStartClimb(playerPos)) {
                    this._state = 4 /* State.Climbing */;
                }
                else {
                    this._state = 2 /* State.Chasing */;
                }
            }
            ...
            if (this._stateTimer <= 0) {
                if (this._state === 1 /* State.Walking */)
                    this._enterIdle();
                else
                    this._enterWalk();
            }
        }
        const moving = this._state === 1 /* State.Walking */ || this._state === 2 /* State.Chasing */ || this._state === 4 /* State.Climbing */;
        if (moving) {
            if (this._state === 4 /* State.Climbing */)
                this._stepClimb(dt, playerPos);
            else if (this._info.IsFly)
                this._stepFly(dt, playerPos);
            else
                this._stepWalk(dt);
        }
        else {
        ...
        if (!this._info.NoFlip)
            this._mob.SetFacing(this._facingLeft);
        this._mob.SetState(this._state === 4 /* State.Climbing */ ? (this._climb?.IsLadder ? 6 /* Ladder */ : 7 /* Rope */)
            : this._state === 1 /* State.Walking */ || this._state === 2 /* State.Chasing */ ? 1 /* Move */
                : this._state === 3 /* State.Attacking */ ? 2 /* Attack */
                    : 0 /* Stand */);
        this._flushTimer += dt;
    ...
    OnServerMove(path, moveAction, facingLeft) {
    ...
    _pickAttack(dx, dy) {
        ...
        for (const atk of this._info.Attacks.values()) {
            ...
            if (r) {
                ...
                if (dx < ltx || dx > rbx || dy < r.lty || dy > r.rby)
                    continue;
            }
    ...
    _tryStartClimb(playerPos) {
        ...
        if (this._info.MoveAbility !== 6 || Math.abs(playerPos.y - this._mob.Position.y) < 12)
            return false;
        const ladder = this._field.GetLadderOrRope(this._mob.Position.x, Math.min(this._mob.Position.y, playerPos.y), this._mob.Position.x, Math.max(this._mob.Position.y, playerPos.y));
        if (!ladder || this._mob.Position.x < ladder.X - 10 || this._mob.Position.x > ladder.X + 10)
            return false;
        this._climb = ladder;
        this._velocity = { x: 0, y: 0 };
        return true;
    }
    _stepClimb(dt, playerPos) {
        ...
        if (fh) {
            ...
            if (nextX < leftEdge + MobController.EdgeMargin) {
                ...
                if (prevFh && !prevFh.IsWall) {
                    ...
                    const canWalk = typeof this._field.CanWalkThrough === 'function'
                ...
                nextX = leftEdge + MobController.EdgeMargin;
                if (this._state !== 2 /* State.Chasing */)
                    this._facingLeft = false;
            }
            ...
            if (this._velocity.y > maxFall)
                this._velocity.y = maxFall;
            this._mob.Position = { x: nextX, y: this._mob.Position.y + this._velocity.y * dt };
        }
        this._seatOnFoothold();
    }
    _stepFly(dt, playerPos) {
        if (this._state === 2 /* State.Chasing */) {
            ...
            if (toT.x * toT.x + toT.y * toT.y < 64 || this._flyTargetTimer <= 0)
                this._pickFlyTarget();
        }
        const d = {
            x: this._flyTarget.x - this._mob.Position.x,
    ...
    _lastFhX2 = 0;
    _lastFhY2 = 0;
    _findLandingFoothold(x1, y1, x2, y2) {
        if (y2 < y1)
            return null;
        let landing = null;
        let bestT = Infinity;
        for (const fh of Object.values(this._field.Footholds)) {
            if (fh.State === 0 || fh.IsWall)
                continue;
            if (!segmentsIntersect(x1, y1, x2, y2, fh.X1, fh.Y1, fh.X2, fh.Y2))
                continue;
            const dx = x2 - x1;
            const dy = y2 - y1;
