...
export class ItemInfoService {
    ...
    _equipItems = new Map();
    _bundleItems = new Map();
    _setItemInfo = new Map();
    _coupleChairItems = new Map();
    _setItemEffects = [];
    // String cache (item name/desc resolved once per id)
    _nameCache = new Map();
    _descCache = new Map();
    constructor(_characterWz, _itemWz, _tamingMobWz = null, _morphWz = null, _etcWz = null, _skillWz = null) {
        this._characterWz = _characterWz;
        this._itemWz = _itemWz;
        this._tamingMobWz = _tamingMobWz;
        this._morphWz = _morphWz;
        this._etcWz = _etcWz;
        this._skillWz = _skillWz;
    ...
    GetItemInfo(itemId) {
        ...
        return info instanceof Object ? info : null;
    }
    /**
     ...
     */
    GetItemIconCanvas(itemId, variant = 'icon') {
        const info = this.GetItemInfo(itemId);
        if (!info && Math.floor(itemId / 10000) === 500) {
            return this.GetPetIconCanvas(itemId, variant === 'iconRaw' || variant === 'iconRawD', variant === 'iconD' || variant === 'iconRawD');
        }
        if (!info)
            return null;
    ...
    GetPetIconCanvas(templateId, raw = false, dead = false) {
        if (!this._itemWz || templateId <= 0)
            return null;
    ...
    GetRingIconCanvas(itemId, raw = false) {
        if (equipCategory(itemId) !== 'Ring')
            return null;
     ...
     */
    GetSkillIconCanvas(skillId, variant = 'icon') {
        if (!this._skillWz || skillId <= 0)
            return null;
        const key = variant === 'icon' ? 'icon'
    ...
    GetItemString(itemId, sKey) {
        ...
        if (node instanceof Object) {
            ...
            const sub = node.Get(String(itemId));
    ...
    GetEquipItem(itemId) {
        ...
        if (data)
            this._equipItems.set(itemId, data);
        return data;
    }
    _loadEquipItem(itemId) {
        const info = this.GetItemInfo(itemId);
        if (!info)
            return null;
        return {
    ...
    _loadMaxLevel(info) {
        ...
        if (!levelNode || typeof levelNode !== 'object')
            return 0;
        const levelObj = levelNode;
        let max = 0;
        for (let i = 1;; i++) {
            if (!(String(i) in levelObj))
                break;
            max = i;
        }
        return max;
    ...
    GetSetItemTooltip(itemId) {
        const setItemId = this.GetItemInfo(itemId) ? N(this.GetItemInfo(itemId), 'setItemID') : 0;
    ...
    RegisterSetItemInfo() {
        ...
        if (!(setNode instanceof WzProperty))
            return;
        const obj = setNode.Items;
        for (const key of Object.keys(obj)) {
            const setItemId = parseInt(key, 10);
            if (!isFinite(setItemId))
                continue;
            const setProp = obj[key];
            if (!setProp || typeof setProp !== 'object')
                continue;
            const p = setProp;
            const info = {
                setItemId,
            ...
            if (itemIdNode instanceof WzProperty) {
                for (const member of Object.keys(itemIdNode.Items)) {
                    ...
                    if (itemId)
                        info.items.push({ itemId, equippedCount: 0 });
                }
            }
    ...
    RegisterSetItemEffect() {
        ...
        for (const key of Object.keys(obj)) {
            ...
            for (let i = 0;; i++) {
                ...
                if (!effNode || typeof effNode !== 'object')
                    break;
                const ep = effNode;
                const effect = {
     ...
     */
    GetSetItemEffect(hairEquip, weaponStickerId, petIds) {
        let bestEffect = null;
        let bestSetItemId = -1;
     ...
     */
    RegisterCoupleChairItem(itemId, info) {
     ...
     */
    GetPortableChairRecoveryRate(itemId, bHP) {
        const info = this.GetItemInfo(itemId);
        if (!info)
            return 0;
     ...
     */
    IsTherePortableChairStatUp(itemId) {
        const info = this.GetItemInfo(itemId);
        if (!info)
            return false;
        // StringPool 0x928 = 'info' sub-check for stat bonuses
        return (N(info, 'incSTR') > 0 || N(info, 'incDEX') > 0 ||
     ...
     */
    GetAppliableKarmaType(itemId) {
        if (this.IsEquipItem(itemId)) {
            const equip = this.GetEquipItem(itemId);
            if (equip?.tradeBlock)
                return 3; // Untradeable
            if (equip?.cash)
                return 1; // Cash item
            return 0; // Normal
        }
        const bundle = this.GetBundleItem(itemId);
        if (bundle?.tradeBlock)
            return 3;
        if (bundle?.cash)
            return 1;
        return 0;
     ...
     */
    GetLevelInfo(itemId, level) {
        if (!this.IsGrowthItem(itemId))
            return null;
        const equip = this.GetEquipItem(itemId);
        if (!equip)
            return null;
        if (level <= 0 || level > equip.maxLevel)
            return null;
        ...
        if (!levelNode || typeof levelNode !== 'object')
            return null;
        const levelData = levelNode[String(level)];
     ...
     */
    GetLevelAbilityInfo(itemId, level, levelUpType) {
        ...
        if (!abilityNode || typeof abilityNode !== 'object')
            return null;
        const abilityData = abilityNode[String(levelUpType)];
        if (!abilityData || typeof abilityData !== 'object')
            return null;
        const ap = abilityData;
        return {
            levelUpType,
     ...
     */
    GetPetAbilityFlag(itemId) {
        const info = this.GetItemInfo(itemId);
        if (!info)
            return 0;
        const petNode = info.Get('pet');
        if (!petNode || typeof petNode !== 'object')
            return 0;
        const flag = petNode.Get('dwPetAbilityFlag');
     ...
     */
    GetMovementProfile(shoeItemId, vehicleItemId, vehicleEquipIds, morphId) {
        if (morphId > 0) {
            const morph = this._getTemplate(this._morphWz, morphId);
            if (morph) {
                const nSpeed = N(morph, 'nSpeed');
                const nJump = N(morph, 'nJump');
                return movementProfile(nSpeed > 0 ? clamp(nSpeed, 70, 140) : 100, nJump > 0 ? clamp(nJump, 80, 123) : 100, N(morph, 'dFs'), N(morph, 'nSwim'), 'morph');
            }
        }
        if (vehicleItemId > 0) {
    ...
    GetVehicleMovement(vehicleItemId, equipItemIds = []) {
        ...
        return movementProfile(clamp(speed, 70, 140), clamp(jump, 80, 123), N(template, 'dFs'), N(template, 'nSwim'), 'vehicle');
    }
    _getTemplate(wz, id) {
        if (!wz || id <= 0)
            return null;
    ...
    static equipCategory(itemId) {
        return equipCategory(itemId);
    }
}
export function movementProfile(speed, jump, dFs, nSwim, source) {
...
function canvasAt(parent, key) {
    if (!(parent instanceof WzProperty))
        return null;
...
//# sourceMappingURL=ItemInfoService.js.map