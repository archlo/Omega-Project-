...
export class MobInfoService {
    _mobWz;
    _cache = new Map();
    constructor(mobWz) { this._mobWz = mobWz; }
    Get(templateId) {
    ...
    _parse(templateId) {
        ...
        if (flySpeed === 0)
            flySpeed = MobInfoService._readInt(info, 'fs');
        ...
        if (speakRoot instanceof WzProperty) {
            for (const [, sv] of Object.entries(speakRoot.Items)) {
                ...
                if (lines.length > 0)
                    speakEntries.push({ action: sAction, lines });
            }
        }
        const m = new MobInfo();
    ...
    static _readBool(p, key) { return MobInfoService._readInt(p, key) !== 0; }
    static _readStr(p, key) {
        const v = p.Get(key);
    ...
    static _readHitRect(attackNode) {
        const frame0 = attackNode.Get('0');
        let prop = null;
        if (frame0 instanceof WzCanvas)
            prop = frame0.Property;
        else if (frame0 instanceof WzProperty)
            prop = frame0;
        if (!prop)
            return null;
...
//# sourceMappingURL=MobInfoService.js.map