...
export class SequencedKeyMan {
    ...
    observeAttackState(isDown, nowMs, ctx) {
    ...
    onAttackKey(down, nowMs, ctx) {
        if (down) {
            this._keyDownTick = nowMs;
            return;
        }
        if (this._keyDownTick < 0)
            return;
        ...
        if (!f && this._lastTier !== 'none')
            f = this._pick('none', dir, heldMs, ctx);
        if (!f) {
            this._lastTier = 'none';
            return;
        }
    ...
    _pick(tier, dir, heldMs, ctx) {
        for (const f of FINISHER_TABLE) {
            if (f.fromTier !== tier)
                continue;
            if (f.direction !== 'none' && f.direction !== dir)
                continue;
            if (heldMs > f.tapWindowMs)
                continue;
            if (f.jobEligible && !f.jobEligible(ctx.jobId))
                continue;
            if (ctx.getSkillLevel(f.skillId(ctx.jobId, ctx)) <= 0)
                continue;
            return f;
        }
        return null;
     ...
     */
    update(ctx) {
        if (!this._reserved || ctx.isAttacking())
            return null;
...
//# sourceMappingURL=SequencedKeyMan.js.map