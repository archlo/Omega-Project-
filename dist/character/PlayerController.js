import { Stance, StanceMoveAction } from './Stance.js';
import { DEFAULT_PHYSICS } from '../map/FieldScene.js';
import { isBlockedArea } from '../map/Foothold.js';
import { EncodeMovePath } from '../net/packet/MovePathEncoder.js';
export class PlayerController {
    ...
    static BaseWalkSpeed = 125; // WZ: Physics.img/walkSpeed
    static WalkForce = 140000; // WZ: Physics.img/walkForce
    static WalkDrag = 80000; // WZ: Physics.img/walkDrag
    static BaseJumpSpeed = 555; // WZ: Physics.img/jumpSpeed
    static Gravity = 2000; // WZ: Physics.img/gravityAcc
    static MaxFallSpeed = 670; // WZ: Physics.img/fallSpeed
    static FlyForce = 120000; // WZ: Physics.img/flyForce
    static FlySpeed = 200; // WZ: Physics.img/flySpeed
    static SwimForce = 120000; // WZ: Physics.img/swimForce
    static SwimSpeed = 140; // WZ: Physics.img/swimSpeed
    ...
    static FloatCoefficient = 0.01;
    static BodyHeight = 60;
    static ClimbSpeed = 120;
    static FlushSeconds = 0.10;
    static MaxElements = 12;
    static FallDamageThreshold = 500;
    static FallDamagePerPx = 0.08;
    ...
    // slipSpeed=100).
    static WalkSlant = 0.1;
    static SlipForce = 5000;
    static SlipSpeed = 100;
    ...
    _landingNext = 0;
    _fallZMass = 0;
    _fallStartFoothold = 0;
    _fallOffset = { x: 0, y: 0 };
    _trackedFhId = 0;
    _trackedFhOffset = { x: 0, y: 0 };
    ...
    _lastFootholdId = 0;
    _staggerTimer = 0;
    _shortDragTimer = 0; // OG: m_nSlideCount — temporary drag reduction after slide
    _shortDragForce = 0; // OG: m_dShortDrag — reduced drag during slide
    _climb = null;
    _climbGrabFrame = false; // true for one frame after grabbing ladder
    _animTimer = 0;
    _flushTimer = 0;
    _pending = [];
    _movePathAttribute = 0;
    _lastSyncPos = { x: 0, y: 0 };
    _lastSyncVel = { x: 0, y: 0 };
    _lastSyncStance = Stance.Stand1;
    _prevJump = false;
    _fallStartY = 0;
    _freeFallElapsedMs = 0;
    _walkSpeed = PlayerController.BaseWalkSpeed;
    _jumpSpeed = PlayerController.BaseJumpSpeed;
    _shoe = {
        mass: 1, walkAcc: 1, walkDrag: 1, walkSpeed: 1,
        walkSlant: PlayerController.WalkSlant, swimSpeedMultiplier: 1,
    };
    get _physics() {
        return this._field.Physics ?? DEFAULT_PHYSICS;
    }
    _distanceAlongFoothold(fh, x, y) {
        const px = x - fh.X1;
        const py = y - fh.Y1;
    ...
    _isSitting = false;
    get IsSitting() { return this._isSitting; }
    Sit(x, y) {
    ...
    onTakeFallDamage = null;
    onAttachedObjectChanged = null;
    ...
    get CurrentFoothold() { return this._currentFoothold; }
    set CurrentFoothold(v) { this._currentFoothold = v; }
    get IsStaggered() { return this._staggerTimer > 0; }
    get Climb() { return this._climb; }
    SetUserFlying(value) { this._userFlying = value; }
    SetOneTimeAction(action) { this._oneTimeAction = action; }
    SetMechanicMode(mode) { this._mechanicMode = mode; }
    SetMorphed(templateId) { this._morphTemplateId = templateId; }
    SetRidingVehicle(vehicleId) { this._ridingVehicle = vehicleId; }
    SetReservedLandingFoothold(id) { this._landingNext = id; }
    SetLadderRestrictions(restrictions) {
        if (restrictions.actionBlocked !== undefined)
            this._ladderActionBlocked = restrictions.actionBlocked;
        if (restrictions.vehicleActive !== undefined)
            this._vehicleActive = restrictions.vehicleActive;
        if (restrictions.oneTimeAction !== undefined)
            this._oneTimeAction = restrictions.oneTimeAction;
        if (restrictions.mechanicMode !== undefined)
            this._mechanicMode = restrictions.mechanicMode;
        if (restrictions.morphTemplateId !== undefined)
            this._morphTemplateId = restrictions.morphTemplateId;
        if (restrictions.ridingVehicle !== undefined)
            this._ridingVehicle = restrictions.ridingVehicle;
        if (restrictions.repeatSkillId !== undefined)
            this._repeatSkillId = restrictions.repeatSkillId;
        if (restrictions.userFlying !== undefined)
            this._userFlying = restrictions.userFlying;
    }
    /** CVecCtrlUser::IsAbleToClimbLadderOrRope restrictions. */
    CanClimbLadderOrRope() {
    ...
    get IsImmovable() {
        ...
        if (this._isStunned)
            return true;
        if (this._knockbackStun)
            return true;
        if (this._directionMode)
            return true;
        return false;
    }
    get IsPreparingSkill() { return this._isPreparingSkill; }
    get PreparingSkillId() { return this._preparingSkillId; }
    get RepeatSkillId() { return this._repeatSkillId; }
    get IsAttract() { return this._isAttract; }
    get DirectionMode() { return this._directionMode; }
    SetStunned(v) { this._isStunned = v; }
    SetKnockbackStun(v) { this._knockbackStun = v; }
    SetPreparingSkill(skillId) {
        this._isPreparingSkill = skillId !== 0;
        this._preparingSkillId = skillId;
    }
    SetRepeatSkill(skillId) { this._repeatSkillId = skillId; }
    SetAttract(v) { this._isAttract = v; }
    SetDirectionMode(v) { this._directionMode = v; }
    constructor(field) {
        this._field = field;
        this._walkSpeed = this._physics.walkSpeed;
        this._jumpSpeed = this._physics.jumpSpeed;
    }
    SetStats(speedPct, jumpPct) {
        this._walkSpeed = this._physics.walkSpeed * (1 + speedPct / 100);
        this._jumpSpeed = this._physics.jumpSpeed * (1 + jumpPct / 100);
    }
    SetShoePhysics(values) {
        this._shoe = {
            ...this._shoe,
            ...values,
    ...
    ApplyKnockback(vx, vy, staggerSec = 0.4) {
    ...
    _tickAnimAndFlush(dt, advanceFrame = true) {
        if (advanceFrame) {
            this._animTimer += dt;
    ...
    _walkOnFoothold(dx, dt, input) {
        ...
        for (let guard = 0; guard < 64; guard++) {
            if (this._footholdPos + remaining < 0 || this._footholdPos + remaining > fh.Length) {
                ...
                const edgeFraction = Math.max(0, Math.min(1, Math.abs(remaining) > 0 ? edgeDistance / Math.abs(remaining) : 1));
                ...
                if (nextId === 0) {
                    ...
                    this._notifyAttached();
                    if (remainingDt > 0 && dt <= 0.2)
                        this._fallFreely(input, remainingDt);
                    return;
                }
    ...
    _tryRescueIfBelowMap() {
        if (this._climb !== null || this._grounded)
            return;
        const b = this._field.Bounds;
        if (!b)
            return;
        if (this.Position.y < b.bottom - 1)
            return;
        if (this._field.GetFootholdBelow(this.Position.x, this.Position.y) !== null)
            return;
        if (this._field.Info.Cloud)
            return;
    ...
    _tryGrabLadder(input) {
        if (!input.Up && !input.Down)
            return false;
        if (!this.CanClimbLadderOrRope())
            return false;
        const lr = input.Up
        ...
        if (lr === null)
            return false;
        if (input.Up && !input.Down && this.Position.y <= lr.Top)
            return false;
        if (input.Down && !input.Up && (this.Position.y < lr.Top || this.Position.y > lr.Bottom))
            return false;
        this._climb = lr;
    ...
    MakeContinuousMovePath(_tElapse) {
        if (_tElapse > 0) {
    ...
    CollisionDetectFloat(newX, newY) {
        ...
        for (const fh of candidates) {
            ...
            // landing candidate.
            if (crossOld > 0 || crossNew < 0 || (crossOld === 0 && crossNew === 0))
                continue;
            ...
            if (Math.abs(crossFh1) <= endpointEpsilon) {
                ...
                if (!prev || !isBlockedArea(prev, fh, newX, newY))
                    continue;
            }
            else if (Math.abs(crossFh2) <= endpointEpsilon) {
                const next = this._field.GetFoothold(fh.Next);
                if (!next || !isBlockedArea(fh, next, newX, newY))
                    continue;
            }
            if (!fh.IsWall && this._velocity.y <= 0)
                continue;
        ...
        if (bestFh !== null) {
            ...
            if (bestFh.Id === this._landingNext)
                this._landingNext = 0;
            return {
                x: bestIx,
                y: bestIy,
    ...
    _hasChangedSinceSync() {
        if (this.Stance !== this._lastSyncStance)
            return true;
        ...
        if (dvx * dvx + dvy * dvy > 0.25)
            return true;
        return false;
    }
    _applyMovingFootholdOffset() {
        if (!this._grounded)
            return;
        const fh = this._field.GetFoothold(this._currentFoothold);
        if (!fh)
    ...
    TryFlushMovePath() {
        if (this._pending.length === 0)
            return null;
        const blob = EncodeMovePath(this._lastSyncPos.x, this._lastSyncPos.y, this._lastSyncVel.x, this._lastSyncVel.y, this._pending);
        this._pending = [];
        return blob;
    }
}
