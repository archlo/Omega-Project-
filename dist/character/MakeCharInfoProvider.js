...
export class MakeCharInfoProvider {
    ...
    constructor(etcWz) {
        try {
            ...
            console.warn('MakeCharInfo load failed', ex);
        }
    }
    ...
    _loadSection(root, section, male) {
        ...
        for (let cat = 0; cat < MakeCharInfoProvider.CatCount; cat++) {
            ...
            if (!(catNode instanceof Object))
                continue;
            const ids = [];
            let i = 0;
            while (true) {
                const raw = catNode.Get(i.toString());
            ...
            if (ids.length > 0)
                this._options.set(MakeCharInfoProvider._key(section, male, cat), ids);
        }
    }
    ...
    static _nameKey(section, male, type, id) {
    ...
    _loadNames(root, section, male) {
        const nameRoot = root.Get('Name');
        if (!(nameRoot instanceof Object))
            return;
        const prefix = section === 'Info' ? '' : section;
        const gender = male ? 'CharMale' : 'CharFemale';
        const node = nameRoot.Get(`${prefix}${gender}`);
        if (!(node instanceof Object))
            return;
        for (const [typeKey, typeVal] of Object.entries(node.Items)) {
            if (!(typeVal instanceof Object))
                continue;
            const type = parseInt(typeKey, 10);
            if (isNaN(type))
                continue;
            for (const [idKey, idVal] of Object.entries(typeVal.Items)) {
...
//# sourceMappingURL=MakeCharInfoProvider.js.map