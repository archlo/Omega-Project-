...
// input fields below.
export function defaultBasicStatInput() {
    return {
...
export function computeBasicStat(inp) {
    ...
    if (inp.rateStrPct !== 0)
        str += Math.floor(inp.rateStrPct * str / 100);
    if (inp.rateDexPct !== 0)
        dex += Math.floor(inp.rateDexPct * dex / 100);
    if (inp.rateIntPct !== 0)
        int += Math.floor(inp.rateIntPct * int / 100);
    if (inp.rateLukPct !== 0)
        luk += Math.floor(inp.rateLukPct * luk / 100);
    ...
    if (totalHpPct !== 0)
        maxHp += Math.floor(maxHp * totalHpPct / 100);
