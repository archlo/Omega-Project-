...
export class SkillInfo {
    ...
    Common = null;
    // --- Canvas icons ---
    Icon0 = null; // apCanvas[0]
    Icon1 = null; // apCanvas[1]
    Icon2 = null; // apCanvas[2]
    MpConAt(level) { return SkillInfo._at(this.MpCon, level); }
    CooltimeAt(level) { return SkillInfo._at(this.Cooltime, level); }
    BuffTimeAt(level) { return SkillInfo._at(this.BuffTime, level); }
    LevelDescriptionAt(level) {
...
export class SkillInfoService {
    ...
    _bookNames = new Map();
    _castCache = new Map();
    _mobSkillCache = new Map();
    _itemSkillCache = new Map();
    constructor(skillWz) {
    ...
    EnumerateSkillIds(root) {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(jobNode.Items)) {
                ...
                if (isNaN(id))
                    continue;
                if (value instanceof WzProperty && SkillInfoService._readInt(value.Get('invisible')) !== 0)
                    continue;
                ids.push(id);
            }
            ids.sort((a, b) => a - b);
    ...
    GetRecommendSkill(root, totalSp) {
        const wz = this._skillWz();
        if (!wz)
            return 0;
        try {
            const recNode = SkillInfoService._jobItem(wz, root, 'recommend');
            if (!(recNode instanceof WzProperty))
                return 0;
    ...
    GetCastInfo(skillId) {
        ...
        try {
            ...
            if (node) {
                ...
                info.Actions = SkillInfoService._readActions(node.Get('action'));
    ...
    GetMobSkill(mobSkillId) {
        const cached = this._mobSkillCache.get(mobSkillId);
        if (cached !== undefined)
            return cached;
        const wz = this._skillWz();
        if (!wz) {
            this._mobSkillCache.set(mobSkillId, null);
            return null;
        }
        let entry = null;
        try {
    ...
    GetItemSkill(itemSkillId) {
        const cached = this._itemSkillCache.get(itemSkillId);
        if (cached !== undefined)
            return cached;
        const wz = this._skillWz();
        if (!wz) {
            this._itemSkillCache.set(itemSkillId, null);
            return null;
        }
        let entry = null;
        try {
    ...
    _load(skillId) {
        ...
        for (const key of ['reqSkill', 'req']) {
            ...
            if (!(req instanceof WzProperty))
                continue;
            for (const [skillKey] of Object.entries(req.Items)) {
                const requiredId = parseInt(skillKey, 10);
                if (!Number.isFinite(requiredId))
                    continue;
                const requiredLevel = SkillInfoService._readInt(req.Get(skillKey));
                if (requiredId > 0 && requiredLevel > 0)
                    info.RequiredSkills.set(requiredId, requiredLevel);
            }
            if (info.RequiredSkills.size > 0)
                break;
        }
        ...
        if (hitNode instanceof WzProperty) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (hitItem)
                    info.HitUOLs.push(SkillInfoService._readStr(hitItem));
            }
        }
        // --- Canvas icons (apCanvas[0..2]) ---
        const icon0 = node.Get('icon');
        if (icon0 instanceof WzCanvas)
            info.Icon0 = icon0;
        const icon1 = node.Get('icon1');
        if (icon1 instanceof WzCanvas)
            info.Icon1 = icon1;
        const icon2 = node.Get('icon2');
        if (icon2 instanceof WzCanvas)
            info.Icon2 = icon2;
        ...
        if (info.PsdSkill) {
            ...
            if (psdNode instanceof WzProperty) {
                ...
                info.PrepareAction = SkillInfoService._actionCodeFromName(SkillInfoService._readStr(psdNode.Get('action')));
    ...
    // -------------------------------------------------------------------------
    _loadLevelDataCommon(commonNode) {
    ...
    _loadMobSkill(wz, mobSkillId) {
        ...
        for (const [levelKey] of Object.entries(mobSkillNode.Items)) {
            ...
            if (!(levelNode instanceof WzProperty))
                continue;
    ...
    // Helpers
    // -------------------------------------------------------------------------
    static _actionCodeFromName(name) {
    ...
    static _readActionCodes(actionNode) {
        const codes = [];
    ...
    static _d3(n) { return n.toString().padStart(3, '0'); }
    static _d7(n) { return n.toString().padStart(7, '0'); }
    static _jobItem(wz, job, tail) {
        return wz.GetItem(`${SkillInfoService._d3(job)}.img/${tail}`)
            ?? wz.GetItem(`${job}.img/${tail}`);
    }
    static _skillNode(wz, skillId) {
    ...
    static _countProps(node) {
        return Object.keys(node.Items).length;
    }
}
//# sourceMappingURL=SkillInfoService.js.map