...
export class SecondaryStat {
    ...
    getTempSpeed() { return this._buff.speed; }
    getTempJump() { return this._buff.jump; }
    getBasicStatUp() { return this._buff.basicStatUp; }
    ...
    getBuffPAD() { return this._buff.pad + this._buff.magicAtk; }
    /** Total magical ATK buff. */
    getBuffMAD() { return this._buff.mad; }
    /** Total physical DEF buff. */
    getBuffPDD() { return this._buff.pdd; }
    /** Total magical DEF buff. */
    getBuffMDD() { return this._buff.mdd; }
    /** Flat accuracy buff value. */
    getBuffACC() { return this._buff.acc; }
    /** Flat avoidability buff value. */
    getBuffEVA() { return this._buff.eva; }
    /** Magic Guard damage-to-MP conversion percentage (0-100). */
    getMagicGuardReduction() { return this._buff.magicGuard; }
    /** Power Guard damage-to-HP reflection percentage (0-100). */
    getPowerGuardReduction() { return this._buff.powerGuard; }
    /** Meso Guard damage-to-meso absorption percentage (0-100). */
    getMesoGuardReduction() { return this._buff.mesoGuard; }
    /** Holy Symbol EXP rate bonus (0-100%). */
    getHolySymbolExpRate() { return this._buff.holySymbol; }
    /** Sharp Eyes critical rate bonus (0-100%). */
    getSharpEyesCritRate() { return this._buff.sharpEyes; }
    /** Stance dodge probability (0-100%). */
    getStanceRate() { return this._buff.stance; }
    /** DarkSight active (non-zero = active). */
    isDarkSightActive() { return this._buff.darkSight !== 0; }
    /** Booster active (non-zero = active). */
    isBoosterActive() { return this._buff.booster !== 0; }
    /** ShadowPartner active (non-zero = active). */
    isShadowPartnerActive() { return this._buff.shadowPartner !== 0; }
    /** HyperBody active (non-zero = active). */
    isHyperBodyActive() { return this._buff.hyperBody !== 0; }
    /** Stun active (non-zero = active). */
    isStunActive() { return this._buff.stun !== 0; }
    /** Poison active (non-zero = active). */
    isPoisonActive() { return this._buff.poison !== 0; }
    /** Seal active (non-zero = active). */
    isSealActive() { return this._buff.seal !== 0; }
    /** Combo counter from aran combo skill (non-zero = active). */
    getAranCombo() { return this._buff.aranCombo; }
    /** Combo drain percentage. */
    getComboDrainRate() { return this._buff.comboDrain; }
    /** ShadowPartner damage percentage bonus (0-100%). */
    getShadowPartnerDamageRate() { return this._buff.shadowPartner; }
    /** Booster attack speed bonus (negative = faster). */
    getBoosterSpeedBonus() { return this._buff.booster; }
    /** HyperBody HP/MP multiplier percentage. */
    getHyperBodyHpMultiplier() { return this._buff.hyperBodyHp; }
    getHyperBodyMpMultiplier() { return this._buff.hyperBodyMp; }
    /**
    ...
    decode(p) {
        ...
        for (const entry of this._rawEntries) {
            ...
            if (fieldName && fieldName in this._buff) {
                this._buff[fieldName] = entry.value;
            }
...
//# sourceMappingURL=SecondaryStat.js.map