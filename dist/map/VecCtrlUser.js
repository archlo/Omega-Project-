...
export class VecCtrlUser extends VecCtrl {
    ...
    _fhOffset = { id: 0, x: 0, y: 0 };
    static WalkSpeed = 125;
    static WalkForce = 140000;
    static WalkDrag = 80000;
    static JumpSpeed = 555;
    static Gravity = 2000;
    static MaxFallSpeed = 670;
    WorkUpdateActiveLadderOrRope(dt) {
    ...
    IsUp = false;
    IsDown = false;
    SetSwimming(value, speedMultiplier = 1) {
    ...
    static _swimAxis(v, input, force, max, dt) {
    ...
    static _swimVertical(v, input, force, max, dt) {
        v = VecCtrlUser._dec(v, 100000, max, dt);
        const g = force / max;
        if (input === 0)
            return Math.min(max, v + g * dt);
        const floor = input < 0 ? max * 0.3 : max * 1.5;
        return input < 0 ? Math.max(floor, v - g * dt) : Math.min(floor, v + g * 0.5 * dt);
    }
    static _dec(v, force, max, dt) {
        if (v > max)
            return Math.max(max, v - force * dt);
        if (v < -max)
            return Math.min(-max, v + force * dt);
        return v;
    }
}
//# sourceMappingURL=VecCtrlUser.js.map