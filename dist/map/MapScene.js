...
export class MapScene {
    ...
    _screenW = 800;
    _screenH = 600;
    constructor(_mapPkg, _loader) {
        this._mapPkg = _mapPkg;
        this._loader = _loader;
    }
    Load(mapRoot) {
        const info = mapRoot.Get('info');
        if (info instanceof WzProperty) {
            this.BgmPath = info.Get('bgm') ?? null;
        }
        ...
        for (let layerIdx = 0; layerIdx < 8; layerIdx++) {
            ...
            if (!(layer instanceof WzProperty))
                continue;
            const objRoot = layer.Get('obj');
            if (!(objRoot instanceof WzProperty))
                continue;
     ...
     */
    SetCamera(camera, screenW = 800, screenH = 600) {
        this.Camera = camera;
    ...
    WorldToScreen(mapX, mapY, screenW, screenH) {
        return {
    ...
    _rebuildDisplay() {
        ...
        for (let i = 0; i < this._backgrounds.length; i++) {
            ...
            this._drawBackEntry(info, sprite, anim, screenCenter, this._screenW, this._screenH, this._bgScrollX[i] ?? 0, this._bgScrollY[i] ?? 0);
        }
        ...
        for (let i = 0; i < this._foregrounds.length; i++) {
            ...
            this._drawBackEntry(info, sprite, anim, screenCenter, this._screenW, this._screenH, this._fgScrollX[i] ?? 0, this._fgScrollY[i] ?? 0);
        }
    }
    ...
    update(dtMs) {
            ...
            switch (info.Type) {
                case BackType.HMoveA:
                    if (!this._scrollLogged) {
                        console.log(`[MapScene] HMoveA bg cx=${info.Cx} cy=${info.Cy} period=${period}`);
                        this._scrollLogged = true;
                    }
                    this._bgScrollX[i] = ((this._bgScrollX[i] ?? 0) - spd) % period;
                    break;
                case BackType.HMoveB:
                    this._bgScrollX[i] = ((this._bgScrollX[i] ?? 0) + spd) % period;
                    break;
                case BackType.VMoveA:
                    this._bgScrollY[i] = ((this._bgScrollY[i] ?? 0) - spd) % periodY;
                    break;
        ...
        for (let i = 0; i < this._foregrounds.length; i++) {
            ...
            switch (info.Type) {
                case BackType.HMoveA:
                    this._fgScrollX[i] = ((this._fgScrollX[i] ?? 0) - spd) % period;
                    break;
                case BackType.HMoveB:
                    this._fgScrollX[i] = ((this._fgScrollX[i] ?? 0) + spd) % period;
                    break;
                case BackType.VMoveA:
                    this._fgScrollY[i] = ((this._fgScrollY[i] ?? 0) - spd) % periodY;
                    break;
                case BackType.VMoveB:
                    this._fgScrollY[i] = ((this._fgScrollY[i] ?? 0) + spd) % periodY;
                    break;
        ...
        for (const e of this._layer0Objects)
            e.anim?.Update(dtMs);
    }
    _drawBackEntry(info, sprite, anim, screenCenter, screenWidth, screenHeight, scrollX = 0, scrollY = 0) {
    ...
    _makeSprite(wz) {
        ...
        s.anchor.set(wz.Width > 0 ? wz.OriginX / wz.Width : 0, wz.Height > 0 ? wz.OriginY / wz.Height : 0);
        return s;
    }
    /**
     ...
     */
    _tileH(wz, bx, by, info, screenWidth, screenHeight, alpha = 1) {
     ...
     */
    _tileV(wz, bx, by, info, screenWidth, screenHeight, alpha = 1) {
     ...
     */
    _tileBoth(wz, bx, by, info, screenWidth, screenHeight, alpha = 1) {
    ...
    _tileHLegacy(wz, bx, by, info, screenWidth) {
        const period = info.Cx > 0 ? info.Cx : wz.Width;
        if (period <= 0) {
            const s = this._cloneSprite(wz);
            s.position.set(bx, by);
    ...
    _tileVLegacy(wz, bx, by, info, screenHeight) {
        const period = info.Cy > 0 ? info.Cy : wz.Height;
        if (period <= 0) {
            const s = this._cloneSprite(wz);
            s.position.set(bx, by);
    ...
    _tileBothLegacy(wz, bx, by, info, screenWidth, screenHeight) {
        const px = info.Cx > 0 ? info.Cx : wz.Width;
        const py = info.Cy > 0 ? info.Cy : wz.Height;
        if (px <= 0 || py <= 0) {
            const s = this._cloneSprite(wz);
        ...
        let sx = bx;
        while (sx - wz.OriginX > -px)
            sx -= px;
        let sy = by;
        while (sy - wz.OriginY > -py)
            sy -= py;
    ...
    _loadBackEntry(bg) {
        if (this._mapPkg === null)
            return { sprite: null, anim: null };
