...
export class FieldScene {
    ...
    _fgContainer = new Container();
    _portalContainer = new Container();
    Camera;
    Crc = 0;
    _mapScene = null;
    _info = new MapInfo();
    _physics = { ...DEFAULT_PHYSICS };
    _footholds = {};
    _footholdIndex = new FootholdIndex();
    _portals = {};
    _ladderRopes = [];
    ...
    _portalPixiSprites = new Map();
    _loaded = false;
    constructor(_mapWz, _loader, camera) {
        this._mapWz = _mapWz;
    ...
    get LoadedMapId() { return this._loadedMapId; }
    ...
    Load(mapId) {
        ...
        try {
            this._mapScene.Load(root);
        }
        catch (ex) {
            console.warn('MapScene backdrop load failed', ex);
        }
        this._bgContainer.removeChildren();
        if (this._mapScene?.container)
            this._bgContainer.addChild(this._mapScene.container);
        this._loadInfo(root);
    ...
    _loadInfo(root) {
        ...
        mi.OnFirstUserEnter = info.Get('onFirstUserEnter') ?? '';
        mi.OnUserEnter = info.Get('onUserEnter') ?? '';
        mi.FieldType = this._readInt(info, 'fieldType');
        mi.Effect = info.Get('effect') ?? '';
        ...
        mi.WeatherMsg = info.Get('weather') ?? '';
        // RestorePhaseBG (0x532DD0): reads `phase` node
        mi.PhaseBG = info.Get('phase') ?? '';
        ...
        mi.UserInfo = info.Get('userInfo') ?? '';
        // RestorePeculiarInfo (0x546560): reads `peculiarInfo` node
        mi.PeculiarInfo = info.Get('peculiarInfo') ?? '';
    ...
    _loadMiniMap(root) {
        ...
        if (!(mm instanceof WzProperty))
            return;
        const canvas = mm.Get('canvas');
        const markName = (root.Get('info') instanceof WzProperty ? root.Get('info').Get('mapMark') : null)?.toString();
        let mark = null;
        if (markName && this._mapWz?.GetItem(`MapHelper.img/mark/${markName}`) instanceof WzCanvas) {
            mark = this._loader.Load(this._mapWz.GetItem(`MapHelper.img/mark/${markName}`));
        }
        this._miniMap = new MiniMapData(canvas instanceof WzCanvas ? this._loader.Load(canvas) : null, mark, this._readInt(mm, 'width'), this._readInt(mm, 'height'), this._readInt(mm, 'centerX'), this._readInt(mm, 'centerY'), Math.max(0, this._readInt(mm, 'mag')), [], // Footholds — filled in _finalizeMiniMapData
    ...
    _finalizeMiniMapData() {
        ...
        this._miniMap = new MiniMapData(this._miniMap.Canvas, this._miniMap.Mark, this._miniMap.Width, this._miniMap.Height, this._miniMap.CenterX, this._miniMap.CenterY, this._miniMap.Mag, fhs, lrs, this._miniMap.Real_W, this._miniMap.Real_H, this._miniMap.Real_CX, this._miniMap.Real_CY);
    }
    _loadFootholds(root) {
        ...
        for (const [layerKey, layerNode] of Object.entries(fhRoot.Items)) {
            if (!(layerNode instanceof WzProperty))
                continue;
            ...
            for (const [groupKey, groupNode] of Object.entries(layerNode.Items)) {
                ...
                for (const [idStr, entryNode] of Object.entries(groupNode.Items)) {
                    if (!(entryNode instanceof WzProperty))
                        continue;
    ...
    _computeBounds() {
        ...
        if (info.VRLeft !== 0)
            left = Math.max(left, info.VRLeft + 20);
        if (info.VRRight !== 0)
            right = Math.min(right, info.VRRight - 4);
        if (info.VRTop !== 0)
            top = Math.max(top, info.VRTop + 65);
        if (info.VRBottom !== 0)
            bottom = Math.min(bottom, info.VRBottom);
    ...
    _loadPortals(root) {
        ...
        for (const [idxStr, node] of Object.entries(portalRoot.Items)) {
            ...
            p.TargetPortal = node.Get('tn') ?? '';
            p.Image = node.Get('image') ?? '';
            ...
            p.ReactorName = node.Get('reactorName') ?? '';
            p.SessionValueKey = node.Get('sessionValueKey') ?? '';
            p.SessionValue = node.Get('sessionValue') ?? '';
            p.Script = node.Get('script') ?? '';
            p.Teleport = this._readInt(node, 'teleport') !== 0;
            this._portals[idx] = p;
        }
    }
    _loadLadderRope(root) {
        ...
        for (const [snStr, node] of Object.entries(lrRoot.Items)) {
            ...
            this._ladderRopes.push(new LadderRope(sn, this._readInt(node, 'l') !== 0, this._readInt(node, 'uf') !== 0, this._readInt(node, 'x'), this._readInt(node, 'y1'), this._readInt(node, 'y2'), this._readInt(node, 'page')));
        }
    }
    _loadSeats(root) {
        const seatRoot = root.Get('seat');
        this._seats = [];
        if (!(seatRoot instanceof WzProperty))
            return;
        for (const node of Object.values(seatRoot.Items)) {
            if (node instanceof WzVector)
                this._seats.push({ x: node.X, y: node.Y });
        }
    }
    ...
    FindSeatByPosition(x, y) {
        for (let i = 0; i < this._seats.length; i++) {
            const s = this._seats[i];
            if (x >= s.x - 10 && x <= s.x + 10 && y >= s.y - 30 && y <= s.y + 30)
                return i;
        }
        return -1;
    }
    ...
    _loadPortalAnimations() {
        ...
        for (const portal of Object.values(this._portals)) {
            ...
            if (!cache.has(image)) {
                ...
                cache.set(image, this._loader.LoadAnimation(this._mapWz?.GetItem(`MapHelper.img/portal/game/${family}/${image}/portalContinue`)));
            }
        }
    }
    _loadLayers(root) {
        ...
        for (let layer = 0; layer < LayerCount; layer++) {
            ...
            if (!(lp instanceof WzProperty))
                continue;
            const tileSet = (lp.Get('info') instanceof WzProperty ? lp.Get('info').Get('tS') : null)?.toString();
            if (tileSet && lp.Get('tile') instanceof WzProperty) {
                const tileRoot = lp.Get('tile');
    ...
    // ── Per-frame update ─────────────────────────────────────────────
    Update(dtMs, screenW = 800, screenH = 600) {
        if (!this._loaded)
            return;
        ...
        for (const layer of this._objLayers) {
            for (const o of layer)
                o.sprite?.Update(dtMs);
        }
        this._portalPv?.Update(dtMs);
        for (const [, anim] of this._portalPh)
            anim?.Update(dtMs);
        for (const [, anim] of this._portalPsh)
            anim?.Update(dtMs);
        this._syncPortalTextures();
    }
    // ── Drawing ───────────────────────────────────────────────────────
    UpdateEntities(characters, player, drops, mobs, npcs, screenW, screenH) {
            ...
            for (const t of this._tileLayers[layer]) {
                ...
                if (t.anim) {
                    ...
                    if (!t.pixiSprite) {
                        ...
                        t.pixiSprite.anchor.set(t.sprite.Width > 0 ? t.sprite.OriginX / t.sprite.Width : 0, t.sprite.Height > 0 ? t.sprite.OriginY / t.sprite.Height : 0);
                    }
    ...
    _updateEntityContainers(characters, player, drops, mobs, npcs, cx, cy) {
    ...
    _worldToScreen(wx, wy, cx, cy) {
        return { x: wx - this.Camera.Position.x + cx, y: wy - this.Camera.Position.y + cy };
    }
    _portalAnimation(portal) {
        if (portal.Type === 2 || portal.Type === 4 || portal.Type === 7)
            return this._portalPv;
        if (this._activeHiddenPortalIndex !== portal.Index)
            return null;
        const image = portal.Image || 'default';
    ...
    _syncPortalTextures() {
        for (const portal of Object.values(this._portals)) {
            ...
            sprite.anchor.set(frame.width > 0 ? frame.OriginX / frame.width : 0, frame.height > 0 ? frame.OriginY / frame.height : 0);
        }
    }
    ...
    GetFootholdAbove(x, yTop, yBottom) {
        ...
        for (const fh of candidates.length > 0 ? candidates : Object.values(this._footholds)) {
            ...
            if (gy === null)
                continue;
            if (gy > yTop && gy <= yBottom && gy > bestY) {
                bestY = gy;
                best = fh;
            }
        }
    ...
    GetCrossCandidate(xm1, ym1, xm2, ym2) {
    ...
    GetLadderOrRope(x1, y1, x2 = x1, y2 = y1) {
        ...
        for (const lr of this._ladderRopes) {
            if (lr.X >= minX && lr.X <= maxX && lr.Bottom >= minY && lr.Top <= maxY)
                return lr;
        }
        return null;
    }
    _loadPhysics() {
        ...
        const read = (key) => {
            ...
            if (typeof value === 'number' && Number.isFinite(value))
                this._physics[key] = value;
        };
        for (const key of Object.keys(this._physics))
            read(key);
    }
    /** CWvsPhysicalSpace2D::CanWalkThrough: validates a linked walkable chain. */
    CanWalkThrough(from, to) {
        if (!from || !to || from.Uvx <= 0 || to.Uvx <= 0)
            return false;
        if (from === to)
            return true;
        if (from.X2 > to.X1) {
    ...
    LayerOfFoothold(id, fallback = 7) {
        const fh = this._footholds[id];
        return fh ? fh.Layer : fallback;
    }
    LayerAt(x, y, fallback = 7) {
        const fh = this.GetFootholdBelow(x, y);
        return fh ? fh.Layer : fallback;
    }
    GetZMassWallX(zmass, fromX, toX, yTop, yBottom) {
        if (fromX === toX)
            return null;
        ...
        for (const fh of candidates.length > 0 ? candidates : Object.values(this._footholds)) {
            if (fh.X1 >= fh.X2 || fh.ZMass !== zmass)
                continue;
            const wx = fh.X1;
            if (wx < lo || wx > hi)
                continue;
            const wTop = Math.min(fh.Y1, fh.Y2), wBot = Math.max(fh.Y1, fh.Y2);
            if (wBot < yTop || wTop > yBottom)
                continue;
            if (best === null || (movingRight ? wx < best : wx > best))
                best = wx;
        }
        return best;
    }
    // ── Player placement ──────────────────────────────────────────────
    PlacePlayerAtPortal(player, portalIndex) {
        let portal = this._portals[portalIndex];
        if (!portal)
            portal = this._portals[0];
        if (!portal) {
        ...
        if (fhBelow) {
            ...
            if (gy !== null)
                player.Position.y = Math.min(player.Position.y, gy);
        }
...
//# sourceMappingURL=FieldScene.js.map