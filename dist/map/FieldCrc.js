...
function crcByte(data, init) {
    return step(init >>> 0, data) | 0;
}
function crcLong(data, init) {
    let r = init >>> 0;
...
function doubleToLow32(v) {
    _f64Buf[0] = v;
    return _f64View.getInt32(0, true);
}
function constantCrc(mapWz) {
    if (_constantCrc !== null)
        return _constantCrc;
...
function computeSpace2D(constant, mapImg) {
    ...
    if (lrRoot instanceof WzProperty) {
        for (const [snKey, lrVal] of orderByIntKey(lrRoot)) {
            if (!(lrVal instanceof WzProperty))
                continue;
            crc = crcInt(parseIntKey(snKey), crc);
...
function computePortalList(mapId, mapImg) {
    let crc = crcInt(mapId, 0);
    const pRoot = mapImg.Get("portal");
    if (!(pRoot instanceof WzProperty))
        return crc;
    for (const [, pVal] of orderByIntKey(pRoot)) {
        if (!(pVal instanceof WzProperty))
            continue;
...
function readStr(node, key) {
    if (!node)
        return '';
    const val = node.Get(key);
    return typeof val === 'string' ? val : '';
}
function parseIntKey(s) {
    const v = parseInt(s, 10);
    return isNaN(v) ? 0 : v;
}
export function compute(mapId, mapImg, mapWz) {
...
//# sourceMappingURL=FieldCrc.js.map