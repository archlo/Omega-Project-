...
export class MigrationCoordinator {
    _session;
    _pending = null;
    onPhase2BoundaryReached = null;
    get migrationActive() { return this._pending !== null; }
    constructor(session) {
        this._session = session;
    }
    async beginMigrateAsync(channelHost, channelPort, characterId) {
        if (channelHost.length !== 4)
            throw new Error('channelHost must be 4 bytes');
        ...
        if (!this._session.isConnected)
            this.onLoginDisconnect();
    }
    onLoginDisconnect(err) {
        if (!this._pending)
            return;
