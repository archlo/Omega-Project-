...
export class CashShopDecoder {
    ...
    static decodeModifiedCommodity(p) {
        const flags = p.readInt();
        const has = (bit) => (flags & bit) !== 0;
        const m = { flags };
        if (has(CommodityFlag.ITEMID))
            m.itemId = p.readInt();
        if (has(CommodityFlag.COUNT))
            m.count = p.readShort();
        if (has(CommodityFlag.PRIORITY))
            m.priority = p.readByte();
        if (has(CommodityFlag.PRICE))
            m.price = p.readInt();
        if (has(CommodityFlag.BONUS))
            m.bonus = p.readByte() !== 0;
        if (has(CommodityFlag.PERIOD))
            m.period = p.readShort();
        if (has(CommodityFlag.REQPOP))
            m.reqPop = p.readShort();
        if (has(CommodityFlag.REQLEV))
            m.reqLevel = p.readShort();
        if (has(CommodityFlag.MAPLEPOINT))
            m.maplePoint = p.readInt();
        if (has(CommodityFlag.MESO))
            m.meso = p.readInt();
        if (has(CommodityFlag.FORPREMIUMUSER))
            m.forPremiumUser = p.readByte() !== 0;
        if (has(CommodityFlag.COMMODITYGENDER))
            m.gender = p.readByte();
        if (has(CommodityFlag.ONSALE))
            m.onSale = p.readByte() !== 0;
        if (has(CommodityFlag.CLASS))
            m.classField = p.readByte();
        if (has(CommodityFlag.LIMIT))
            m.limit = p.readByte();
        if (has(CommodityFlag.PBCASH))
            m.pbCash = p.readShort();
        if (has(CommodityFlag.PBPOINT))
            m.pbPoint = p.readShort();
        if (has(CommodityFlag.PBGIFT))
            m.pbGift = p.readShort();
...
//# sourceMappingURL=CashShopDecoder.js.map