...
export class ShootAttackTarget {
    mobId;
    damage;
    hitX;
    hitY;
...
export class ShootAttackEncoder {
    /**
     ...
     */
    static Encode(fieldKey, actionAndDir, attackSpeed, bulletPosition, userX, userY, ballStartX, ballStartY, targets, damagePerMob = 1, flag = 0, options = {}) {
        if (damagePerMob < 1 || damagePerMob > 0xF) {
            throw new RangeError('damagePerMob must be 1..15');
        }
        if (targets.length > 0xF) {
            throw new Error('a single shoot attack can target at most 15 mobs');
        }
        const p = OutPacket.Of(InHeader.UserShootAttack);
        ...
        p.writeByte(flag);
        p.writeByte(options.exJablin ?? 0);
        p.writeShort(actionAndDir);
        p.writeInt(0); // GETCRC32Svr
        p.writeByte(0); // nAttackActionType
        p.writeByte((attackSpeed & 0xF) | ((options.reduceCount ?? 0) << 4));
        p.writeInt(0); // tAttackTime
        p.writeInt(0); // dwID
        p.writeShort(bulletPosition);
        p.writeShort(0); // pnCashItemPos
        p.writeByte(0); // nShootRange0a
        if (options.bulletItemId !== undefined)
            p.writeInt(options.bulletItemId);
        for (const t of targets) {
            if (t.damage.length !== damagePerMob) {
                throw new Error(`ShootAttackTarget ${t.mobId} has ${t.damage.length} damage values but damagePerMob is ${damagePerMob}`);
            }
            p.writeInt(t.mobId);
        ...
        p.writeShort(userX);
        p.writeShort(userY);
        if (options.wildHunterBodyRelMoveY !== undefined)
            p.writeShort(options.wildHunterBodyRelMoveY);
        p.writeShort(ballStartX);
        p.writeShort(ballStartY);
        if (options.reserveSpark !== undefined)
            p.writeInt(options.reserveSpark);
        return p.toArray();
    }
}
//# sourceMappingURL=ShootAttackEncoder.js.map