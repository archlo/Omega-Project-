...
function isEvanJob(jobId) { return Math.floor(jobId / 100) === 22 || jobId === 2001; }
function isDualJob(jobId) { return Math.floor(jobId / 10) === 43; }
function getJobLevel(jobId) {
    if (jobId % 100 === 0 || jobId === 2001)
        return 1;
    const raw = isDualJob(jobId) ? Math.floor((jobId - 430) / 2) : jobId % 10;
    const level = raw + 2;
    if (level >= 2 && (level <= 4 || (level <= 10 && isEvanJob(jobId))))
        return level;
    return 0;
}
...
function isSkillNeedMasterLevel(skillId) {
    if (IGNORE_MASTER_LEVEL_COMMON.has(skillId))
        return false;
    ...
    if (jobId === 100 * Math.floor(jobId / 100))
        return false;
    return jobId % 10 === 2;
}
function decodeIndexedItemsShort(p) {
    const out = [];
...
function decodeIndexedItemsByte(p) {
    const out = [];
    let slot = p.readByte();
    while (slot !== 0) {
        const item = ItemDecoder.Decode(p);
...
export class CharacterDataDecoder {
    static Decode(p) {
        ...
        d.flag = p.readLong();
        const flag = d.flag;
        const has = (bit) => (flag & BigInt(bit)) !== 0n;
        ...
        if (has(DBChar.CHARACTER)) {
            ...
            if (p.readByte() !== 0)
                d.linkedCharacter = p.readString();
        }
        const o1 = snap();
        if (has(DBChar.MONEY))
            d.money = p.readInt();
        ...
        const o3 = snap();
        if (has(DBChar.EQUIPEXT))
            d.equipExtExpire = p.readLong();
        const o4 = snap();
        ...
        const o5 = snap();
        if (has(DBChar.ITEMSLOTCONSUME))
            d.consumeInventory = decodeIndexedItemsByte(p);
        if (has(DBChar.ITEMSLOTINSTALL))
            d.installInventory = decodeIndexedItemsByte(p);
        if (has(DBChar.ITEMSLOTETC))
            d.etcInventory = decodeIndexedItemsByte(p);
        if (has(DBChar.ITEMSLOTCASH))
            d.cashInventory = decodeIndexedItemsByte(p);
        const o6 = snap();
        ...
        if (has(DBChar.SKILLRECORD)) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const entry = { skillId, level };
                if (isSkillNeedMasterLevel(skillId))
                    entry.masterLevel = p.readInt();
        ...
        if (has(DBChar.SKILLCOOLTIME)) {
            ...
            const entries = [];
            for (let i = 0; i < count; i++)
                entries.push({ skillId: p.readInt(), cooltime: p.readShort() });
        ...
        if (has(DBChar.QUESTRECORD)) {
            ...
            const entries = [];
            for (let i = 0; i < count; i++)
                entries.push({ questId: p.readShort(), value: p.readString() });
        ...
        if (has(DBChar.QUESTCOMPLETE)) {
            ...
            const entries = [];
            for (let i = 0; i < count; i++)
                entries.push({ questId: p.readShort(), completedTime: p.readLong() });
        ...
        if (has(DBChar.COUPLERECORD)) {
            ...
            for (let i = 0; i < coupleCount; i++)
                d.coupleRecords.push(decodeCoupleRecord(p));
            const friendCount = p.readShort();
            for (let i = 0; i < friendCount; i++)
                d.friendRecords.push(decodeFriendRecord(p));
            const marriageCount = p.readShort();
            for (let i = 0; i < marriageCount; i++)
                d.marriageRecordsRaw.push(p.readBytes(0x30));
        }
...
//# sourceMappingURL=CharacterDataDecoder.js.map