...
export class PacketCipher {
    ...
    static BuildHeader(payloadLen, iv, destination) {
        if (iv.length < 4)
            throw new Error('iv must be at least 4 bytes');
        if (destination.length < HeaderSize)
            throw new Error('destination must be at least 4 bytes');
    ...
    static ParseHeader(header, iv) {
        if (header.length < HeaderSize || iv.length < 4)
            return { valid: false, payloadLength: 0 };
