...
export class LoginSender {
    static CheckPassword(username, password, machineId) {
    ...
    static SelectWorld(worldId, channelId) {
        const p = OutPacket.Of(InHeader.SelectWorld);
        p.writeByte(worldId);
        p.writeByte(channelId);
        return p;
    }
    static SelectCharacter(characterId) {
    ...
    static ConfirmEULA(accepted) {
        const p = OutPacket.Of(InHeader.ConfirmEULA);
        p.writeByte(accepted ? 1 : 0);
        return p;
    }
    static CheckSPWRequest(pic, characterId) {
    ...
    static EnableSPWRequest(characterId, pic) {
    ...
    static CreateNewCharacter(name, race, face, hair, hairColor, skin, coat, pants, shoes, weapon, male, subJob = 0) {
    ...
    static CreateNewCharacterInCS(name, race, charSaleJob, abilityAllocations) {
        if (abilityAllocations.length !== 9)
            throw new Error('CreateNewCharacterInCS requires exactly 9 ability allocations');
        const p = OutPacket.Of(InHeader.CreateNewCharacterInCS);
        p.writeString(name);
        p.writeInt(race);
        p.writeInt(charSaleJob - 1);
        for (const al of abilityAllocations)
            p.writeInt(al);
        return p;
    }
    static DeleteCharacter(characterId, secondaryPassword) {
    ...
    static ViewAllChar(gameStartMode, passport, machineId, gameRoomClient) {
    ...
    static SelectCharacterByVAC(characterId, worldId) {
    ...
    static EnableSPWRequestByVAC(characterId, worldId, secondaryPassword) {
    ...
    static CheckSPWRequestByVAC(characterId, worldId, secondaryPassword) {
...
//# sourceMappingURL=LoginSender.js.map