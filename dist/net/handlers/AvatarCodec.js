...
export class AvatarCodec {
    ...
    static PopulateEquipsFromInventory(look, equipped, equippedCash = []) {
    ...
    static FromCharacterData(stat, equipped, equippedCash) {
        const look = new AvatarLook();
        ...
        AvatarCodec.PopulateEquipsFromInventory(look, equipped.map(({ slot, item }) => ({ pos: slot, itemId: item.itemId })), equippedCash.map(({ slot, item }) => ({ pos: slot, itemId: item.itemId })));
        return look;
    }
    static EncodeAvatarLook(p, look) {
...
//# sourceMappingURL=AvatarCodec.js.map