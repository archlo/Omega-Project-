...
var SubScreen;
(function (SubScreen) {
    SubScreen[SubScreen["Name"] = 0] = "Name";
    SubScreen[SubScreen["Look"] = 1] = "Look";
})(SubScreen || (SubScreen = {}));
...
function convertRaceToUiRace(serverRace) {
    switch (serverRace) {
        case 0: return 4;
        case 1: return 1;
        case 2: return 0;
...
export class CharCreationStage extends Stage {
    ...
    _nameFieldNudgeX = 0;
    _nameFieldNudgeY = 0;
    _nameOkOffset = { x: 27, y: 178 };
    _nameNoOffset = { x: 101, y: 178 };
    _cameraOffset = { x: 27, y: -1216 };
    _scrollT = 0;
    _boardWorld = { x: 109, y: -2613 };
    _avatarWorld = { x: 22, y: -2369 };
    ...
    _lastLook = '';
    _uiContainer = new Container();
    // Pixi sprites for board elements (created once in onEnter)
    _charNameBoardSprite = null;
    _charSetBoardSprite = null;
    ...
    _boardBuilt = false;
    _lastSubScreen = -1;
    constructor(ui, map, sound, worldId, channelId, isMale, raceIndex, cameraStart, loginCameraOffset) {
        super();
        this._ui = ui;
        this._map = map;
        this._sound = sound;
        ...
        for (let i = 0; i < RowCount; i++)
            this._rowNormal.push(null);
        for (let i = 0; i < RowCount; i++)
            this._rowDisabled.push(null);
        for (let i = 0; i < RowCount; i++)
            this._btLeftRow.push(null);
        for (let i = 0; i < RowCount; i++)
            this._btRightRow.push(null);
        for (let i = 0; i < RowCount; i++)
            this._rowSprites.push(null);
        for (let i = 0; i < RowCount; i++)
            this._rowTextSprites.push(null);
    }
    onEnter(game) {
        super.onEnter(game);
        ...
        this._renderer = new CharacterRenderer(game.wz.character ?? null, game.wz.item ?? null, game.wz.base ?? null, this._loader);
        ...
        const classRoot = this._ui.GetItem(`Login.img/NewChar${suffix}`);
        const newChar = this._ui.GetItem('Login.img/NewChar');
        ...
        if (this._charNameBoard)
            this._charNameBoardSprite = this._charNameBoard.ToPixi();
        if (this._charSetBoard)
            this._charSetBoardSprite = this._charSetBoard.ToPixi();
        for (let i = 0; i < RowCount; i++) {
        ...
        this._notice.onOk = () => { if (this._nameField)
            this._nameField.isFocused = true; };
    ...
    _updateBoardVisibility() {
        ...
        if (this._subScreen === SubScreen.Name) {
            if (this._charNameBoardSprite) {
                this._charNameBoardSprite.visible = true;
                this._boardContainer.addChild(this._charNameBoardSprite);
            }
            if (this._nameField) {
                this._nameField.container.visible = true;
                this._boardContainer.addChild(this._nameField.container);
            }
            if (this._btYes)
            ...
            if (this._charSetBoardSprite)
                this._charSetBoardSprite.visible = false;
            for (let i = 0; i < RowCount; i++) {
                if (this._rowSprites[i])
                    this._rowSprites[i].visible = false;
                if (this._rowTextSprites[i])
                    this._rowTextSprites[i].visible = false;
                if (this._btLeftRow[i])
                    this._btLeftRow[i].container.visible = false;
                if (this._btRightRow[i])
                    this._btRightRow[i].container.visible = false;
            }
        }
        else {
            if (this._charSetBoardSprite) {
            ...
            if (this._charNameBoardSprite)
                this._charNameBoardSprite.visible = false;
            if (this._nameField)
                this._nameField.container.visible = false;
        }
        this._applyBoardPositions();
    }
    _updatePreview() {
        ...
        if (lookKey === this._lastLook && !this._previewNeedsUpdate)
            return;
    ...
    onKeyPress(key) {
        ...
        switch (key) {
            ...
            case 'ArrowDown':
                ...
                break;
            case 'ArrowLeft':
                this._cycleRow(this._selectedRow, -1);
                break;
            case 'ArrowRight':
                this._cycleRow(this._selectedRow, +1);
                break;
    ...
    _buildLook() {
        ...
        if (coat !== 0)
            look.hairEquip.set(BodyPartSlot.Clothes, coat);
        if (pants !== 0)
            look.hairEquip.set(BodyPartSlot.Pants, pants);
        if (shoes !== 0)
            look.hairEquip.set(BodyPartSlot.Shoes, shoes);
        if (weapon !== 0)
            look.hairEquip.set(BodyPartSlot.Weapon, weapon);
        return look;
    }
    _rowName(row) {
        switch (row) {
    ...
    _onCheckName() {
        ...
        if (err) {
            this._notice?.show(err);
            return;
        }
        if (this._forbidden.IsForbidden(name)) {
            this._notice?.show('This name cannot be used.');
            return;
        }
    ...
    _sendCreate() {
        ...
        this.game.session.send(LoginSender.CreateNewCharacter(name, srace, face, hairBase, hairColor, skin, coat, pants, shoes, weapon, this._male, subJob));
    }
    _onCreateCharacterResult(args) {
    ...
    _goBack() {
        this._playClick();
        this.stageDirector.replace(new CharSelectStage(this._ui, this._map, this._sound, this.game.wz.character ?? null, this.game.wz.item ?? null, this.game.wz.base ?? null, this._worldId, this._channelId, this._scene?.Camera ?? { x: 0, y: 0 }, this._loginCameraOffset));
    }
    _goBackToCharSelect() {
        this.stageDirector.replace(new CharSelectStage(this._ui, this._map, this._sound, this.game.wz.character ?? null, this.game.wz.item ?? null, this.game.wz.base ?? null, this._worldId, this._channelId, this._scene?.Camera ?? { x: 0, y: 0 }, this._loginCameraOffset));
    }
    _validateName(name) {
        ...
        for (const c of name)
            if (c === 'I' || c === 'l')
                il++;
        if (il >= 4)
            return 'Too many look-alike I/l characters.';
        return '';
    }
    ...
    _applyBoardPositions() {
        const board = this._boardScreen();
        if (this._subScreen === SubScreen.Name) {
            if (this._charNameBoardSprite)
                this._charNameBoardSprite.position.set(board.x, board.y);
            if (this._nameField) {
                this._nameField.setPosition(board.x + 33 + this._nameFieldNudgeX, board.y + 106 + this._nameFieldNudgeY);
            }
            if (this._btYes)
                this._btYes.container.position.set(board.x + this._nameOkOffset.x, board.y + this._nameOkOffset.y);
            if (this._btNo)
                this._btNo.container.position.set(board.x + this._nameNoOffset.x, board.y + this._nameNoOffset.y);
        }
        else {
            if (this._charSetBoardSprite)
                this._charSetBoardSprite.position.set(board.x, board.y);
            for (let i = 0; i < RowCount; i++) {
                if (this._rowSprites[i])
                    this._rowSprites[i].position.set(board.x + 14, board.y + 102 + 18 * i);
                if (this._rowTextSprites[i]) {
                ...
                if (this._btLeftRow[i])
                    this._btLeftRow[i].container.position.set(board.x + 66, board.y + 103 + 18 * i);
                if (this._btRightRow[i])
                    this._btRightRow[i].container.position.set(board.x + 198, board.y + 103 + 18 * i);
            }
            if (this._btYes)
                this._btYes.container.position.set(board.x + 37, board.y + 330);
            if (this._btNo)
                this._btNo.container.position.set(board.x + 111, board.y + 330);
        }
    }
    _boardScreen() {
        if (this._scene) {
    ...
    _nameColors(suffix) {
        if (suffix === 'Resistance')
            return { textColor: 0x000000, caretColor: 0x000000 };
        return { textColor: 0xFFFFFF, caretColor: 0x78DCFF };
    }
    _nameButtonOffsets(suffix) {
        switch (suffix) {
    ...
    _makeButtonFrom(root, name, onClick) {
        try {
            if (!root)
                return null;
...
//# sourceMappingURL=CharCreationStage.js.map