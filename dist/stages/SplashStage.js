...
const FILES = ['UI', 'Map', 'Sound', 'Character', 'Item', 'Base', 'Skill', 'Etc'];
const COL = {
    bg: 0x06080F,
    panel: 0x0D1426,
    border: 0x1E2D50,
...
export class SplashStage extends Stage {
    ...
    draw() { }
    onMouseButton(_x, _y, down, button) {
        // Only used for retry after a load error
        if (!down || button !== MouseButton.Left || this._loading)
            return;
        if (this._promptText.visible)
            this._startLoading();
    }
    _startLoading() {
        this._loading = true;
    ...
    async _loadAll() {
        ...
        const open = (name) => WzPackage.OpenBaseAsync(wzDir, name, 95, mkCb(name)).then(done(name));
        ...
        g.wz.skill = skill;
        g.wz.etc = etc;
        g.cursor.loadFromWz(ui).catch((e) => console.warn('[SplashStage] cursor WZ load failed', e));
...
//# sourceMappingURL=SplashStage.js.map