...
export class PinStage extends Stage {
    ...
    onEnter(game) {
        ...
        for (const g of this._cells)
            this._panelContainer.addChild(g);
        for (const t of this._cellTexts)
            this._panelContainer.addChild(t);
    ...
    onTextInput(character) {
        if (this._submitting || !this._pinEntryEnabled)
            return;
        if (this._digits.length >= PIN_DIGITS)
            return;
    ...
    onKeyPress(key) {
        ...
        if (key === 'Enter') {
            if (this._pinEntryEnabled && this._digits.length === PIN_DIGITS && !this._submitting)
                this._submitPin();
            return;
        }
    ...
    _buildPanel() {
        ...
        this._loginBtn = this._makeButton('BtLogin', signTopLeft, { x: BT_LOGIN_X_LOCAL, y: BTN_ROW_Y_LOCAL }, () => {
            if (this._digits.length === PIN_DIGITS && !this._submitting)
                this._submitPin();
        });
    ...
    _loadCanvas(path) {
        try {
            ...
            if (!(item instanceof WzCanvas)) {
                console.warn(`PinStage: ${path} → ${item === null ? 'null' : item?.constructor?.name ?? typeof item}`);
                return null;
            }
...
//# sourceMappingURL=PinStage.js.map