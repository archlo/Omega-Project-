...
export class CashShopStage extends Stage {
    ...
    _commodities = [];
    _bestItems = [];
    _modifiedCommodities = [];
    _discountRates = new Map(); // key: "category:index" → rate%
    _notSaleSNs = new Set();
    _stockStates = new Map();
    _limitGoods = [];
    _zeroGoods = [];
    // ── UI state ──
    _activeTab = 0;
    ...
    _nxPrepaid = 0;
    _maplePoints = 0;
    _cashShopAuthorized = false;
    _statusMessage = '';
    _couponVisible = false;
    _couponValue = '';
    _selectedPlate = -1;
    _focusedPlate = -1;
    _buyPending = false;
    ...
    _confirmBuyVisible = false;
    _confirmBuyItem = null;
    _confirmBuyPaymentType = 0; // 0=NX Credit, 1=Maple Point, 2=Prepaid NX
    // ── Character preview ──
    _charLook = null;
    _charWz = null;
    _itemWz = null;
    ...
    _giftReceiver = '';
    _giftMessage = '';
    _giftEditingField = 0; // 0=none, 1=receiver, 2=message
    // ── Inventory click state ──
    _selectedInvCell = -1;
    _lastInventoryClickCell = -1;
    _lastInventoryClickAt = 0;
    _invItemTI = 0; // OG m_nItemTI — current inventory tab (0=equip, 1=use, 2=setup, 3=etc, 4=cash)
    _invFirstPosition = 0; // OG m_nFirstPosition — scroll offset for inventory grid
    _invSlotCount = 0; // total items in current inventory tab
    ...
    _lockerScroll = 0;
    _cashInventoryItems = [];
    _wishlist = new Array(10).fill(0);
    _giftRecords = new Uint8Array(0);
    // ── Dialog states (OG: CConfirmPurchaseDlg / CUINameChangeDlg / etc) ──
    _nameChangeVisible = false;
    _nameChangeItem = null;
    _nameChangeNewName = '';
    _worldTransferVisible = false;
    _worldTransferItem = null;
    _worldTransferNames = [];
    _worldTransferSelected = -1;
    _coupleNameVisible = false;
    _coupleNameItem = null;
    _coupleNameValue = '';
    _friendNameVisible = false;
    _friendNameItem = null;
    _friendNameValue = '';
    _equipSlotExtVisible = false;
    _equipSlotExtItem = null;
    _equipSlotExtBodyPart = 0;
    // ── Active dialog type ──
    _activeDialog = 'none';
    // ── One-a-Day state ──
    _oneADayItemSN = 0;
    _oneADayItemDate = 0;
    _oneADayPrevItems = [];
    _oneADayTimer = { hours: 0, minutes: 0, seconds: 0 };
    _oneADayTimerAccum = 0;
    _oneADaySelected = -1;
    _oneADayMode = 0;
    // ── WZ assets (loaded from UI.nx/CashShop.img) ──
    _bg = null;
    _bgList = null;
    _tabSprites = new Array(TAB_COUNT).fill(null);
    ...
    _previewBgs = [null, null, null];
    _previewOn = null;
    _previewOff = null;
    _previewEnabled = true;
    _btBuyAvatar = null;
    _btDefaultAvatar = null;
    _btTakeoffAvatar = null;
    ...
    _btSearch = null;
    _btSearchBuy = null;
    _btSearchCancel = null;
    _searchPopup = null;
    // UI/OneADay.img/CSOneADay assets.
    _oneADayBase = null;
    _oneADayItemBox = null;
    _oneADayBuy = null;
    _oneADayGift = null;
    ...
    _btExInstall = null;
    _btExTrunk = null;
    _inventoryScrollbar = null;
    _lockerScrollbar = null;
    // Effect badges (animated — multiple frames)
    _effectHot = null;
    _effectNew = null;
    _effectSale = null;
    // Discount digits 0-9 + bonus/line/arrow/backgrnd/total
    _discountDigits = new Array(10).fill(null);
    _discountBg = null;
    _discountBonus = null;
    _discountLine = null;
    _discountTotal = null;
    // PicturePlate states (normal, mouseOver, pressed, disabled, noItem)
    _plateStateNormal = null;
    _plateStateHover = null;
    _plateStatePressed = null;
    _plateStateDisabled = null;
    // Panel backgrounds
    _bgLocker = null;
    _bgInventory = null;
    _bgStatus = null;
    ...
    _btGiftHide = null;
    _bgNameChange = null;
    _bgTransferWorld = null;
    _bgNameChangeNotice = null;
    _bgTransferWorldNotice = null;
    _btNameCheck = null;
    _confirmNotice = null;
    _confirmOk = null;
    _confirmNo = null;
        ...
        const loadWz = async () => {
            try {
                ...
                for (const commodity of this._commodities)
                    commodity.name = this._getItemName(commodity.itemId);
                for (const item of this._lockerItems)
                    item.name = this._getItemName(item.itemId);
                this._tryBuildCharacterPreview();
            }
            catch (ex) {
                console.warn('CashShopStage: failed to load WZ for character preview', ex);
            }
        };
        loadWz();
    }
    onExit() {
        ...
        this._clearDynamic();
        for (const s of this._dynamicIcons)
            s.destroy();
        this._dynamicIcons = [];
        this._icons = null;
    ...
    _wireHandlers(game) {
        ...
        h.onPurchaseExpChanged = (v) => { this._statusMessage = `Purchase EXP: ${v}`; };
        ...
        fh.onSetCashShop = (args) => {
            ...
            if (stat && args.characterData?.equipped) {
                const look = AvatarCodec.FromCharacterData(stat, args.characterData.equipped, args.characterData.equippedCash);
                if (!this._charLook) {
    ...
    _getDiscountRate(category, index) {
        const key = `${category}:${index}`;
        if (this._discountRates.has(key))
            return this._discountRates.get(key);
        const wildcard = `${category}:*`;
        if (this._discountRates.has(wildcard))
            return this._discountRates.get(wildcard);
        return 0;
    }
    ...
    _tryBuildCharacterPreview() {
        const data = this._characterData;
        const stat = data?.characterStat;
        if (!stat || !data?.equipped || !this._charWz || !this._itemWz || !this._baseWz || !this._loader)
            return;
    ...
    _isCommodityUsable(item) {
        if (item.reqLevel > 0 && this._playerLevel < item.reqLevel)
            return false;
        if (item.reqPop > 0 && this._playerFame < item.reqPop)
            return false;
        if (item.gender !== 0 && item.gender !== 2 && item.gender !== this._playerGender)
            return false;
    ...
    _decodeSaleTables(stock, limitGoods, zeroGoods) {
    ...
    _getLimitGoodsState(sn) {
        return this._limitGoods.find(entry => entry.sns.includes(sn))?.state ?? 0;
    }
    _saleTimeMatches(condition, dateStart, dateEnd, hourStart, hourEnd, weekdays) {
        const now = new Date();
        const date = now.getFullYear() * 10000 + (now.getMonth() + 1) * 100 + now.getDate();
        if ((condition & 2) !== 0 && (date < dateStart || date > dateEnd))
            return false;
        if ((condition & 4) !== 0 && !weekdays[now.getDay()])
            return false;
        if ((condition & 8) !== 0 && (now.getHours() < hourStart || now.getHours() >= hourEnd))
            return false;
        return true;
    }
    _isSaleAvailable(item) {
        if (item.stockState === 2 || item.limitState === 2)
            return false;
        const limit = this._limitGoods.find(entry => entry.sns.includes(item.sn));
        if (limit && (limit.count <= 0 || limit.state === 2 || !this._saleTimeMatches(limit.condition, limit.dateStart, limit.dateEnd, limit.hourStart, limit.hourEnd, limit.weekdays)))
            return false;
        const zero = this._zeroGoods.find(entry => item.sn >= entry.startSN && item.sn <= entry.endSN);
        if (zero && !this._saleTimeMatches(zero.condition, zero.dateStart, zero.dateEnd, zero.hourStart, zero.hourEnd, zero.weekdays))
            return false;
        return true;
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    update(_dt) {
        ...
        if (this._giftVisible)
            this._drawGiftDialog();
        if (this._confirmBuyVisible)
            this._drawConfirmBuy();
        if (this._activeDialog === 'nameChange')
            this._drawNameChangeDialog();
        if (this._activeDialog === 'worldTransfer')
            this._drawWorldTransferDialog();
        if (this._activeDialog === 'coupleName')
            this._drawCoupleNameDialog();
        if (this._activeDialog === 'friendName')
            this._drawFriendNameDialog();
        if (this._activeDialog === 'equipSlotExt')
            this._drawEquipSlotExtDialog();
    }
    draw() { }
    onResize(windowW, _windowH) {
        this._prevW = windowW;
    ...
    _drawCharacterPreview() {
        ...
        if (previewToggle)
            this._drawWzSprite(previewToggle, toggleX, toggleY);
        const avatarButtons = [
        ...
        for (const [sprite, x] of avatarButtons) {
            if (sprite)
                this._drawWzSprite(sprite, CHAR_X + x, CHAR_Y + 237);
        }
    }
    ...
    _getPreviewIndex() {
        ...
        if (Math.floor(job / 1000) === 1)
            return 1; // Cygnus Knights
        if (Math.floor(job / 100) === 21 || job === 2000)
            return 2; // Aran
    ...
    _drawTabBar() {
        ...
        if (tabSprite)
            this._drawWzSprite(tabSprite, TAB_X, TAB_Y);
    }
    ...
    _drawItemGrid() {
        ...
        for (let row = 0; row < PLATE_ROWS; row++) {
            for (let col = 0; col < PLATE_COLS; col++) {
                ...
                if (this._bgList)
                    this._drawWzSprite(this._bgList, px, py);
                if (absIdx < items.length) {
                    ...
                    if (plateBg && plateBg !== this._bgList)
    ...
    _drawOneADay() {
        ...
        if (this._oneADayBase)
            this._drawWzSprite(this._oneADayBase, LIST_X + 3, LIST_Y + 3);
        // Title
        ...
        const todayH = 100;
        if (this._oneADayItemBox)
            this._drawWzSprite(this._oneADayItemBox, todayX, todayY);
        ...
        if (this._oneADayItemSN > 0) {
            ...
            if (comm) {
                ...
                if (this._oneADayBuy)
                    this._drawWzSprite(this._oneADayBuy, buyX, buyY);
                if (this._oneADayGift)
                    this._drawWzSprite(this._oneADayGift, buyX, buyY - 22);
            }
            else {
    ...
    _getLockerScrollbarX() {
        const job = this._playerJob;
        if (Math.floor(job / 1000) === 1)
            return 5; // Cygnus Knights
        if (Math.floor(job / 100) === 21 || job === 2000)
            return 6; // Aran
        if (Math.floor(job / 1000) === 3)
            return 9; // Legendary (Aran/Evan job families)
        return 0; // Normal
    }
    // ── Inventory (CCSWnd_Inventory) — BOTTOM LEFT ──
    ...
    _drawInventoryPanel() {
        ...
        for (const [sprite, x, y] of expansionButtons) {
            if (sprite)
                this._drawWzSprite(sprite, INV_X + x, INV_Y + y);
        }
        ...
        if (this._btRebate)
            this._drawWzSprite(this._btRebate, LOCKER_X + 160, LOCKER_Y + 82);
    ...
    _drawSearchButton() {
        if (this._btSearch)
            this._drawWzSprite(this._btSearch, SEARCH_X, SEARCH_Y);
        // Search overlay — input field when search is active
        if (this._searchActive) {
            const overlayY = SEARCH_Y + SEARCH_H + 4;
            if (this._searchPopup)
                this._drawWzSprite(this._searchPopup, SEARCH_X - 10, overlayY - 4);
            // Input field label
            ...
            if (this._btSearchBuy)
                this._drawWzSprite(this._btSearchBuy, SEARCH_X - 6, overlayY + 20);
            if (this._btSearchCancel)
                this._drawWzSprite(this._btSearchCancel, SEARCH_X + 38, overlayY + 20);
            // Result count
    ...
    _drawConfirmBuy() {
        ...
        if (this._confirmOk)
            this._drawWzSprite(this._confirmOk, dlgX + 157, okY);
        // Cancel button
        if (this._confirmNo)
            this._drawWzSprite(this._confirmNo, dlgX + 207, okY);
    }
    // ── Name Change Dialog (OG: CUINameChangeDlg) ──
    _drawNameChangeDialog() {
        const dlgW = 266;
        const dlgH = 124;
        const dlgX = (CS_W - dlgW) / 2;
        const dlgY = (CS_H - dlgH) / 2;
        if (this._bgNameChange)
            this._drawWzSprite(this._bgNameChange, dlgX, dlgY);
    ...
    _drawWorldTransferDialog() {
        ...
        if (background)
            this._drawWzSprite(background, dlgX, dlgY);
    ...
    onKeyPress(key) {
        if (this._couponVisible) {
            if (key === 'Escape') {
                ...
                if (this._couponValue.length > 0)
                    this.game?.session.send(GameSender.CashShopUseCoupon(this._couponValue));
                this._couponVisible = false;
            }
        ...
        if (this._confirmBuyVisible) {
            ...
            if (key === 'Tab' || key === 'ArrowDown') {
                ...
                if (comm) {
                    ...
                    for (let tries = 0; tries < 3; tries++) {
                        if (payAvail[next]) {
                            this._confirmBuyPaymentType = next;
                            break;
                        }
                        next = (next + 1) % 3;
                    }
            ...
            if (key === 'ArrowUp') {
                ...
                if (comm) {
                    ...
                    for (let tries = 0; tries < 3; tries++) {
                        if (payAvail[prev]) {
                            this._confirmBuyPaymentType = prev;
                            break;
                        }
                        prev = (prev + 2) % 3;
                    }
        ...
        if (this._activeDialog !== 'none') {
            ...
            if (key === 'Enter') {
                switch (this._activeDialog) {
                    case 'nameChange':
                        this._confirmNameChange();
                        break;
                    case 'worldTransfer':
                        this._confirmWorldTransfer();
                        break;
                    case 'coupleName':
                        this._confirmCoupleName();
                        break;
                    case 'friendName':
                        this._confirmFriendName();
                        break;
                    case 'equipSlotExt':
                        this._confirmEquipSlotExt();
                        break;
                }
                return;
            }
            ...
            if (key === 'Backspace') {
                if (this._activeDialog === 'nameChange')
                    this._nameChangeNewName = this._nameChangeNewName.slice(0, -1);
                if (this._activeDialog === 'coupleName')
                    this._coupleNameValue = this._coupleNameValue.slice(0, -1);
                if (this._activeDialog === 'friendName')
                    this._friendNameValue = this._friendNameValue.slice(0, -1);
                return;
            }
            if (key.length === 1) {
                if (this._activeDialog === 'nameChange' && this._nameChangeNewName.length < 12)
                    this._nameChangeNewName += key;
                if (this._activeDialog === 'coupleName' && this._coupleNameValue.length < 12)
                    this._coupleNameValue += key;
                if (this._activeDialog === 'friendName' && this._friendNameValue.length < 12)
                    this._friendNameValue += key;
            }
            return;
        }
        ...
        if (this._searchActive) {
            ...
            if (key === 'Enter') {
                ...
                if (q.length > 0) {
                    this._searchResults = this._commodities.filter(c => c.onSale && c.name.toLowerCase().includes(q));
                    this._page = 0;
    ...
    onMouseMove(x, y) {
        ...
        if (ly >= STATUS_Y && ly < STATUS_Y + 49) {
            if (lx >= STATUS_X + 248 && lx < STATUS_X + 289)
                this._hoveredBtn = 'charge';
            else if (lx >= STATUS_X + 289 && lx < STATUS_X + 330)
                this._hoveredBtn = 'check';
            else if (lx >= STATUS_X + 330 && lx < STATUS_X + 371)
                this._hoveredBtn = 'coupon';
            else if (lx >= STATUS_X + 378 && lx < STATUS_X + 546)
                this._hoveredBtn = 'exit';
        }
        // Inventory tab hover
        const tabNames = ['Equip', 'Use', 'Setup', 'Etc', 'Cash'];
        const tabW = Math.floor(156 / tabNames.length);
    ...
    onMouseButton(x, y, down, _button) {
        ...
        if (this._activeTab === 8 && this._oneADayItemSN > 0 && this._handleOneADayClick(lx, ly))
            return;
        if (this._inventoryScrollbar?.handleMouseButton(lx - INV_X, ly - INV_Y - 160, down))
            return;
        if (this._lockerScrollbar?.handleMouseButton(lx - this._getLockerScrollbarX(), ly - 229, down))
            return;
        if (ly >= STATUS_Y && ly < STATUS_Y + 49) {
            if (lx >= STATUS_X + 248 && lx < STATUS_X + 289) {
            ...
            if (lx >= STATUS_X + 289 && lx < STATUS_X + 330) {
                ...
                if (selected)
                    this.game?.session.send(GameSender.CashShopPurchaseRecord(selected.sn));
                return;
            }
        ...
        if (this._confirmBuyVisible && this._confirmBuyItem) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (lx >= payX && lx < payX + payW && ly >= py && ly < py + payH) {
                    if (!payAvail[i])
                        return; // Don't select unavailable types
        ...
        if (this._activeDialog !== 'none' && this._handleActiveDialogClick(lx, ly))
            return;
        ...
        if (lx >= LOCKER_X && lx < LOCKER_X + LOCKER_COLS * LOCKER_COL_STEP) {
            if (ly >= scrollArrowY && ly < scrollArrowY + 14) {
                if (lx < LOCKER_X + LOCKER_COLS * LOCKER_COL_STEP / 2 && this._lockerScroll > 0) {
                    ...
                    const maxScroll = Math.max(0, Math.ceil(Math.max(0, this._lockerItems.length - LOCKER_COLS * LOCKER_ROWS) / LOCKER_COLS));
                    if (this._lockerScroll < maxScroll)
                        this._lockerScroll++;
                }
                return;
            }
    ...
    _handleActiveDialogClick(lx, ly) {
        let dlgW = 300;
        let dlgH = 160;
        if (this._activeDialog === 'nameChange') {
            dlgW = 266;
        ...
        else if (this._activeDialog === 'equipSlotExt')
            dlgH = 200;
        const dlgX = Math.floor((CS_W - dlgW) / 2);
        const dlgY = Math.floor((CS_H - dlgH) / 2);
        if (lx < dlgX || lx >= dlgX + dlgW || ly < dlgY || ly >= dlgY + dlgH) {
    ...
    _executeGift() {
        ...
        this.game.session.send(GameSender.CashShopSendGift('', // SPW (empty — not required for v95 client)
        this._giftItem.sn, false, // requestBuyOneADay
        this._giftReceiver, this._giftMessage));
    ...
    _confirmNameChange() {
        if (!this._nameChangeItem || !this.game)
            return;
        if (this._nameChangeNewName.length < 4) {
            this._statusMessage = 'Name too short.';
            return;
        }
    ...
    _confirmWorldTransfer() {
        if (!this._worldTransferItem || !this.game || this._worldTransferSelected < 0)
            return;
    ...
    _confirmCoupleName() {
        if (!this._coupleNameItem || !this.game)
            return;
        if (this._coupleNameValue.length < 2) {
            this._statusMessage = 'Name too short.';
            return;
        }
    ...
    _confirmFriendName() {
        if (!this._friendNameItem || !this.game)
            return;
        if (this._friendNameValue.length < 2) {
            this._statusMessage = 'Name too short.';
            return;
        }
    ...
    _confirmEquipSlotExt() {
        if (!this._equipSlotExtItem || !this.game)
            return;
    ...
    _executeBuy() {
        if (!this._confirmBuyItem || !this.game)
            return;
        const commodity = this._confirmBuyItem;
        // Dismiss dialog
        this._confirmBuyVisible = false;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    _handleCashItemResult(args) {
        switch (args.subAction) {
            case 0x54:
                this._statusMessage = `Limited: ${args.remainCount} left`;
                break;
            case 0x58: {
            ...
            case 0x6D:
                ...
                break;
            case 0x6F:
                this._statusMessage = `Storage expanded to ${args.trunkCount}`;
                break;
            case 0x77: {
            ...
            case 0x98:
                this._buyPending = false;
                this._statusMessage = 'Couple item sent!';
                break;
            case 0x9E:
                this._buyPending = false;
                this._statusMessage = 'Purchase complete!';
            ...
            case 0xA2:
                this._buyPending = false;
                this._statusMessage = 'Friendship item sent!';
                break;
            ...
            case 0xA3: // FriendShipFailed
                this._buyPending = false;
                this._statusMessage = this._noticeFailReason(args.reason);
                break;
            default: {
                const raw = args;
    ...
    _noticeFailReason(reason) {
        switch (reason) {
            ...
            default:
                if (reason >= 50)
                    return `Purchase failed (code ${reason}).`;
    ...
    _getCurrentPageItems() {
        ...
        return this._commodities.filter(c => {
            if (!c.onSale || !this._isSaleAvailable(c))
                return false;
            // Tab 0 (New/Best) and Tab 8 (Popular) show all items
            if (this._activeTab === 0 || this._activeTab === 8)
                return true;
    ...
    _exit() {
        if (this.game?.session.isConnected)
            this.game.session.send(GameSender.ReturnFromCashShop());
        this.stageDirector.pop();
    }
    _clearDynamic() {
        for (const t of this._dynamicTexts)
            t.destroy();
        this._dynamicTexts = [];
        for (const s of this._dynamicIcons)
            s.destroy();
    ...
    _addText(text, x, y, fill, fontSize) {
        const t = new Text({
            text,
    ...
    _loadAssets() {
        if (!this._ui || !this._loader)
            return;
        const cs = this._ui.GetItem('CashShop.img');
        if (!cs)
            return;
        const rootItems = cs.Root?.Items;
        if (!rootItems)
            return;
        const tryLoad = (path) => {
            try {
                const parts = path.split('/');
                let node = rootItems[parts[0]];
        ...
        const oneADay = this._ui.GetItem('OneADay.img');
        const oneADayItems = oneADay?.Root?.Items;
        const tryLoadOneADay = (path) => {
            if (!oneADayItems)
                return null;
            try {
                const parts = path.split('/');
                let node = oneADayItems[parts[0]];
                for (let i = 1; i < parts.length && node; i++)
                    node = node.Items?.[parts[i]];
                return node instanceof WzCanvas ? this._loader.Load(node) : null;
            }
        ...
        const tryLoadImage = (image, path) => {
            try {
                const pkg = this._ui.GetItem(image);
                const items = pkg?.Root?.Items;
                if (!items)
                    return null;
                const parts = path.split('/');
                let node = items[parts[0]];
                for (let i = 1; i < parts.length && node; i++)
                    node = node.Items?.[parts[i]];
                return node instanceof WzCanvas ? this._loader.Load(node) : null;
            }
...
//# sourceMappingURL=CashShopStage.js.map