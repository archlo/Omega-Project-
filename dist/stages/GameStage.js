...
import { ChatBar, FILTER_ALL, FILTER_PARTY, FILTER_GUILD, FILTER_ALLIANCE, FILTER_BUDDY, FILTER_EXPEDITION } from '../ui/game/ChatBar.js';
...
import { computeBasicStat, defaultBasicStatInput } from '../character/BasicStat.js';
import { ItemOptionLoader } from '../character/ItemOptionInfo.js';
export class GameStage extends Stage {
    _loader = new WzTextureLoader();
    ...
    _employees = new Map();
    _summons = new Map();
    _townPortals = new Map();
    _affectedAreas = new Map();
    _openGates = new Map();
    _mobCtl = new Map();
    _mobInfoSvc = null;
    _mobWz = null;
    ...
    _dmgNumbers = null;
    _shopMarker = null;
    _shopMarkerLayer = new Container();
    _skillEffects = null;
    _skillEffectLayer = new Container();
    _itemEffects = null;
    _itemEffectLayer = new Container();
    _skillScreenLayer = new Container();
    _projectiles = new ProjectileOverlay();
    _projectileLayer = new Container();
    /** couple-chair heart zone overlays: midpoint position + animation frames. */
    _coupleHearts = [];
    _coupleHeartLayer = new Container();
    ...
    onCoupleChairPairChanged = null;
    /** One-shot field effects (e.g. Summon.img animations at world positions). */
    _fieldFx = [];
    _fieldFxLayer = new Container();
    _fearEffect = new FearEffect();
    ...
    _dojangSpecialArts = false;
    _limitedView = new LimitedViewOverlay();
    _comboCounter = 0;
    _keyDownBar = new KeyDownBar();
    _comboDisplay = new ComboDisplay();
    _buffVisual = new BuffVisualOverlay();
    _buffVisualLayer = new Container();
    ...
    _entityLayer = new Container();
    _statusBar;
    _chatBar = new ChatBar();
    _chatTarget = 'all';
    _pendingLocalBalloon = null;
    _chatTab = 0;
    _miniMap;
    ...
    _partyCharIds = new Map();
    _buffList = new BuffList();
    _clock = new Clock();
    _killCountHud = new KillCountHud();
    _massacreGaugeHud = new MassacreGaugeHud();
    _questTimerHud = new QuestTimerHud();
    _fieldSubgameHud = new FieldSubgameHud();
    _monsterCarnival = null;
    _slideNotice = new SlideNotice();
    _partyHPBar = new PartyHPBar();
    _equip;
    _item;
    _itemIcons = null;
    _itemInfo = null;
    ...
    _skill;
    _stats;
    _prevExp = -1; // track EXP delta for popup display
    _job = 0;
    _quest;
    _medalQuestInfo = new MedalQuestInfo();
    _keyConfig;
    _skillIconCache = new Map();
    _optionMenu = new OptionMenu();
    _charInfo = null;
    _npcTalk = new NpcTalk();
    _shop = null;
    _trunk = null;
    _messengerWin = null;
    _gameMenu = null;
    ...
    _guildBBS = new GuildBBS();
    // OG: CConfig::IsInBlackList — TODO_AUDIT.md Eighty-second pass's
    // `CTabBlackList` finding. Local-only ignore list, not server state.
    _blackList = new Set();
    _statusMessenger = new StatusMessenger();
    _tipOfTheDay = new TipOfTheDay();
    _eventAlarm = new EventAlarm();
    _skillGuide = null;
    _questAlarm = new QuestAlarm();
    _dojangHud = new DojangHud();
    ...
    _miracleCube = null;
    _goldHammer = null;
    _megaphoneCompose = null;
    _karmaScissors = null;
    _itemProtector = null;
    _repair = null;
    _scrollDialog = null;
    _vegaDialog = null;
    _partySearchDialog = null;
    _questReward = null;
    _notice = null;
    _utilDlg = null;
    _antiMacroDialog = null;
    _adminShopNpcTemplateId = null;
    _chatBalloon = null;
    _tradingRoom = null;
    _cashTradingRoom = null;
    _personalShop = null;
    _entrustedShop = null;
    _memoryGame = null;
    _channelSelect = null;
    _quickSlotConfig = null;
    _quickSlots = null;
    ...
    _statDetailInfo = null;
    _tombstone = null;
    _quitOverlay = null;
    _contextMenu = null;
    _panels = [];
    _fadePhase = 0; // 0 idle, +1 fading to black, -1 fading in
    _fadeAlpha = 0; // 0 = clear .. 1 = opaque black
    _pendingField = null;
    _fadeOverlay = new Graphics();
    _field = null;
    _mapWz = null;
    _characterWz = null;
    _itemWz = null;
    ...
    _morphWz = null;
    _etcWz = null;
    _questStates = new Map();
    _pendingInviterId = 0;
    _hasPendingPartyInvite = false;
    _guildLoadSent = false;
    _friendLoadSent = false;
    ...
    _forcedStat = { str: 0, dex: 0, int: 0, luk: 0, speed: 0, jump: 0 };
    // ponytail: ItemOptionLoader for option/socket WZ data — loaded from Item.nx
    _itemOptionLoader = null;
    ...
    _localCharId = 0;
    _pendingQuestId = 0;
    _pendingQuestNpcId = 0;
    _attackCooldown = 0;
    _comboKeys = new SequencedKeyMan();
    _comboClockMs = 0;
    _comboCount = 0;
    _pendingBridle = null;
    _isPlayerDead = false;
    _fieldKey = 0;
    _isFieldTransferring = false;
    _townPortalStatus = '';
    _lastUnequipTime = 0;
    _isRidingTamingMob = false;
    _mobNameOf = () => '';
    _itemNameOf = () => '';
    _directionModeActive = false;
    _bg;
    constructor() {
        super();
    ...
    _moveChildren(target, source) {
        const kids = source.removeChildren();
        if (kids.length > 0)
            target.addChild(...kids);
    }
    draw() {
        ...
        if (this._field && this._camera) {
            ...
            this._field.UpdateEntities(this._otherChars, this._player, this._drops, this._mobs.values(), this._npcs, w, h);
            ...
            if (this._shopMarker) {
                ...
                const rebuilt = this._shopMarker.RebuildDisplay((name) => { for (const c of this._otherChars.values())
                    if (c.Name === name)
                        return c.Position; return null; }, (wx, wy) => this._camera.WorldToScreen(wx, wy));
            ...
            this._chatBalloon?.Draw((charId) => {
                if (charId === this._localCharId && this._player) {
                    ...
                    return this._camera.WorldToScreen(p.x, p.y === this._player.Position.y ? p.y - 60 : p.y);
                }
    ...
    onMouseWheel(x, y, deltaY) {
        for (const p of this._panels)
            p?.onMouseWheel?.(x, y, deltaY);
    }
    onKeyPress(key) {
        if (this._quitOverlay?.isVisible) {
            this._quitOverlay.onKeyPress?.(key);
            return;
        }
        if (this._gameMenu?.isVisible) {
            if (this._gameMenu.onKeyPress(key))
                return;
        }
        for (let i = this._panels.length - 1; i >= 0; i--) {
            if (this._panels[i]?.isVisible && this._panels[i].onKeyPress(key))
                return;
        }
        ...
        if (fk.type === 1 /* FuncKeyType.Skill */) {
            const rec = this._skillRecords.find((r) => r.skillId === fk.id);
            if (rec && rec.level > 0)
                this._skill.onSkillUse?.(rec.skillId, rec.level);
        }
        else if (fk.type === 8 /* FuncKeyType.MacroSkill */) {
        ...
        else if (fk.type === 4 /* FuncKeyType.Menu */) {
    ...
    _executeMenuAction(menuId) {
        switch (menuId) {
            ...
            case 1:
                ...
                if (this._item.isVisible && !this._item.container.parent)
                    this.uiRoot.addChild(this._item.container);
                break;
            case 2:
                this._stats.isVisible = !this._stats.isVisible;
                if (this._stats.isVisible) {
                    if (!this._stats.container.parent)
                        this.uiRoot.addChild(this._stats.container);
            ...
            case 5:
                if (this._worldMap)
                    this._worldMap.isVisible = !this._worldMap.isVisible;
                break; // WorldMap
            case 6:
                this._chatBar?.focus();
                break; // MapleChat
            ...
            case 11:
                ...
                break; // Whisper
            case 12:
                this._chatBar?.setChatTarget(2);
                this._chatBar?.focus();
                break; // PartyChat → party target
            case 13:
                this._chatBar?.setChatTarget(3);
                this._chatBar?.focus();
                break; // FriendsChat → buddy target
            case 14:
                if (this._gameMenu)
                    this._gameMenu.isVisible = !this._gameMenu.isVisible;
                break; // Game Menu
            case 15:
                if (this._quickSlotConfig)
                    this._quickSlotConfig.isVisible = !this._quickSlotConfig.isVisible;
                break; // QuickSlots
            case 16:
                this._chatBar?.toggleChat();
                break; // ToggleChat
            case 17:
                this._chatBar?.focus();
                break; // Guild
            case 18:
                this._chatBar?.setChatTarget(4);
                this._chatBar?.focus();
                break; // GuildChat → guild target
            case 19:
                this._chatBar?.focus();
                break; // Party
            case 20:
                this._chatBar?.addLine('[Notifier] Not yet implemented.', 12);
                break; // Notifier
            case 21:
                this._chatBar?.setChatTarget(7);
                this._chatBar?.focus();
                break; // SpouseChat → whisper target
            case 22:
                if (this.game.session.isConnected) {
                    this.game.session.send(GameSender.MigrateToCashShop());
                    this.stageDirector.push(new CashShopStage(this._uiWz));
                }
                break; // CashShop
            case 24:
                this._chatBar?.setChatTarget(5);
                this._chatBar?.focus();
                break; // AllianceChat → alliance target
            case 25: break; // ManageLegion — no-op
            case 26:
                if (this._familyWindow)
                    this._familyWindow.isVisible = !this._familyWindow.isVisible;
                break; // Family
            case 27: break; // BossParty — no-op
            case 29:
                this._chatBar?.focus();
                break; // ExpeditionChat
            case 44:
                if (this._charInfo) {
                    this._charInfo.isVisible = !this._charInfo.isVisible;
                    if (this._charInfo.isVisible && !this._charInfo.container.parent)
                        this.uiRoot.addChild(this._charInfo.container);
                }
                break;
            case 45:
    ...
    onMouseButton(x, y, down, _button) {
        if (!down)
        ...
        if (this._gameMenu?.isVisible) {
            if (this._gameMenu.handleMouseButton(x, y, down))
                return;
            if (down) {
                this._gameMenu.isVisible = false;
                return;
            }
        }
        if (!down && this._dragController.isDragging) {
            const payload = this._dragController.payload;
            const visible = this._panels.filter((p) => p?.isVisible).reverse();
            ...
            if (!claimed && payload && typeof payload === 'object' && 'invType' in payload) {
                ...
                else if (invType === 1 /* InventoryType.Equip */ || invType === 5 /* InventoryType.Cash */) {
                    const tab = invType === 1 /* InventoryType.Equip */ ? 0 : 1;
                    const free = this._item.firstFreeSlot?.(tab) ?? 0;
                    if (free > 0)
                        this.game.session.send(GameSender.ChangeSlotPosition(invType, p.slotPos, free, 1));
                }
            }
            return;
        }
        ...
        for (let i = this._panels.length - 1; i >= 0; i--) {
            ...
            if (p.beginDrag(lx, ly, down))
                return;
            if (p.handleMouseButton(x, y, down))
                return;
        }
        // Global mouse up for ChatBar scrollbar drag
        if (!down) {
            this._chatBar?.handleMouseButtonGlobal?.(x, y, down);
        }
        if (!down) {
            ...
            let other = null;
            for (const c of this._otherChars.values()) {
                if (c.HitTest(world.x, world.y)) {
                    other = c;
                    break;
                }
            }
    ...
    _showPlayerContextMenu(target, screenX, screenY) {
    ...
    onEnter(game) {
        ...
        for (const p of this._panels)
            if (p)
                this.uiRoot.addChild(p.container);
        ...
        this._loadWzAsync(game).catch((ex) => {
            console.warn('GameStage: async WZ load failed', ex);
        });
    }
    async _loadWzAsync(game) {
        const dir = game.wzDir ?? '/wz_client';
        const open = (name) => WzPackage.OpenBaseAsync(dir, name);
        try {
            this._mapWz = game.wz.map ?? await open('Map');
        }
        catch (ex) {
            console.warn('Failed to open Map.wz', ex);
        }
        try {
            ...
            try {
                ...
                console.warn('List package unavailable — skipping ListService', ex);
            }
            this._skillService = new SkillInfoService(() => this._skillWz);
        }
        catch (ex) {
            console.warn('Failed to open WZ files', ex);
        }
        ...
        actMan.Init();
        try {
            this._initMenu(this._uiWz);
        }
        catch (ex) {
            console.warn('_initMenu failed', ex);
        }
        try {
            this._wireNames(game);
        }
        catch (ex) {
            console.warn('_wireNames failed', ex);
        }
        ...
        if (this._field && this._miniMap) {
            ...
            this._miniMap.setPortals(Object.values(this._field.Portals).map((p) => ({ x: p.X, y: p.Y })));
            // OG: m_nMiniMapType — read from field info (0=simple, 1=normal)
            this._miniMap.setMiniMapType(this._field.Info.MiniMapType);
    ...
    _initMenu(uiWz) {
        ...
        this._skill = new SkillBook(this._loader, uiWz, font, this._itemIcons, (id) => this.game.nameService.ItemDesc(id) ?? null, (id) => {
            ...
            for (let i = entry.aLevelData.length - 1; i >= 0; i--) {
                if (entry.aLevelData[i].nLevel <= level) {
                    return entry.aLevelData[i];
                }
            }
            return entry.aLevelData[0];
        ...
        this._questDetail.OnResign = (id) => {
            ...
            if (this._questDetail)
                this._questDetail.isVisible = false;
        };
        this._questDetail.OnFindNpc = (_id) => { };
        ...
        this._gameMenu.onQuit = () => { if (this._quitOverlay)
            this._quitOverlay.isVisible = true; };
        ...
        this._quickSlots = new QuickSlotBar(this._loader, uiWz, font, (scancode) => this._keyConfig.bindingAt(scancode), (scancode, skillId) => this._keyConfig.bindSkillToKey(scancode, skillId), (skillId) => this._skillIcon(skillId), (itemId) => this._itemIcons?.LoadIcon(itemId) ?? null, (skillId) => this._skill.cooldownOf(skillId), (itemId) => this._isStateChangeItem(itemId), (itemId, invType) => this._isBindableItem(itemId, invType), () => this._itemIcons?.GetCashTag() ?? null, (itemId) => this._item.countItem(itemId));
        ...
        this._equip = new EquipInventory({
            ...
            setItemOf: (id) => {
                const data = this._itemInfo?.GetSetItemTooltip(id);
                return data ? { name: data.name, effects: data.effects.map((e) => ({ threshold: e.threshold, effect: e.effect })) } : null;
            },
        ...
        this._equip.onUnequip = (bodyPart) => {
            ...
            if ((this._stats.hp ?? 0) <= 0)
                return;
            // 2. 500ms cooldown between unequips
            const now = Date.now();
            if (now - this._lastUnequipTime < 500)
                return;
            ...
            this.game.session.send(GameSender.ChangeSlotPosition(1 /* InventoryType.Equip */, -bodyPart, free, 1));
        };
        ...
        this._item.onEquipItem = (item) => {
            ...
            this.game.session.send(GameSender.ChangeSlotPosition(1 /* InventoryType.Equip */, item.slot, -bodyPart, 1));
        };
        this._item.onActivateCashItem = (item) => {
            ...
            if (item.id >= 5000000 && item.id < 6000000) {
                ...
                this.game.session.send(GameSender.ChangeSlotPosition(5 /* InventoryType.Cash */, item.slot, -slot, 1));
                return;
            }
        ...
        this._item.onGather = (invType) => {
            if (this.game.session.isConnected)
                this.game.session.send(GameSender.GatherItemRequest(Date.now(), invType));
        };
        this._item.onSort = (invType) => {
            if (this.game.session.isConnected)
                this.game.session.send(GameSender.SortItemRequest(Date.now(), invType));
        };
        // OG: CUIItem::ItemRelease → CWvsContext::SendItemReleaseRequest(useSlot, equipSlot)
        this._item.onItemRelease = (useSlot, equipSlot) => {
            if (this.game.session.isConnected)
                this.game.session.send(GameSender.ItemReleaseRequest(useSlot, equipSlot));
        };
        ...
        this._item.onSplitItem = (item, qty) => {
            ...
            if (freeSlot <= 0) {
                console.warn('No free slot to split item');
                return;
            }
        ...
        this._keyDownBar.container.position.set(this.game.pixiApp.screen.width / 2, this.game.pixiApp.screen.height - 40);
        this.uiRoot.addChild(this._keyDownBar.container);
        this._comboDisplay.container.position.set(this.game.pixiApp.screen.width - 80, 60);
        ...
        this._rpsGame.onSendPacket = (subOpcode, data) => {
            switch (subOpcode) {
                case 0:
                    this.game.session.send(GameSender.RPSGameStart());
                    break;
                case 1:
                    this.game.session.send(GameSender.RPSGameSelection(data ?? 0));
                    break;
                case 2:
                    this.game.session.send(GameSender.RPSGameTimeout());
                    break;
                case 3:
                    this.game.session.send(GameSender.RPSGameContinue());
                    break;
                case 4:
                    this.game.session.send(GameSender.RPSGameExit());
                    break;
                case 5:
                    this.game.session.send(GameSender.RPSGameRetry());
                    break;
            }
        };
        ...
        if (uiWz) {
            const carnProp = uiWz.GetItem('UIWindow2.img/MonsterCarnival');
            if (carnProp) {
        ...
        this._cashTradingRoom.setResolvers((id) => this.game.nameService.ItemName(id) ?? `[${id}]`, (id) => this._itemIcons?.LoadIcon(id) ?? null);
        ...
        this._entrustedShop.setResolvers((id) => this.game.nameService.ItemName(id) ?? `[${id}]`, (id) => this._itemIcons?.LoadIcon(id) ?? null);
        ...
        this._panels.push(this._familyWindow, this._guildBBS, this._channelSelect, this._quickSlotConfig, this._statDetailInfo, this._trunk, this._messengerWin, this._revivePanel, this._worldMap, this._tournamentWindow, this._ranking, this._monsterBook, this._memo, this._battleRecord, this._titleWindow, this._maker, this._adminShop, this._storeBank, this._characterSale, this._weddingWishList, this._findFriend, this._shopScanner, this._incubator, this._rpsGame, this._logoutGift, this._parcel, this._wildHunterInfo, this._monsterCarnival, this._skillMacro, this._reset, this._delivery, this._claim, this._enchantSkill, this._miracleCube, this._goldHammer, this._scrollDialog, this._vegaDialog, this._karmaScissors, this._itemProtector, this._repair, this._megaphoneCompose, this._tradingRoom, this._cashTradingRoom, this._personalShop, this._entrustedShop, this._quickSlots, this._questDetail, this._skill, this._stats, this._quest, 
        ...
        // call).
        this._statusBar, this._miniMap, this._equip, this._item, this._keyConfig, this._questReward, this._notice, this._antiMacroDialog, this._questAlarm);
        ...
        if (equipTipContainer)
            this.uiRoot.addChild(equipTipContainer);
        const itemTipContainer = this._item.tooltipContainer;
        if (itemTipContainer)
            this.uiRoot.addChild(itemTipContainer);
        this._statusBar.onInfo = () => { if (this._charInfo)
            this._charInfo.isVisible = !this._charInfo.isVisible; };
        ...
        this._item.onUseItem = (item) => {
            ...
            if (category === 219) {
                ...
                if (target.length > 0)
                    this.game.session.send(GameSender.AntiMacroItemUseRequest(target, item.slot, item.id));
                return;
            }
            ...
            if (category === 224) {
                ...
                if (target.length > 0)
                    this.game.session.send(GameSender.MarriageRequest(target, item.id));
                return;
            }
            ...
            if (category === 232) {
                ...
                if (this._itemWz) {
                    ...
                    const desc = this._itemWz.GetItem(`Consume/${group}.img/${id}/desc`);
                    if (desc) {
        ...
        this._statusBar.onSkills = () => {
            this._skill.isVisible = !this._skill.isVisible;
            if (this._skill.isVisible && !this._skill.container.parent)
                this.uiRoot.addChild(this._skill.container);
        };
        ...
        this._skill.onMacroOpen = () => {
            const macros = this._macroSlots.map((s, i) => ({
                slot: i, name: s.name, mute: s.mute, skills: Array.from(s.skills),
            }));
        ...
        this._statusBar.onOptions = () => {
            ...
            if (this._optionMenu.isVisible && !this._optionMenu.container.parent)
                this.uiRoot.addChild(this._optionMenu.container);
        };
        this._statusBar.onRanking = () => {
            if (!this._ranking)
                return;
            this._ranking.isVisible = !this._ranking.isVisible;
            if (this._ranking.isVisible && !this._ranking.container.parent)
                this.uiRoot.addChild(this._ranking.container);
        };
        ...
        this._userList.onGuildLeave = () => {
            if (this._localCharId)
                this.game.session.send(GameSender.GuildLeave(this._localCharId, this._statusBar.charName));
        };
        ...
        this._userList.onGuildInvite = (name) => {
            ...
            if (target)
                this.game.session.send(GameSender.GuildJoin(target.CharId, name));
            else
        ...
        this._userList.onGroupWhisper = (_groupName) => {
            ...
            if (friendIds.length > 0) {
                ...
                if (msg)
                    this.game.session.send(GameSender.GroupChat(ChatGroupType.Friend, friendIds, msg));
            }
        };
        ...
        this._statusBar.onKeys = () => {
            this._keyConfig.isVisible = !this._keyConfig.isVisible;
            if (this._keyConfig.isVisible && !this._keyConfig.container.parent)
                this.uiRoot.addChild(this._keyConfig.container);
        };
        this._statusBar.onQuit = () => { if (this._quitOverlay)
            this._quitOverlay.isVisible = true; };
        ...
        this._statusBar.onCharacter = () => { if (this._charInfo)
            this._charInfo.isVisible = !this._charInfo.isVisible; };
        ...
        this._statusBar.onQuest = () => {
            this._quest.isVisible = !this._quest.isVisible;
            if (this._quest.isVisible && !this._quest.container.parent)
                this.uiRoot.addChild(this._quest.container);
        };
        ...
        this._chatBar.onSendChat = (msg) => {
            ...
            if (!msg.startsWith('/') && this._chatTarget === 'whisper') {
                ...
                if (target) {
                    ...
                    if (targetChar)
                        this._chatBalloon?.Set(targetChar.CharId, msg, 5, 2);
                }
                return;
            }
            if (!msg.startsWith('/') && this._chatTarget !== 'all') {
                ...
                if (prefix) {
                    this._handleChatCommand(prefix + msg);
                    return;
                }
            }
            if (!msg.startsWith('/') && this._chatTarget === 'all') {
        ...
        this._skill.onSkillUp = (_skillId) => { };
        this._skill.onSkillUse = (skillId, slv) => {
            ...
            if (action)
                this._player?.PlayOneTimeAction(action);
            const effect = cast?.Effect ?? cast?.Effect0;
            if (effect)
                this._skillEffects?.PlayAtCaster(effect, this._localCharId, this._physics?.FacingLeft ?? true);
            if (cast?.Screen)
                this._skillEffects?.PlayFullScreen(cast.Screen);
        };
        ...
        this._stats.onHpUp = () => { this.game.session.send(GameSender.UserAbilityUp(2048 /* MapleStat.MaxHp */)); };
        this._stats.onMpUp = () => { this.game.session.send(GameSender.UserAbilityUp(8192 /* MapleStat.MaxMp */)); };
        this._stats.onStrUp = () => { this.game.session.send(GameSender.UserAbilityUp(64 /* MapleStat.Str */)); };
        this._stats.onDexUp = () => { this.game.session.send(GameSender.UserAbilityUp(128 /* MapleStat.Dex */)); };
        this._stats.onIntUp = () => { this.game.session.send(GameSender.UserAbilityUp(256 /* MapleStat.Int */)); };
        this._stats.onLukUp = () => { this.game.session.send(GameSender.UserAbilityUp(512 /* MapleStat.Luk */)); };
        this._stats.onAutoApUp = (_mode) => { };
        this._stats.onDetailToggle = () => {
            if (!this._statDetailInfo)
                return;
            this._statDetailInfo.isVisible = this._stats.detailVisible;
        };
        this._reset.OnApUp = (stat) => {
            if (stat === 'str')
                this.game.session.send(GameSender.UserAbilityUp(64 /* MapleStat.Str */));
            else if (stat === 'dex')
                this.game.session.send(GameSender.UserAbilityUp(128 /* MapleStat.Dex */));
            else if (stat === 'int')
                this.game.session.send(GameSender.UserAbilityUp(256 /* MapleStat.Int */));
            else if (stat === 'luk')
                this.game.session.send(GameSender.UserAbilityUp(512 /* MapleStat.Luk */));
        };
    ...
    update(dt) {
        ...
        if (this._physics && this._keyConfig) {
            ...
            const isDown = (key) => held.has(key);
            const input = this._directionModeActive
            ...
            if (movePath)
                this.game.session.send(GameSender.UserMove(this._fieldKey, movePath));
        ...
        for (const drop of this._drops)
            drop.Update(dt);
        if (this._drops.some((d) => d.Finished))
            this._drops = this._drops.filter((d) => !d.Finished);
        for (const ch of this._otherChars.values())
            ch.Update(dt);
        ...
        if (this._petPickupTimer >= 0.5 && this._drops.length > 0) {
            ...
            if (playerPos) {
                for (const pet of localPets) {
                    if (!pet || pet.IsInPickupForbiddenMap(this._field?.LoadedMapId ?? 0))
                        continue;
                    if (!pet.CanPickupItem() && !pet.CanPickupMeso())
                        continue;
                    ...
                    for (const drop of this._drops) {
                        ...
                        if (dx * dx + dy * dy > range * range)
                            continue;
                        if (!pet.CanPickupMeso() && drop.IsMoney)
                            continue;
                        if (!pet.CanPickupItem() && !drop.IsMoney)
                            continue;
                        if (pet.IsInExceptionList(drop.ItemIdOrAmount))
                            continue;
                        pet.SendDropPickUpRequest(Math.round(drop.Position.x), Math.round(drop.Position.y), drop.DropId, 0);
        ...
        for (const [charId, dragon] of this._dragons) {
            const ownerPos = charId === this._localCharId
                ...
                : this._otherChars.get(charId)?.Position;
            if (ownerPos)
                dragon.SetOwnerPosition(ownerPos.x, ownerPos.y);
            dragon.Update(dt);
        }
        for (const s of this._summons.values())
            s.Update(dt);
        ...
        if (this._miniMap) {
            ...
            this._miniMap.setOtherPlayers(Array.from(this._otherChars.values(), (c) => ({ x: c.Position.x, y: c.Position.y })));
            ...
            this._miniMap.setMerchants(Array.from(this._employees.values(), (e) => ({ x: e.Position.x, y: e.Position.y })));
        }
        if (!this._isPlayerDead) {
            ...
            if (this._mobCtl.size > 0 && !window.__mobCtlLogged) {
                window.__mobCtlLogged = true;
        ...
        if (this._stats?.level !== undefined) {
            ...
            if (tip)
                this._statusMessenger.showTip(tip);
        }
        for (const p of this._panels) {
            p?.update(dt);
            p?.updateDrag();
    ...
    _wireNames(game) {
        ...
        const resolve = (fn) => (id) => fn?.call(ns, id) ?? '';
        ...
        if (this._item)
            this._item.nameOf = resolve(ns.ItemName.bind(ns));
        if (this._skill)
            this._skill.nameOf = resolve(ns.SkillName.bind(ns));
        if (this._quest)
            this._quest.nameOf = resolve(ns.QuestName.bind(ns));
        if (this._medalQuestInfo)
            this._medalQuestInfo.nameOf = resolve(ns.QuestName.bind(ns));
        if (this._questTimerHud)
            this._questTimerHud.nameOf = resolve(ns.QuestName.bind(ns));
    ...
    handleKeyDown(key) {
        if (key === 'Escape') {
            ...
            if (this._quitOverlay?.isVisible) {
                this._quitOverlay.isVisible = false;
                return true;
            }
    ...
    _wireHandlers(game) {
        ...
        fh.onNpcMove = ({ npcId, actionIdx, chatIdx, movePath }) => {
            ...
            if (actionIdx === -1) {
                ...
                const animName = npc._getActionName?.(actionIdx);
                if (animName) {
                    // OG: set m_nOneTimeAction and m_bSpecialAction, then PrepareActionLayer
                    npc._nOneTimeAction = actionIdx;
                    npc._bSpecialAction = false;
        ...
        fh.onNpcUpdateLimitedInfo = ({ npcId, enabled }) => {
            const npc = this._npcs.find(n => n.ObjId === npcId);
            if (npc)
                npc.OnUpdateLimitedInfo(enabled);
        };
        ...
        fh.onInventoryOperation = (ops) => {
            ...
            for (const op of ops) {
                if (op.opType !== 0 /* InventoryOpType.Add */)
                    continue;
                if (op.petTameness === undefined)
                    continue;
                const localPets = this._pets.get(this._localCharId) ?? [];
                for (const pet of localPets) {
                    if (!pet)
                        continue;
                    if (pet.TemplateId !== op.itemId)
                        continue;
                    if (op.petLevel !== undefined)
                        pet.SetLevel(op.petLevel);
        ...
        fh.onSkillCooltimeSet = (skillId, remainSec) => {
            if (remainSec > 0)
                this._skill.startCooldown(skillId, remainSec);
            else
                this._skill.clearCooldown(skillId);
        };
        ...
        fh.onFamilyJoinRequest = (args) => {
            if (this._blackList.has(args.inviterName))
                return;
            if (this._notice) {
        ...
        fh.onMarriageRequest = (args) => {
            ...
            if (args.requestType !== 0 || !args.partnerName || args.partnerId === undefined)
                return;
            if (this._notice) {
        ...
        this.game.mapleTVHandlers.onSetMessage = (args) => {
            ...
            for (const m of args.messages)
                if (m.length > 0)
                    this._chatBar.addLine(`[MapleTV ${who}] ${m}`);
        };
        ...
        fh.onAntiMacroResult = (args) => {
            if (args.subType === 6)
                this._antiMacroDialog?.showQuestion(args.jpeg);
            else
                this._antiMacroDialog?.showNotice(args.message ?? `Anti-macro check (code ${args.reasonCode})`);
        };
        ...
        fh.onItemExpireReplace = (messages) => {
            for (const msg of messages)
                this._statusMessenger.showLoot(msg);
        };
        ...
        fh.onOpenUrl = (args) => {
            ...
            if (!win)
                this._notice?.show('Open URL', args.url);
        };
        ...
        fh.onUserChat = (args) => {
            ...
            if (isLocalEcho)
                ...
                this._chatBalloon?.Set(args.charId, resolved, 5, 0);
        };
        ...
        fh.onAffectedAreaRemove = (objId) => { const aa = this._affectedAreas.get(objId); if (aa) {
            aa.container.removeFromParent();
            aa.container.destroy();
        } this._affectedAreas.delete(objId); };
        ...
        fh.onGroupMessage = (groupType, fromName, text, charId) => {
            if (this._blackList.has(fromName))
                return;
            ...
            switch (groupType) {
                case 2:
                    prefix = '[Party]';
                    lType = 1;
                    filterType = FILTER_PARTY;
                    break; // ChatType.GROUPPARTY
                case 3:
                    prefix = '[Buddy]';
                    lType = 2;
                    filterType = FILTER_BUDDY;
                    break; // ChatType.GROUPFRIEND
                case 4:
                    prefix = '[Guild]';
                    lType = 3;
                    filterType = FILTER_GUILD;
                    break; // ChatType.GROUPGUILD
                case 5:
                    prefix = '[Alliance]';
                    lType = 4;
                    filterType = FILTER_ALLIANCE;
                    break; // ChatType.GROUPALLIANCE
                case 26:
                    prefix = '[Expedition]';
                    lType = 26;
                    filterType = FILTER_EXPEDITION;
                    break; // ChatType.EXPEDITION
                default:
                    prefix = '[Group]';
                    lType = 0;
        ...
        fh.onWhisper = ({ fromName, channelId, text }) => {
            ...
            if (sender)
                this._chatBalloon?.Set(sender.CharId, this._resolveChatItemLinks(text), 4, 2);
        };
        fh.onPartyInvite = ({ inviterId, inviterName }) => {
            if (this._blackList.has(inviterName))
                return;
        ...
        fh.onMigrateCommand = (host, port) => {
            game.migration.beginMigrateAsync(host, port, this._localCharId).catch((ex) => {
        ...
        fh.onUserMove = (args) => {
            ...
            if (args.facingLeft !== undefined)
                other.SetFacing(args.facingLeft);
            if (args.stance !== undefined)
                other.SetStance(args.stance);
        };
        fh.onUserPassiveMove = (args) => {
            const other = this._otherChars.get(args.charId);
            if (!other || !args.movePath)
                return;
            other.SetMovePath(args.movePath);
        };
        ...
        fh.onOpenSkillGuide = () => {
            ...
            if (!this._skill.container.parent)
                this.uiRoot.addChild(this._skill.container);
        ...
        fh.onSetDirectionMode = (bDirection, afterDelay) => {
            ...
            if (bDirection) {
                ...
                setTimeout(() => {
                    ...
                    if (this._physics)
                        this._physics.SetDirectionMode(false);
                }, afterDelay);
            }
            else {
                this._directionModeActive = false;
        ...
        fh.onFieldEffect = (args) => {
            switch (args.subType) {
                ...
                case 3: {
                    ...
                    if (node)
                        this._skillEffects?.PlayFullScreen(node);
                    break;
                }
                case 4: {
                    // OG: play_field_sound — field ambient sound from Sound.wz
                    const sound = this._mobSoundWz?.GetItem(args.soundUol ?? '');
                    if (sound instanceof WzSound)
                        this.game.audioPlayer.PlayEffect(sound.AudioBytes);
                    break;
                }
                case 5:
                ...
                case 6: {
                    ...
                    if (sound instanceof WzSound)
                        this.game.audioPlayer.PlayLoop(sound.AudioBytes);
                    break;
                }
                case 7: {
                    ...
                    if (jobNode)
                        this._skillEffects?.PlayFullScreen(jobNode);
                    if (partNode)
                        this._skillEffects?.PlayFullScreen(partNode);
                    if (levNode)
                        this._skillEffects?.PlayFullScreen(levNode);
        ...
        fh.onClock = (args) => {
            switch (args.subType) {
                case 0:
                    if (args.fireNow)
                        this._statusMessenger.showLoot('[Event Timer] triggered');
                    break;
                case 1:
                    this._clock.setWallClock(args.hour ?? 0, args.minute ?? 0, args.second ?? 0);
                    break;
                case 2:
                    if (args.durationSec !== undefined && args.durationSec >= 0)
                        this._clock.startCountdown(args.durationSec);
                    else
                        this._clock.hide();
                    break;
                case 3:
                case 0x64:
                    if (args.active && args.durationSec !== undefined)
                        this._clock.startCountdown(args.durationSec);
                    else if (!args.active)
                        this._clock.hide();
                    break;
        ...
        fh.onSetQuestTime = (entries) => {
            for (const e of entries) {
                if (e.end === 0n)
                    this._questTimerHud.ClearTimer(e.questId);
                else
                    this._questTimerHud.SetTimerFromFiletime(e.questId, e.end);
            }
        };
        ...
        fh.onBroadcastMsg = (msgType, text) => {
            if (msgType === 4) {
                if (text)
                    this._slideNotice.show(text, this.game.pixiApp.screen.width);
                else
                    this._slideNotice.hide();
            }
            else {
        ...
        fh.onMobChangeController = (mobId, isCtrl) => {
            ...
            const mob = this._mobs.get(mobId);
            if (!mob) {
                console.log(`[MobCtrl] mob ${mobId} not in _mobs`);
                return;
            }
        ...
        fh.onMobSpecialEffectBySkill = ({ mobId, skillId, delay }) => {
            ...
            const skillNode = (skillWz.GetItem(`${String(job).padStart(3, '0')}.img/skill/${String(skillId).padStart(7, '0')}`)
                ?? skillWz.GetItem(`${job}.img/skill/${skillId}`));
            if (!skillNode)
                return;
            ...
            if (frames)
                this._fieldFx.push({ frames, frameIndex: 0, frameTimer: 0, x: mob.Position.x, y: mob.Position.y, done: false });
        };
        ...
        const bossTimerLine = (boss, args) => `[${boss}] ${args.value === 0 ? 'Boss summoned!' : `Phase timer (flag ${args.flag}): ${args.value}s`}`;
        ...
        fh.onPartyLoad = ({ members, bossId }) => {
            ...
            for (const m of members)
                this._partyCharIds.set(m.charId, m.charId === bossId);
        ...
        fh.onPartyBossChanged = (newBossCharId) => {
            for (const id of this._partyCharIds.keys())
                this._partyCharIds.set(id, id === newBossCharId);
        ...
        fh.onRadioSchedule = (musicFile, duration) => {
        ...
        fh.onUserTeleport = ({ x, y }) => {
            ...
            if (fhBelow) {
                ...
                if (gy !== null)
                    this._physics.Position.y = Math.min(this._physics.Position.y, gy);
            }
            this._camera.Target = this._physics.Position;
        };
        ...
        fh.onGuildSetMarkPrompt = () => {
            const ask = (label) => Number(window.prompt(label) ?? '0') | 0;
        ...
        fh.onSkillLearnItemResult = ({ isMasterybook, used, succeed }) => {
            if (succeed)
                this._notice?.show('Skill Book', isMasterybook ? 'Mastery successful!' : 'Skill learned!');
            else if (used)
                this._notice?.show('Skill Book', 'Skill book failed (already maxed or no SP).');
        };
        ...
        fh.onSkillPrepare = ({ charId, skillId }) => {
            ...
            if (node)
                this._skillEffects?.PlayHoldAtCaster(node, charId, facingLeft);
        };
        ...
        fh.onGatherItemResult = ({ invType, resultCode }) => {
            if (resultCode !== 0)
                this._notice?.show('Gather', `Gather failed (type ${invType}, code ${resultCode}).`);
            else {
        ...
        fh.onSortItemResult = ({ invType, resultCode }) => {
            if (resultCode !== 0)
                this._notice?.show('Sort', `Sort failed (type ${invType}, code ${resultCode}).`);
            else {
        ...
        fh.onMiniMapOnOff = ({ onOff }) => { if (this._miniMap)
            this._miniMap.isVisible = onOff; };
        ...
        fh.onWildHunterInfo = ({ packedByte, capturedMobIds }) => {
            ...
            this._skill.wildHunterMobNames = capturedMobIds.map((id) => this.game.nameService?.MobName(id) ?? `Mob ${id}`);
        ...
        fh.onQuiz = ({ isQuestion, category, problemId }) => {
            if (isQuestion)
                this._statusMessenger.showLoot(`[Quiz] Q${problemId} (cat ${category})`);
            else
                this._statusMessenger.showLoot(`[Quiz] Answer: ${problemId}`);
        };
        ...
        fh.onGivePopularityResult = ({ subResult, name, accepted, fame }) => {
            if (subResult === 0 && accepted)
                this._statusMessenger.showLoot(`[Fame] ${name} ${accepted ? '+' : '-'}${fame ?? 0} fame`);
            else
                this._statusMessenger.showLoot(`[Fame] result ${subResult}`);
        };
        fh.onMemoResult = ({ subAction, count, memos, msg, flag, name }) => {
            if (subAction === 3) {
                ...
                this._statusMessenger.showLoot(`[Memo] From ${name ?? 'unknown'}`);
            else
                this._chatBar.addLine(`[Memo] subAction ${subAction}`);
        };
        ...
        fh.onMapTransferResult = ({ subAction, isEx, mapIds, msg }) => {
            if (subAction === 2 || subAction === 3) {
                ...
                if (mapIds)
                    this._worldMap?.OpenMapTransfer(mapIds);
            }
            else
                this._chatBar.addLine(`[Map Transfer] subAction ${subAction}`);
        };
        ...
        fh.onSetAvatarMegaphone = ({ charId, name, messages, whisperBg, whisper, avatarLook, lastUpdate }) => {
            if (name)
                this._chatBar.addLine(`[Megaphone] ${name}: ${messages?.join(' ') ?? ''}`);
        };
        ...
        fh.onNewYearCardRes = ({ subAction, cards, sendResultCode, senderName, cardText, sendDate }) => {
            if (subAction === 2)
                this._statusMessenger.showLoot(`[New Year] ${cards?.length ?? 0} cards`);
            else if (subAction === 6)
                this._notice?.show('New Year Card', `${senderName ?? 'Someone'}: ${cardText ?? ''}`);
            else
                this._chatBar.addLine(`[New Year] subAction ${subAction}`);
        };
        ...
        fh.onDragonBallBox = ({ remainTime, showUI, close, ableToSummon, orbCount }) => {
            if (close)
                this._statusMessenger.showLoot('[Dragon Ball] Closed');
            else
                this._statusMessenger.showLoot(`[Dragon Ball] ${remainTime}s orbs:${orbCount ?? 0} summon:${ableToSummon}`);
        };
        fh.onUserChatHistory = ({ charId, text }) => {
            ...
            if (charName)
                this._chatBar.addLine(`${charName}: ${text}`);
            else
                this._chatBar.addLine(`[Chat History] char ${charId}: ${text}`);
        };
        fh.onUserADBoard = ({ charId, message }) => {
            const other = this._otherChars.get(charId);
            other?.SetADBoard(message);
            if (other)
                this._statusMessenger.showLoot(`[AD] ${other.Name}: ${message}`);
            else
                this._statusMessenger.showLoot(`[AD] char ${charId}: ${message}`);
        };
        fh.onSetConsumeItemEffect = ({ charId, itemId }) => {
            ...
            if (node)
                this._skillEffects?.PlayAtCaster(node, charId);
        ...
        fh.onUserHitByUser = ({ charId, damage }) => {
            ...
            if (pos)
                this._dmgNumbers?.Add(damage, pos.x, pos.y - 10, DamageKind.MobDamage);
        ...
        fh.onUserSetActiveEffectItem = ({ charId, itemId }) => {
            const other = this._otherChars.get(charId);
            if (itemId > 0)
                other?.SetStatusBadge('activeItem', 'E', 8);
            else
                other?.ClearStatusBadge('activeItem');
            ...
            if (node)
                this._skillEffects?.PlayLoopAtCaster('activeItem', node, charId, other?.FacingLeft ?? true);
            else
                this._skillEffects?.CancelLoopAtCaster('activeItem', charId);
        ...
        fh.onUserReceiveHP = ({ charId, curHP, maxHP }) => {
            ...
            const pos = other?.HeadPosition ?? other?.Position;
            if (pos)
                this._dmgNumbers?.Add(curHP, pos.x, pos.y - 10, DamageKind.HealHp);
        ...
        fh.onOpenUIWithOption = (uiType, option) => {
            if (uiType === 7) {
        ...
        fh.onMobStatSet = ({ mobId, statMask, statMaskHigh, stats }) => {
            ...
            if (names.length)
                this._chatBar.addLine(`[MobStat] ${mobId}: ${names.join(', ')}`);
        };
        fh.onMobStatReset = ({ mobId, statMask, statMaskHigh }) => {
            this._mobs.get(mobId)?.ClearStatusBadge('stat');
            const names = describeMobStatMask(statMask, statMaskHigh);
            if (names.length)
                this._chatBar.addLine(`[MobStat Reset] ${mobId}: ${names.join(', ')}`);
        };
        fh.onMobSuspendReset = ({ mobId, isSuspended }) => {
            if (isSuspended)
                this._statusMessenger.showLoot(`[Mob] ${mobId} doom/suspend reset`);
        };
        ...
        fh.onMobIncChargeCount = ({ mobId, chargeCount, attackReady }) => {
            const mob = this._mobs.get(mobId);
            if (mob)
                mob.OnIncChargeCount(chargeCount, attackReady);
        };
        ...
        fh.onNpcTemplatePacket = ({ npcId, bMove }) => {
            const npc = this._npcs.find((n) => n.ObjId === npcId);
            if (npc)
                this._statusMessenger.showLoot(`[NPC] ${npc.Name || npcId} template ${bMove ? 'move' : 'update'}`);
            this._chatBar.addLine(`[NPC Template] ${npcId} move ${bMove}`);
        };
    }
    _ensureDragon(charId) {
    ...
    static _keyName(code) {
        if (GameStage._KEY_NAMES[code])
            return GameStage._KEY_NAMES[code];
        if (code >= 48 && code <= 57)
            return String.fromCharCode(code); // 0-9
        if (code >= 65 && code <= 90)
    ...
    _resolveChatItemLinks(text) {
        const ns = this.game.nameService;
        return text
            // Types 7/9: #i/#v/#e + numeric id → [ItemName]
            .replace(/#[ive](\d+)#/g, (_m, id) => `[${ns.ItemName(Number(id)) ?? id}]`)
            // Type 10: #s + skillId → [SkillName]
            .replace(/#s(\d+)#/g, (_m, id) => `[${ns.SkillName(Number(id)) ?? id}]`)
    ...
    _applyFieldChange(args) {
        ...
        if (this._townPortalStatus)
            this._fieldSubgameHud.SetMessage(this._townPortalStatus);
    ...
    _restoreFieldState() {
        ...
        if (info.FieldType === 14) {
            ...
            if (this._stats) {
                this._dojangHud.updatePlayerStats(this._stats.hp ?? 0, this._stats.maxHp ?? 0, this._stats.mp ?? 0, this._stats.maxMp ?? 0);
            }
        }
    ...
    _onMobEnter(args) {
        ...
        if (args.maxHp != null && args.maxHp > 0)
            mob.MaxHp = args.maxHp;
        if (args.curHp != null)
            mob.Hp = args.curHp;
        if (this._mobInfoSvc) {
            const info = this._mobInfoSvc.Get(args.templateId);
            if (mob.MaxHp <= 0)
                mob.MaxHp = info.MaxHp;
        ...
        if (args.controllerFlag)
            this._createMobController(args.mobId, mob);
    ...
    _onMobMove(args) {
        ...
        if (path.elements.length > 0) {
            ...
            mob._movePathIndex = 0;
            mob._movePathTimer = 0;
    ...
    _mapMoveActionToState(moveAction) {
    ...
    _onEmployeeLeave(objId) {
        const emp = this._employees.get(objId);
        if (emp) {
            emp.container.removeFromParent();
            emp.container.destroy();
        }
        this._employees.delete(objId);
    }
    _onSummonedEnter(args) {
    ...
    _onSummonedMove(args) {
        const s = this._summons.get(args.objId);
        if (!s)
            return;
        s.ReplayMove(args.movePath);
    }
    _onTownPortalEnter(args) {
    ...
    _onTownPortalLeave(args) {
        const tp = this._townPortals.get(args.objId);
        if (tp) {
            tp.container.removeFromParent();
            tp.container.destroy();
        }
        this._townPortals.delete(args.objId);
    }
    _setTownPortalStatus(message) {
    ...
    _handleChatCommand(line) {
        ...
        const rest = (cmd) => line.length > cmd.length ? line.substring(cmd.length).trim() : '';
        if (lower === '/accept') {
        ...
        if (lower === '/partysearch' || lower === '/lfg') {
            if (this._partySearchDialog)
                this._partySearchDialog.Open();
            return;
        }
        if (lower === '/vega') {
            if (this._vegaDialog)
                this._vegaDialog.Open();
            return;
        }
        ...
        if (lower.startsWith('/w ') || lower.startsWith('/whisper ')) {
            ...
            if (sp > 0) {
                ...
                if (text.length > 0)
                    this.game.session.send(GameSender.Whisper(target, text));
            }
            return;
        }
        if (lower.startsWith('/dropmeso ')) {
            ...
            if (Number.isFinite(amount) && amount > 0)
                ...
                this._statusMessenger.showLoot('Usage: /dropmeso <amount>');
            return;
        }
        if (lower.startsWith('/apup ')) {
            const args = rest('/apup ').split(/\s+/);
            const statMap = { str: 64 /* MapleStat.Str */, dex: 128 /* MapleStat.Dex */, int: 256 /* MapleStat.Int */, luk: 512 /* MapleStat.Luk */ };
        ...
        if (lower.startsWith('/m ')) {
            const msg = rest('/m ');
            if (msg.length > 0)
                this.game.session.send(GameSender.MessengerChat(msg));
            return;
        }
        ...
        if (lower === '/tournament') {
            if (this._tournamentWindow)
                this._tournamentWindow.isVisible = !this._tournamentWindow.isVisible;
            return;
        }
        if (lower === '/maker') {
    ...
    static AutoTouchPortalTypes = new Set([1, 2, 3, 4, 5, 6, 9]);
    static PortalTouchRadiusX = 20;
    static PortalTouchRadiusYUp = 100;
    static PortalTouchRadiusYDown = 10;
    _checkPortalTouch() {
        if (this._isFieldTransferring || !this._field || !this._physics)
            return;
        const pos = this._physics.Position;
        ...
        let foundHiddenIdx = null;
        for (const portal of Object.values(this._field.Portals)) {
            if (portal.Type !== 10 && portal.Type !== 11)
                continue;
        ...
        for (const portal of Object.values(this._field.Portals)) {
            if (!GameStage.AutoTouchPortalTypes.has(portal.Type))
                continue;
            if (!portal.TargetPortal && portal.TargetMap === 999999999)
                continue;
            const dx = Math.abs(pos.x - portal.X);
    ...
    _tryMeleeAttack() {
        ...
        for (const mob of this._mobs.values()) {
            ...
            if (hx1 < minX || hx0 > maxX || hy1 < minY || hy0 > maxY)
                continue;
        ...
        const blob = MeleeAttackEncoder.Encode(this._fieldKey, actionAndDir, 6, pos.x, pos.y, targets, 1);
    ...
    _registerAfterimage(pos, facingLeft, attackAction) {
    ...
    _applyPendingEquipped() {
        if (this._pendingEquipped) {
            for (const { slot, item } of this._pendingEquipped) {
                const bodyPart = this._bodyPartFromEquippedPos(1 /* InventoryType.Equip */, slot);
                if (bodyPart > 0) {
                    ...
                    if (item.equip)
                        this._equipStats.set(bodyPart, item.equip);
        ...
        if (this._pendingEquippedCash) {
            for (const { slot, item } of this._pendingEquippedCash) {
                const bodyPart = this._bodyPartFromEquippedPos(5 /* InventoryType.Cash */, slot + 100);
                if (bodyPart > 0) {
                    const name = this.game.nameService.ItemName(item.itemId) ?? `[${item.itemId}]`;
                    this._equip.setEquippedByBodyPart(bodyPart, item.itemId, name, item.equip?.grade ?? 0, item.equip ?? undefined);
                }
    ...
    _applyEquipOps(ops) {
        let equipOps = 0;
        for (const op of ops) {
            if (op.invType !== 1 /* InventoryType.Equip */ && !(op.invType === 5 /* InventoryType.Cash */ && (op.pos < 0 || (op.newPos ?? 0) < 0)))
                continue;
            equipOps++;
            switch (op.opType) {
                case 0 /* InventoryOpType.Add */:
                    if (op.pos < 0 && op.itemId !== undefined) {
                        this._setEquippedAvatarItem(op.invType, op.pos, op.itemId, op.equipStats);
                    }
                    break;
                case 2 /* InventoryOpType.Move */: {
                    const newPos = op.newPos;
                    if (newPos === undefined)
                        break;
                    if (op.pos < 0)
                        this._clearEquippedAvatarItem(op.invType, op.pos);
                    if (newPos < 0) {
                        const tab = Math.max(0, op.invType - 1);
                        const itemId = this._item.itemIdAt(tab, newPos);
                        const invItem = this._item.itemAt(tab, newPos);
                        if (itemId > 0)
                            this._setEquippedAvatarItem(op.invType, newPos, itemId, invItem?.equipStats);
                    }
                    break;
                }
                case 3 /* InventoryOpType.Remove */:
                    if (op.pos < 0)
                        this._clearEquippedAvatarItem(op.invType, op.pos);
                    break;
            }
    ...
    _bodyPartFromEquippedPos(invType, pos) {
        const raw = -pos;
        return invType === 5 /* InventoryType.Cash */ && raw >= 101 && raw <= 159 ? raw - 100 : raw;
    }
    _setEquippedAvatarItem(invType, pos, itemId, equipStats) {
        ...
        if (!look || bodyPart <= 0 || bodyPart > 59)
            return;
        if (invType === 5 /* InventoryType.Cash */) {
            if (bodyPart === 11) {
    ...
    _clearEquippedAvatarItem(invType, pos) {
        const bodyPart = this._bodyPartFromEquippedPos(invType, pos);
    ...
    _isBindableItem(itemId, invType) {
        const cat = Math.floor(itemId / 10000);
        if (invType === 2) {
    ...
    _pendingMobControllers = [];
    _createMobController(mobId, mob) {
        ...
        const info = this._mobInfoSvc.Get(mob.TemplateId);
        if (!info) {
            console.log(`[MobCtrl] NO INFO for template ${mob.TemplateId}`);
            return;
        }
        ...
        mc.onAttackPlayer = (dmg) => {
            if (this._stats.hp !== undefined && this._stats.hp > 0) {
                ...
                if (mgReduction > 0 && this._stats.mp !== undefined && this._stats.mp > 0) {
                    ...
                    if (mpDamage > this._stats.mp) {
                        mpDamage = this._stats.mp;
                        hpDamage = dmg - mpDamage;
                    }
    ...
    _onNpcEnter(args) {
        if (this._npcs.some(n => n.ObjId === args.objId))
            return;
    ...
    _onUserEnter(args) {
        ...
        if (args.guildName) {
            ch.SetGuildInfo(args.guildName, args.guildMarkBg ?? 0, args.guildMarkBgColor ?? 0, args.guildMark ?? 0, args.guildMarkColor ?? 0);
        }
        // Medal item from hairEquip slot 49 (OG m_avatarLook.anHairEquip[49])
        if (args.look) {
            const medalId = args.look.hairEquip.get(49 /* BodyPartSlot.Medal */);
            if (medalId)
                ch.SetMedalItemId(medalId);
        }
    ...
    _spawnPetsForOwner(ownerCharId, petIds) {
        ...
        if (ownerCharId === this._localCharId)
            this._syncEquipPetCount();
    }
    _removePetsForOwner(ownerCharId) {
    ...
    _applyPetActivated(args) {
        ...
        if (args.charId === this._localCharId) {
            ...
            const localPets = pets.filter(Boolean);
    ...
    _onDropEnter(args) {
        ...
        const drop = new DropSprite(args.dropId, args.isMoney, args.itemIdOrAmount, { x: args.sourceX ?? args.x, y: args.sourceY ?? args.y }, { x: args.x, y: args.y }, args.animated ?? false, icon);
    ...
    // removal.
    _onDropLeave(args) {
        const isPickup = args.leaveType === 2 /* DropLeaveType.PickedUpByUser */
            || args.leaveType === 3 /* DropLeaveType.PickedUpByMob */
            || args.leaveType === 5 /* DropLeaveType.PickedUpByPet */;
        if (isPickup && args.pickUpId === this._localCharId && this._player) {
            const drop = this._drops.find((d) => d.DropId === args.dropId);
            if (drop) {
                drop.StartAbsorb(() => this._player.Position);
                return;
            }
        }
    ...
    _getOptionContributions(optionId, itemLevel) {
        if (optionId <= 0)
            return null;
        const entry = this._itemOptionLoader?.loadItemOption(optionId);
        if (!entry || entry.aLevelData.length === 0)
    ...
    _getSocketContributions(socketId, itemLevel) {
        if (socketId <= 0)
            return null;
    ...
    _computeWeaponOption(stats, itemLevel) {
        CUserLocal.applyWeaponOption(stats.option1, stats.option2, stats.option3, itemLevel, (id) => this._itemOptionLoader?.loadItemOption(id) ?? null);
    }
    ...
    _getDefenseOptionData(bodyPart) {
        const stats = this._equipStats.get(bodyPart);
        if (!stats)
            return null;
        const itemLevel = stats.level > 0 ? stats.level : (this._itemIcons?.LoadAttr([...(this._equip?.equippedSlots() ?? [])].find(s => s.bodyPart === bodyPart)?.itemId ?? 0)?.ReqLevel ?? 0);
        return CUserLocal.getDefenseOptionData(stats.option1, stats.option2, stats.option3, itemLevel, (id) => this._itemOptionLoader?.loadItemOption(id) ?? null);
    }
    // ponytail: full BasicStat::SetFrom pipeline (OG 0x732BA0)
    _syncStatDetailInputs() {
        if (!this._statDetailInfo)
            return;
        ...
        for (const { itemId, bodyPart } of this._equip.equippedSlots()) {
            ...
            if (stats) {
                ...
                if (opt) {
                    ...
                    ratePctMhp += opt.mhpR;
                    ratePctMmp += opt.mmpR;
                    ratePctWatk += opt.padR;
                    ratePctMatk += opt.madR;
                    ratePctPdd += opt.pddR;
                    ratePctMdd += opt.mddR;
                }
                ...
                if (opt2) {
                    ...
                    pddBonus += opt2.pdd;
                    mddBonus += opt2.mdd;
                    equipSpeed += opt2.speed;
                    equipJump += opt2.jump;
                    ratePctStr += opt2.strR;
                    ratePctDex += opt2.dexR;
                    ratePctInt += opt2.intR;
                    ratePctLuk += opt2.lukR;
                    ratePctMhp += opt2.mhpR;
                    ratePctMmp += opt2.mmpR;
                    ratePctWatk += opt2.padR;
                    ratePctMatk += opt2.madR;
                    ratePctPdd += opt2.pddR;
                    ratePctMdd += opt2.mddR;
                }
                ...
                if (opt3) {
                    ...
                    pddBonus += opt3.pdd;
                    mddBonus += opt3.mdd;
                    equipSpeed += opt3.speed;
                    equipJump += opt3.jump;
                    ratePctStr += opt3.strR;
                    ratePctDex += opt3.dexR;
                    ratePctInt += opt3.intR;
                    ratePctLuk += opt3.lukR;
                    ratePctMhp += opt3.mhpR;
                    ratePctMmp += opt3.mmpR;
                    ratePctWatk += opt3.padR;
                    ratePctMatk += opt3.madR;
                    ratePctPdd += opt3.pddR;
                    ratePctMdd += opt3.mddR;
                }
                ...
                if (sock2) {
                    ...
                    equipInt += sock2.intt;
                    equipLuk += sock2.luk;
                    equipMhp += sock2.maxHp;
                    equipMmp += sock2.maxMp;
                    watk += sock2.watk;
                    matk += sock2.matk;
                    accBonus += sock2.acc;
                    evaBonus += sock2.eva;
                    pddBonus += sock2.pdd;
                    mddBonus += sock2.mdd;
                    equipSpeed += sock2.speed;
                    equipJump += sock2.jump;
                }
        ...
        if (ratePctWatk !== 0)
            watk += Math.floor(watk * ratePctWatk / 100);
        if (ratePctMatk !== 0)
            matk += Math.floor(matk * ratePctMatk / 100);
        if (ratePctPdd !== 0)
            pddBonus += Math.floor(pddBonus * ratePctPdd / 100);
        if (ratePctMdd !== 0)
            mddBonus += Math.floor(mddBonus * ratePctMdd / 100);
        ...
        const weaponId = this._equip.equippedWeaponItemId;
        if (weaponId !== null)
            inp.weaponType = getWeaponType(weaponId);
        ...
        const movement = this._itemInfo?.GetMovementProfile(shoeItemId, vehicleItemId, vehicleEquipIds, morphId);
    ...
    _onStatChanged(args) {
        ...
        if (args.maxHp !== undefined) {
            this._statusBar.maxHp = args.maxHp;
            this._stats.maxHp = args.maxHp;
        }
        if (args.mp !== undefined) {
            this._statusBar.mp = args.mp;
            this._stats.mp = args.mp;
        }
        if (args.maxMp !== undefined) {
            this._statusBar.maxMp = args.maxMp;
            this._stats.maxMp = args.maxMp;
        }
        ...
        if (args.extendedSp !== undefined)
            this._skill.setExtendedSp(args.extendedSp);
        if (args.str !== undefined)
            this._stats.str = args.str;
        if (args.dex !== undefined)
            this._stats.dex = args.dex;
        if (args.int !== undefined)
            this._stats.intStat = args.int;
        if (args.luk !== undefined)
            this._stats.luk = args.luk;
        if (args.ap !== undefined)
            this._stats.ap = args.ap;
        if (args.pop !== undefined) {
            this._stats.fame = args.pop;
            if (this._charInfo)
                this._charInfo.fame = args.pop;
        }
        if (args.job !== undefined) {
            ...
            if (this._statusBar)
                this._statusBar.jobName = jobName;
        }
        ...
        if (look) {
            ...
            if (args.face !== undefined)
                look.face = args.face;
            if (args.hair !== undefined)
                look.hair = args.hair;
        }
        this._equip?.SetPlayerStats(args.level ?? this._stats.level, args.str ?? this._stats.str, args.dex ?? this._stats.dex, args.int ?? this._stats.intStat, args.luk ?? this._stats.luk, args.job ?? 0);
        this._item?.SetPlayerStats(args.level ?? this._stats.level, args.str ?? this._stats.str, args.dex ?? this._stats.dex, args.int ?? this._stats.intStat, args.luk ?? this._stats.luk, args.job ?? 0);
        // TODO_AUDIT.md Hundred-and-fourteenth pass: keep StatDetailInfo.Inputs
        // in sync whenever any relevant stat changes.
        this._syncStatDetailInputs();
    }
    _onSkillRecordResult(records) {
    ...
    _computeMasteryFromSkills() {
        ...
        for (const r of this._skillRecords) {
            ...
            const node = (skillWz.GetItem(`${String(job).padStart(3, '0')}.img/skill/${String(r.skillId).padStart(7, '0')}`)
                ?? skillWz.GetItem(`${job}.img/skill/${r.skillId}`));
            if (!node)
                continue;
            const m = node.Get(`level/${r.level}`)?.Get('mastery');
            if (typeof m === 'number')
                total += m;
        }
        return total;
    }
    _onTemporaryStatSet(entries) {
        ...
        for (const e of entries) {
            ...
            if (e.skillId === 136 /* MobSkillType.Fear */ && this._physics) {
                const p = this._camera.WorldToScreen(this._physics.Position.x, this._physics.Position.y);
    ...
    _onQuestRecord(args) {
        ...
        if (args.state === 0)
            this._questStates.delete(args.questId);
        else
            this._questStates.set(args.questId, args.state);
    ...
    _onScriptMessage(args) {
        switch (args.msgType) {
            ...
            case 5: // ASK_MENU
                if (args.menu)
                    this._npcTalk.showMenu(args.text, args.menu);
                break;
            case 3: // ASK_TEXT
    ...
    _onShopOpen(args) {
        if (args.items) {
            ...
            this._shop.setResolvers((id) => this.game.nameService.ItemName(id) ?? `[${id}]`, (id) => this._itemIcons?.LoadIcon(id) ?? null);
    ...
    _onShopResult(args) {
        ...
        if (t === 14 /* ShopResultType.NotEnoughMesos */) {
            this._notice?.show('Shop', `Not enough mesos (short ${args.shortfall}).`);
        }
        else if (t === 15 /* ShopResultType.NotEnoughItems */) {
            this._notice?.show('Shop', `Not enough items (short ${args.shortfall}).`);
        }
        else if (t === 19 /* ShopResultType.NoItemsInStock */ && args.message) {
    ...
    _onEntrustedShopCheckResult(args) {
        ...
        switch (args.subType) {
            ...
            case 18:
                if (args.message)
                    this._notice?.show('Hired Merchant', args.message);
                break;
            case 9:
            case 10:
            case 11:
    ...
    _onTrunkResult(args) {
        if (!this._trunk)
            return;
        const toTrunkItems = (raw) => raw.map((i) => ({
            name: this.game.nameService.ItemName(i.itemId) ?? `[${i.itemId}]`,
            itemId: i.itemId,
        ...
        if (args.resultType === 22 /* TrunkResultType.Open */) {
    ...
    _updateCoupleChairs() {
        ...
        const pairedThisFrame = new Set();
        for (let i = 0; i < sitters.length; i++) {
            if (pairedThisFrame.has(sitters[i].charId))
                continue;
            for (let j = i + 1; j < sitters.length; j++) {
                if (pairedThisFrame.has(sitters[j].charId))
                    continue;
                if (sitters[i].itemId !== sitters[j].itemId)
                    continue;
        ...
        for (const [charId, pair] of this._couplePairs) {
            ...
            if (!pairCh || pairCh.PortableChairItemId !== pair.itemId) {
                ...
                if (Math.abs(dx) > 200 || Math.abs(dy) > 80)
                    deadPairs.add(charId);
            }
        }
        for (const charId of deadPairs) {
            ...
            this._coupleHearts = this._coupleHearts.filter((h) => h.a !== charId && h.b !== charId);
        }
    }
    ...
    _updateNpcChat(dt) {
        ...
        if (text)
            this._chatBalloon?.Set(npc.ObjId, text, 5, 1);
    }
    ...
    _onUserEffect(args) {
        if (args.effectType !== 14 && args.effectType !== 20)
            return;
        let uol;
        try {
        ...
        if (args.isLocal && this._comboCounter > 0) {
            ...
            if (indexed) {
                this._skillEffects?.PlayAtCaster(indexed, charId, facingLeft);
                return;
            }
        }
        const node = this._effectWz?.GetItem(wzPath);
    ...
    _dispatchCashItem(slot, itemId, itemName) {
        ...
        switch (t) {
            ...
            case 67:
                ...
                if (this._scrollDialog && this._itemIcons)
                    this._scrollDialog.setScrollIcon(this._itemIcons.LoadIcon(itemId));
                return;
            case 71:
    ...
    _onUserAttack(args) {
        ...
        if (attacker && !attacker.PlayAttackCode(args.action))
            attacker.Attack();
        if (args.attackType === 'magic' && attacker && args.skillId === 2221006 && args.targets.length > 0) {
            ...
            for (const target of args.targets) {
                ...
                if (mob)
                    points.push({ x: mob.Position.x, y: mob.Position.y - 40 });
            }
        ...
        switch (action) {
            case 5: // MRP_EnterResult
                if (args.roomType === 3 && this._tradingRoom) {
                    ...
                    const shopItems = (args.items ?? []).map((it, idx) => ({
                        index: idx,
                    ...
                    this._memoryGame.Open(args.title ?? '', args.myPosition ?? 0, args.users ?? [], args.maxUsers ?? 2, false);
                }
            ...
            case 10: // MRP_Leave
                if (this._tradingRoom?.isVisible)
                    this._tradingRoom.OnPartnerLeave();
                if (this._personalShop?.isVisible)
                    this._personalShop.isVisible = false;
                if (this._entrustedShop?.isVisible)
                    this._entrustedShop.isVisible = false;
                break;
            case 15: // TRP_PutItem
            ...
            case 16: // TRP_PutMoney
                if (this._tradingRoom)
                    this._tradingRoom.OnPartnerPutMoney(args.money ?? 0);
                break;
            case 17: // TRP_Trade
                if (this._tradingRoom)
                    this._tradingRoom.OnPartnerTrade();
                break;
            ...
            case MiniRoomProtocolFull.MGP_TurnUpCard:
                this._memoryGame?.OnTurnUpCard(args.cardIndex ?? 0, args.cardType ?? 0, args.showState ?? 0, args.userIndex ?? 0);
                break;
    ...
    _onMessengerResult(args) {
        if (!this._messengerWin)
            return;
        switch (args.action) {
            case 0 /* MessengerAction.Open */:
                this._messengerWin.Open();
                if (args.userIndex !== undefined && args.name)
                    this._messengerWin.SetParticipant(args.userIndex, args.name);
                break;
            case 1 /* MessengerAction.Join */:
                if (args.userIndex !== undefined)
                    this._messengerWin.SetSelf(args.userIndex);
                break;
            case 2 /* MessengerAction.Leave */:
                if (args.userIndex !== undefined)
                    this._messengerWin.RemoveParticipant(args.userIndex);
                break;
            case 3 /* MessengerAction.Invite */:
                if (args.name)
                    this._statusMessenger.showLoot(`[Messenger] Invite from ${args.name} (ch.${args.channel ?? 0})`);
                break;
            case 4 /* MessengerAction.Hide */:
                if (args.name)
                    this._statusMessenger.showLoot(`[Messenger] ${args.name} is ${args.flag ? 'online' : 'offline'}`);
                break;
            case 5 /* MessengerAction.DeclineInvite */:
                if (args.name)
                    this._statusMessenger.showLoot(`[Messenger] ${args.name} declined your invite`);
                break;
            case 6 /* MessengerAction.Chat */:
                if (args.chat)
                    this._messengerWin.AddChat(args.chat);
                break;
            case 8 /* MessengerAction.MigratedIn */:
                this._messengerWin.Reset();
                for (const m of args.migrated)
                    this._messengerWin.SetParticipant(m.index, m.name);
                break;
        }
    }
}
...
function describeSecondaryStatMask(mask) {
    return describeMask(mask, SecondaryStatNames);
}
function describeMobStatMask(maskLow, maskHigh = 0n) {
...
function describeMask(mask, names) {
    const out = [];
    for (let i = 0; i < names.length; i++) {
        if (((mask >> BigInt(i)) & 1n) !== 0n)
            out.push(names[i]);
    }
    if (out.length === 0 && mask !== 0n)
        out.push(`mask ${mask}`);
    return out;
}
