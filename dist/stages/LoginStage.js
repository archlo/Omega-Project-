...
export class LoginStage extends Stage {
    ...
    static SIGNBOARD_CENTER = { x: 371, y: 316 };
    static CAMERA_OFFSET = { x: 28, y: -8 };
    static ID_OFFSET = { x: 15, y: 16 };
    static PW_OFFSET = { x: 15, y: 41 };
    static LOGIN_BTN_OFFSET = { x: 181, y: 15 };
    static CHECK_OFFSET = { x: 17, y: 69 };
    static SAVE_TEXT_OFFSET = { x: 33, y: 69 };
    static LOST_ID_OFFSET = { x: 103, y: 69 };
    static LOST_PW_OFFSET = { x: 173, y: 69 };
    static NEW_OFFSET = { x: 13, y: 93 };
    static HOME_OFFSET = { x: 88, y: 93 };
    static QUIT_OFFSET = { x: 163, y: 93 };
    _statusLabel = '';
    _statusText;
    _idField = null;
    _pwField = null;
    ...
    constructor(ui, map, sound, loader) {
        super();
    ...
    onMouseButton(x, y, down, button) {
        ...
        if (this._saveCheck?.handleMouseButton(x, y, down))
            return;
    ...
    _buildLoginPanel() {
        ...
        this._makeButton('BtLoginIDSave', signTopLeft, LoginStage.SAVE_TEXT_OFFSET, () => {
            if (this._saveCheck) {
                this._saveCheck.isChecked = !this._saveCheck.isChecked;
                this._saveCheck.refresh();
            }
        });
    ...
    _loadCursor() {
        try {
            ...
            if (node instanceof WzCanvas) {
                ...
                if (spr)
                    this.game.cursor.setCursorTexture(spr.Texture);
            }
        }
    ...
    _wireBtnSounds() {
        ...
        for (const b of this._allButtons) {
            if (hover)
                b.onHoverSound = hover;
            if (click)
                b.onClickSound = click;
        }
    }
    _loadCanvas(path) {
        try {
            ...
            if (!(item instanceof WzCanvas)) {
                console.warn(`LoginStage: ${path} → ${item === null ? 'null' : item?.constructor?.name ?? typeof item}`);
                return null;
            }
    ...
    _makeButton(name, signTopLeft, offset, onClick) {
    ...
    onCheckPasswordResult(args) {
        ...
        if (args.eulaRequired) {
            this._errorNotice?.show('EULA required. Please accept the End User License Agreement.'
    ...
    onAccountInfoResult(args) {
        if (args.resultType !== 14 && args.resultType !== 15)
            return;
        this._statusLabel = 'SSN/DOB registration required — disconnect.';
        this._errorNotice?.show('SSN/Date of Birth registration required.\nPlease register at maplestory.nexon.net');
...
//# sourceMappingURL=LoginStage.js.map