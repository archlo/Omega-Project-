...
if ($fullData -match "data: (\{.*\})") {
    ...
    try {
        ...
        # The decompile tool returns structuredContent.code
        ...
        if ($code) {
            ...
            Write-Output "NO_CODE_FIELD"
            # Check content field
            $content = $json.result.content
            if ($content) {
                Write-Output "CONTENT_TYPE: $($content[0].type)"
                $innerJson = $content[0].text | ConvertFrom-Json
                Write-Output "INNER_DECOMPILED_LENGTH: $($innerJson.decompiled.Length)"
                Write-Output "INNER_TRUNCATED: $($innerJson.decompile_truncated)"
                
                $header = "// CUIStat::CreateTip @ 0x866530`n// Decompiled from v95 IDB`n`n"
                $output = $header + $innerJson.decompiled
                [System.IO.File]::WriteAllText("C:\Users\jorge\OneDrive\Desktop\ts\ida_output\cuistat_CreateTip_clean.txt", $output, [System.Text.Encoding]::UTF8)
                Write-Output "FILE_WRITTEN: $(([System.IO.FileInfo]"C:\Users\jorge\OneDrive\Desktop\ts\ida_output\cuistat_CreateTip_clean.txt").Length) bytes"
            }
        }
